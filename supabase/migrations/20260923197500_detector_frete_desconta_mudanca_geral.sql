-- D-399 — O detector de frete desconta a mudança GERAL da faixa no sinal de
-- histórico (refina D-397 depois dos 90 dias recuperados por D-396).
--
-- Medido em produção em 24/09/2026, com o frete de 25/06 a 23/09 inteiro: a
-- mediana do frete subiu na semana de 24/08 nas quatro contas ao mesmo tempo
-- (R$ 7,95 -> R$ 8,45 na faixa de R$ 40 a 79; R$ 13,85 -> R$ 14,45 na de R$ 79
-- a 120). É a tabela do Mercado Livre, não um anúncio. Sozinha ela fica abaixo
-- do limiar de 15% do histórico, mas SOMA à mudança de cada anúncio, e o
-- motivo "22% acima do que pagava" misturava as duas.
--
-- Medida pelos anúncios que venderam nas duas janelas (mediana de frete agora
-- ÷ frete antes), a mudança geral foi de +5,2% (R$ 40 a 79), +4,6% (até R$ 40),
-- +4,3% (R$ 79 a 120), +2,9% (R$ 120 a 200), +3,1% (R$ 200 a 400) e +1,0%
-- (acima de R$ 400), com o intervalo interquartil de cada faixa em poucos
-- pontos -- mudança de tabela, não de anúncio. Descontada, 12 dos 27 alertas de
-- histórico ficam abaixo de 15%.
--
-- Agora o histórico compara o frete atual com o ESPERADO = frete de antes x a
-- mudança geral da faixa. O "frete a mais" do histórico também passa a ser
-- contra o esperado. A resposta ganha `frete_esperado` e
-- `variacao_geral_faixa` em cada alerta, e `variacao_geral` e
-- `anuncios_comparados` em cada faixa. O resto da função é o de D-397.
...
set formula = 'soma dos pontos de cinco sinais (histórico do anúncio contra o frete esperado -- o de antes corrigido pela mudança geral da faixa --, outros anúncios do mesmo SKU, pares da categoria do Mercado Livre na faixa de preço, frete ÷ preço contra o p95 da faixa, margem); 0 normal, 1-2 atenção, 3-4 provável problema, 5+ forte indício',
    definition_updated_on = date '2026-09-24'
where id = 'nivel_anomalia_frete';

update public.metric_definitions
set formula = 'Σ frete dos pedidos dos últimos 14 dias − referência × pedidos; referência = frete esperado do anúncio (o de antes corrigido pela mudança geral da faixa), dos outros anúncios do mesmo SKU ou dos pares, a primeira que pontuou; somado nos alertas provável e forte',
    definition_updated_on = date '2026-09-24'
where id = 'frete_excedente_estimado';
