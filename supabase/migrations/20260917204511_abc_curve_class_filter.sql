...
--
-- ------------------------------------------------------------
-- POR QUE ESTA MIGRATION EXISTE DUAS VEZES, COM CARIMBOS DIFERENTES
-- ------------------------------------------------------------
-- Ela foi aplicada FORA do repositorio nos dois ambientes, por branch de
-- preview do Supabase, e o arquivo nunca foi commitado:
--
--   Dev (nmgccyqquwxecqffsidr)     20260917204511
--   producao (imvjfgnaprqsfjlnsyev) 20260917204505
--
-- O `db push` recusa versao remota sem arquivo local ("Remote migration
-- versions not found in local migrations directory"), entao a CI do Dev ficou
-- vermelha desde 17/09 e o workflow de producao pararia no mesmo erro. Por
-- isso as DUAS versoes entram como arquivo, com o MESMO corpo.
--
-- O corpo e IDEMPOTENTE de proposito: cada ambiente ja tem uma das duas
-- aplicada, e a outra vai rodar em cima do estado final. Os `drop ... if
-- exists` das DUAS assinaturas (10 e 11 parametros) fazem a segunda execucao
-- terminar no mesmo lugar, sem erro.
--
-- Licao registrada: migration aplicada por branch de preview sem commit deixa
...
drop function if exists public.get_sku_abc_curve(uuid, date, date, uuid, text, boolean, integer, integer, text, uuid);
drop function if exists public.get_sku_abc_curve(uuid, date, date, uuid, text, boolean, integer, integer, text, uuid, text);
...
create function public.get_sku_abc_curve(
  ...
  -- Classe opcional, aplicada apos o ranking para preservar a curva ABC.
...
as $$
  ...
  filtrada as (
    ...
    where (not p_only_without_full or full_quantity = 0)
      ...
      and (p_abc_class is null or abc_class = p_abc_class)
  )
...
comment on function public.get_sku_abc_curve(uuid, date, date, uuid, text, boolean, integer, integer, text, uuid, text) is
  'Curva ABC de SKUs com filtros de conta, marca, SKU, Full e classe. O ranking e preservado antes do filtro de classe; totais e classes saem de janelas sobre o conjunto filtrado, ANTES do limit. security invoker.';

revoke all on function public.get_sku_abc_curve(uuid, date, date, uuid, text, boolean, integer, integer, text, uuid, text) from public, anon;
grant execute on function public.get_sku_abc_curve(uuid, date, date, uuid, text, boolean, integer, integer, text, uuid, text) to authenticated, service_role;
