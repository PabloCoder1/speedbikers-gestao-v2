-- ============================================================
-- ETAPA 32 — validação de /compras encontrou divergência real
-- ============================================================
--
-- get_purchase_planning_validation_sample comparou o
-- suggested_purchase_quantity calculado em SQL contra
-- calculatePurchaseRecommendation() em TypeScript para uma amostra de
-- 76 linhas. 25 divergiram — sempre por exatamente 1 unidade, às
-- vezes para cima, às vezes para baixo (ex.: SKU 20017: SQL=497,
-- TS=496; SKU 602047: SQL=243, TS=244).
--
-- Causa raiz: avg_daily_sales_30 é `numeric` no Postgres, com a
-- precisão decimal completa da divisão (ex.: 31.53333333333333...).
-- Esse valor sai da função, vira JSON pelo PostgREST e chega em
-- JavaScript como `number` (double IEEE754). A conversão para double
-- perde os últimos dígitos decimais — e como o mesmo valor entra num
-- `ceil()`, um erro de ponto flutuante ínfimo pode empurrar o
-- resultado para o lado errado da fronteira de um inteiro. O SQL
-- calcula suggested_purchase_quantity com a precisão original (antes
-- de virar JSON); o TypeScript recalcula com a precisão já reduzida —
-- daí a divergência de exatamente 1 unidade.
--
-- Correção: arredondar avg_daily_sales_30 para 6 casas decimais DENTRO
-- da função, antes de ele alimentar tanto o ceil() do SQL quanto a
-- serialização para JSON. Isso não muda a fórmula (lead time, reserva
-- mínima, exclusão de Full/transferência continuam idênticos) — só
-- estabiliza a precisão do único número intermediário que atravessava
-- a fronteira double/numeric, de modo que os dois lados concordam
-- sempre. 6 casas decimais equivalem a uma diferença de menos de
-- 0,0001% em qualquer venda diária real; nenhuma sugestão de compra
-- muda por causa da precisão em si, só a inconsistência é eliminada.
--
-- Verificado após aplicar: 0 divergências em nova amostra de
-- validação (ver auditoria/RELATORIO.md).
--
-- Corpo idêntico a 20260818114954_fix_purchase_history_ready_and_speed_up_signals.sql,
-- exceto pela linha marcada abaixo.
  ...
  scored as (
    select
      ...
      case
        ...
        -- MUDANÇA (20260818150000): arredondado para 6 casas decimais.
        -- Antes deste round(), esta divisão saía com toda a precisão
        -- decimal do tipo `numeric`, que se perde de forma inconsistente
        -- ao virar JSON/double no cliente — causando divergência de ±1
        -- unidade no suggested_purchase_quantity entre SQL e TypeScript
        -- exatamente nas linhas cuja demanda cai perto de uma fronteira
        -- inteira. 6 casas decimais são precisão de sobra para "vendas
        -- médias por dia" e eliminam a fronteira dupla/numeric.
