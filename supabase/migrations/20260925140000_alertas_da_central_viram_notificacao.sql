-- D-411 — O alerta novo de severidade alta da central vira notificação.
--
-- `sincronizar_alertas_central` (D-403/D-404) grava os alertas em `actions`,
-- e até aqui ninguém era avisado: o alerta esperava alguém abrir /acoes ou a
-- central. Agora o episódio NOVO de severidade alta grava, na mesma
-- transação, um `domain_event` `central.alert.opened` -- e o fan-out de D-073
-- faz o resto: notificação para quem alcança a conta (o alerta de produto não
-- tem conta e vai para a organização inteira, o caminho de evento sem conta
-- que existe desde D-054), toast em tempo real e o filtro por família na
-- Central de Notificações (D-393).
--
-- Medido em produção (25/09): no primeiro dia a carga inicial abriu 15
-- episódios de severidade alta (12 produtos no prejuízo, 2 fretes, 1
-- campanha); no segundo, nenhum. Por isso "importante", não "crítico", e só o
-- episódio NOVO: o que continua aberto não notifica todo dia. Os episódios já
-- abertos antes desta migration não geram notificação retroativa.
--
-- O corpo é o de D-404; muda só o bloco 5.3, que passa a devolver os
-- episódios criados, e o resumo ganha `notificadas`.
...
begin
  ...
  -- D-411: o episódio novo de severidade ALTA vira um `domain_event`
  -- `central.alert.opened` na mesma transação -- e o fan-out de D-073 o
  -- transforma em notificação para quem alcança a conta (sem conta, a
  -- organização inteira), com toast e triagem na Central de Notificações.
  -- Um evento por episódio (`dedup_key` pelo id da ação): o que continua
  -- aberto não notifica de novo; o que volta depois de fechado é outro
  -- episódio, e notifica.
  with d as (
    select * from jsonb_to_recordset(v_detectado) as x(
      kind text, subject_key text, severity text, confidence text, impacto numeric,
  ...
  novos as (
    ...
    returning id, kind, severity, ml_account_id, sku_id, mlb_id, subject_key, estimated_impact_brl, recommendation
  ),
  eventos as (
    ...
    select
      p_organization_id, n.ml_account_id, now(), 'central.alert.opened', 'action', n.id::text,
      null,
      jsonb_build_object(
        'kind', n.kind,
        ...
        'sku_id', n.sku_id,
        'mlb_id', n.mlb_id,
        'impacto', n.estimated_impact_brl,
        'recomendacao', n.recommendation,
        'detectado_em', v_hoje),
      'importante', 'system', 'central.alert.opened:' || n.id::text
    from novos n
    where n.severity = 'alta'
    on conflict (dedup_key) do nothing
    returning id
  )
  select (select count(*) from novos), (select count(*) from eventos)
  into v_criadas, v_notificadas;
...
comment on function public.sincronizar_alertas_central(uuid, date, text[]) is
  'D-403: grava em actions os alertas da central -- frete provável/forte (get_detector_frete), campanha crítica/abaixo da meta (get_sinais_ads) e produto no prejuízo em 30 dias com 3 pedidos cobertos (get_ranking_produtos) -- um episódio por assunto: atualiza o aberto, respeita o fechado por uma pessoa enquanto a condição continua, abre um novo depois de 3 dias sem detecção, e encerra o aberto que sumiu há 3 dias numa fonte que rodou. D-404: p_fontes escolhe as fontes (NULL = as três). D-411: o episódio novo de severidade alta grava um domain_event central.alert.opened (importante), que o fan-out transforma em notificação. Só o worker chama (service_role).';
