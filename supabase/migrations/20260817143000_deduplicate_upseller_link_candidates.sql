-- Deduplicate equivalent physical link candidates inside each relationship chunk.
-- The unique key remains authoritative; different source_sku_key values remain
-- separate so promote_links can still detect true physical-SKU conflicts.
    ...
    if current_phase = 'build_links_item' then
      ...
      with selected as (
        ...
        select organization_id, ml_account_id, product_id, user_product_id,
          id as target_id, 'listing'::text as target_kind
        from public.ml_listings
        where is_current and product_id is not null and user_product_id is not null
        union all
        select organization_id, ml_account_id, product_id, user_product_id,
          id as target_id, 'variation'::text as target_kind
        from public.ml_listing_variations
        where is_current and product_id is not null and user_product_id is not null
      ), candidates as (
        select distinct on (target.product_id, relation.source_sku_key)
          batch.organization_id as organization_id,
          batch.id as import_id,
          target.product_id,
          relation.source_sku,
        ...
        where relation.channel = 'mercado_livre' and relation.listing_external_id like 'MLBU%'
          and not exists (select 1 from public.upseller_unresolved_kits unresolved
            where unresolved.organization_id = relation.organization_id
              and unresolved.source_sku_key = relation.source_sku_key and unresolved.is_current)
        order by target.product_id, relation.source_sku_key, relation.row_number,
