...
- **A recusa com essa causa não é elegível para retomada**, e o resumo do corpo do erro põe `item.variations.relist.invalid` antes das outras causas (com teto para `message`/`error`), para o corte de ~800 caracteres nunca apagá-la. O painel não oferece outro pedido para o anúncio cuja conferência reprovou por `VARIACOES_USER_PRODUCT` com variações no retrato. **Lacunas A1–A4 da revisão da primeira versão — corrigidas (D-369):** CLOSING com pai ativo sem conferência (A1), causa cortada do resumo (A2), bloqueio por variação que falhava aberto (A3), painel repetindo o motivo e oferecendo pedido (A4).
