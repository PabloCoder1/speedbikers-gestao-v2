...
| Tags da conta (modelo de user products) | `GET /users/me` — `tags` contém `user_product_seller` | token da conta | — | secao 2.3 | 2026-09-17 (D-369, secao 2.16) |
...
**Rate limit observado em produção (D-156, 2026-08-31)**: ~850 chamadas em rajada por conta produziam 429 sustentado em ~85% das execuções (medido em D-143; ~22 execuções falhas/dia, cada tentativa do Cloud Tasks recomeçando do zero). Não há número oficial de limite (D-042). Mitigação no worker: espaçamento de 150 ms entre chamadas + checkpoint pela própria `daily_listing_visits` (item sincronizado nas últimas 12h é pulado, então cada tentativa SOMA progresso). Ajustar o espaçamento por medição, nunca por chute.
...
| `gross_amount`, `receiver.cost`, `receiver.discounts[]`, `senders[].discounts[]` | `GET /shipments/{id}/costs` | **Conferido em 12 envios reais (24/09, D-407):** o frete cheio; o que o comprador pagou; o que o Mercado Livre bancou dele (`loyal`, `ratio`, `gap`); o desconto do Mercado Livre no frete do vendedor (`mandatory`, 30% ou 50%). Cada desconto tem `rate` e `promoted_amount`. `gross_amount` = as quatro partes somadas pelo `promoted_amount` — não pelo `save`, que veio menor em 2 das 12 |
...
**O incidente (16/09/2026 18:41 UTC) — a causa do 400 não foi registrada, e o corpo com variações só rodou contra o ML real em 17/09 (parágrafo seguinte):** o MLB1476804187 (10 variações, todas com estoque) foi fechado e o POST saiu com o corpo de item sem variação. O ML respondeu **400**, a operação ficou em RELIST_FAILED e o anúncio fora do ar, sem filho. O corpo do erro não foi gravado, então o corpo errado é a causa provável, não a comprovada — o que está CONFIRMADO é só o contrato da doc acima. As variações desse item têm `user_product_id` e `inventory_id`, sobre os quais a seção de relist não diz nada. Desde D-364: o corpo sai de `buildRelistBody` (`@sb/domain`), variação sem estoque fica fora (listada na confirmação da tela) e o preflight bloqueia antes de fechar quando não há estoque (`VARIACOES_SEM_ESTOQUE`/`SEM_ESTOQUE`); o POST sai com **uma tentativa** no cliente HTTP (`maxAttempts: 1`), para o 4xx que o worker vê ser o do único POST; e a recusa 4xx (exceto 408/429) grava `POST_RECUSADO` com o resumo do corpo do erro (`message`, `error`, `cause[]`) e fica elegível para uma retomada **humana**.

**A recusa real (17/09/2026 13:36–13:37 UTC) — CONFIRMADO: o ML não aceita relist de item COM variações de vendedor no modelo de user products (D-369).** As duas retomadas da operação `a7638dc5` (13:36:37 e 13:37:03) mandaram o corpo oficial com variações para o MLB1476804187 — pai já fechado, 10 variações, cada uma com `user_product_id` próprio e nenhum na raiz — e as duas levaram **400** `validation_error`, causa `item.variations.relist.invalid`, mensagem *"Relist item with variations are not allowed for user product seller"* (o `failure_reason` gravado é o da segunda). O anúncio segue fechado, sem filho e sem caminho de relist. Se o 400 de 16/09 (corpo sem variações) tinha a mesma causa não se sabe: o corpo do erro não foi gravado.

**A doc oficial de User Products fala do assunto — a resposta é a regra dela aplicada ao relist.** A página de relist não menciona user products. A de User Products (`developers.mercadolivre.com.br/pt_br/user-products`, perguntas frequentes, atualizada em 17/06/2026; cópia baixada em 17/09, `pt_user-products.txt` linhas 637, 661 e 724-726) não cita o relist, mas: identifica o vendedor já no modelo de "Preço por Variação" pela tag `user_product_seller` na API `/users`; e responde que, depois da ativação, o array `variations` não pode mais ser enviado, porque cada variação passa a ser uma condição de venda (item diferente). **CONFIRMADO em 17/09 (só GET em `/users/me`): as quatro contas de produção — gmr, sbmotos, speedbikers-loja-1 e speedbikers-loja-2 — têm a tag.** (A primeira versão desta seção dizia que as páginas de User Products não mencionavam a regra; estava errado.)

- **Regra da casa desde D-369 — pela CONTA, e falha fechada:** item COM variações bloqueia `VARIACOES_USER_PRODUCT` quando a conta tem a tag `user_product_seller` (lida em `GET /users/me` com o token da conta, só para item com variações e nenhuma com `user_product_id`) OU quando alguma variação traz `user_product_id`; bloqueia `USER_PRODUCT_NAO_VERIFICADO` quando a tag não pôde ser lida. Item sem variações segue as regras de antes. A regra roda antes de todo ato remoto: no pedido (snapshot do multiget), no re-preflight de REQUESTED antes do PUT, na retomada de CLOSING com o pai ainda ativo (CLOSE_FAILED, sem PUT) e na retomada humana antes do POST. O `GET /items/{id}` sem `include_attributes` traz `user_product_id` em cada variação (CONFIRMADO: leitura só-GET do MLB1476804187 em 17/09; o `parent_snapshot` da `a7638dc5`, capturado pelo multiget, também o tem).
- **A recusa com essa causa não é elegível para retomada**, e o resumo do corpo do erro põe `item.variations.relist.invalid` antes das outras causas (com teto para `message`/`error`), para o corte de ~800 caracteres não apagá-la. `cause` como objeto único ou texto conta como causa, e o código achado em qualquer texto do corpo JSON que o resumo perderia (fim de `message`/`error` longo, outro campo) entra solto à frente das causas; o código seguido de ponto final também é reconhecido. Corpo que não é JSON (HTML, vazio) não é legível: nesse caso a recusa parece retomável, e só a conferência do anúncio ao vivo na retomada segura o POST. O painel não oferece outro pedido para o anúncio cuja conferência reprovou por `VARIACOES_USER_PRODUCT` com variações no retrato. **Lacunas A1–A4 da revisão da primeira versão — corrigidas (D-369):** CLOSING com pai ativo sem conferência (A1), causa cortada do resumo (A2), bloqueio por variação que falhava aberto (A3), painel repetindo o motivo e oferecendo pedido (A4). Da segunda revisão, também corrigidas: a leitura do retrato pela página sem teste (B1/R1), o teto de `error` sem teste (B2) e as formas de corpo que apagavam a causa (R2).
- **Sem variações, com `user_product_id` na raiz, continua permitido.** CONFIRMADO em um par medido em 17/09: MLB4604611355 → MLB5249227873 manteve o MESMO user product no filho, e a experiência de compra — que mora no user product (`/reputation/items/{id}/purchase_experience/integrators` responde 302 para `/reputation/user_products/{UP}/...`) — veio junto (Média 65 nos dois). INFERIDO, não medido: que o mesmo valha para os outros itens sem variação em conta de user products.
...
| **Item com variações em conta de user products** | ❌ recusado: 400 `item.variations.relist.invalid` (resposta real, 17/09/2026, D-369) — a página de relist não diz; a de User Products diz que `variations` não é enviável depois da tag `user_product_seller` |
...
- **Nenhum código de erro documentado** para `/relist`, nem HTTP status de sucesso. Por isso, desde D-364, o resumo do corpo do erro que o ML devolve vai para o log (`relist_post_rejected`/`relist_post_failed`) — é a única fonte do motivo. No `failure_reason` da operação ele só entra na **recusa** (`POST_RECUSADO`); nas demais falhas (`POST_FALHOU`) a linha guarda só a mensagem do cliente HTTP, e o corpo fica no log.
