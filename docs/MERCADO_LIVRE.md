...
**A recusa real (17/09/2026 13:36–13:37 UTC) — CONFIRMADO: o ML não aceita relist de item COM variações de vendedor no modelo de user products (D-369).** As duas retomadas da operação `a7638dc5` (13:36:37 e 13:37:03) mandaram o corpo oficial com variações para o MLB1476804187 — pai já fechado, 10 variações, cada uma com `user_product_id` próprio e nenhum na raiz — e as duas levaram **400** `validation_error`, causa `item.variations.relist.invalid`, mensagem *"Relist item with variations are not allowed for user product seller"* (o `failure_reason` gravado é o da segunda). O anúncio segue fechado, sem filho e sem caminho de relist. A doc oficial de relist e as páginas de User Products não mencionam essa regra (relidas em 17/09); a fonte é só a resposta. Se o 400 de 16/09 (corpo sem variações) tinha a mesma causa não se sabe: o corpo do erro não foi gravado.

- **Regra da casa desde D-369:** o preflight bloqueia `VARIACOES_USER_PRODUCT` quando o item tem variações e alguma traz `user_product_id` preenchido — no pedido (snapshot do multiget) e no re-preflight antes do PUT. O `GET /items/{id}` sem `include_attributes` traz o campo em cada variação (CONFIRMADO: leitura só-GET do MLB1476804187 em 17/09; o `parent_snapshot` da `a7638dc5`, capturado pelo multiget, também o tem). A recusa gravada com essa causa não é elegível para retomada, e a retomada que acha o pai ao vivo nessa forma termina sem transição e sem POST.
...
| **Item com variações em conta de user products** | ❌ recusado: 400 `item.variations.relist.invalid` (resposta real, 17/09/2026, D-369) — a doc não diz |
