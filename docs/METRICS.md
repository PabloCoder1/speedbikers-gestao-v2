...
| `esperado_meta` | Esperado até ontem | `meta × Σ peso dos dias completos ÷ Σ peso do mês`; peso = fator do dia da semana × efeito da data comercial (D-406, 1 fora delas); ritmo = `realizado até ontem − esperado` | O domingo fraco não conta como atraso. Abaixo do esperado até 5% é atenção; mais que isso, perigo |
...
| `projecao_fechamento_mes` | Projeção de fechamento | `realizado até ontem + ritmo × Σ peso dos dias restantes`; ritmo = `receita ÷ Σ peso` dos últimos 28 dias completos. Conservador e otimista: o menor e o maior ritmo entre 7, 14 e 28 dias | Data comercial (D-406): o efeito é o do mesmo dia na ocorrência do ano anterior, contra a média do mesmo dia da semana nas 4 semanas antes dela; sem essa ocorrência no histórico, a tela diz e a data não pesa. Hoje entra pela média do dia da semana, não pelo parcial. Sem 7 dias completos de histórico: NULL |
...
**O dia consolidado.** O Mercado Livre publica o gasto e os cliques do dia antes da venda atribuída (medido em 23/09: 21/09 e 22/09 chegaram com gasto em todas as campanhas e venda zero; 22/09 também com impressão zero; nenhum dia mais antigo tem o padrão). **Dia consolidado** (D-401) = gravado por um sync **posterior ao fim dele** (`metric_date` antes da data de `synced_at` em São Paulo), com venda atribuída **e** impressão, somando as contas do recorte. O dia em que o sync roda é retrato parcial — já tem venda e ainda não fechou; em 24/09 a regra de D-398, que não olhava a data do sync, tomou 23/09 por consolidado e pôs 21 e 22/09 sem venda na semana. **Dia pendente** = dia antes de hoje, com gasto, depois do último dia consolidado. `get_ads_overview` devolve `dias_pendentes`; com algum no período, a central não compara vendas com Ads, ROAS nem ACOS (5I) — o investimento, o TACoS e o lucro após Ads seguem, porque o gasto desses dias é conhecido — e o `/faturamento` avisa.
