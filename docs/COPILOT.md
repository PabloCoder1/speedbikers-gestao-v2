...
> Status: **estratégia aprovada. Ferramentas determinísticas + `ai_runs` implementados em 2026-08-25 (D-077)** — `POST /v1/copilot/query`, `apps/api/src/copilot.ts`, schemas em `@sb/contracts`. Modelo/orçamento decididos em 2026-08-25 (D-082) e a primeira ferramenta com LLM de verdade, `narrate_sku_diagnosis`, já implementada — planner por linguagem natural, streaming SSE de verdade e UI de chat implementados em 2026-08-28 (D-114, `POST /v1/copilot/chat` + `/copiloto`); sugestão de resposta e estruturação de sugestões em D-112; Base de Conhecimento Validada em D-113; explicação de AÇÃO da Central de Ações em D-155 (`narrate_action`, vocabulário obrigatório do PRD). O checklist de IA está coberto — a evolução apontada é memória multi-turno do chat, com decisão própria de custo.
...
**`narrate_action` implementada em 2026-08-31 (D-155)** — categoria "Diagnóstico", o último item da Fase 6B: a IA explica a **AÇÃO** da Central de Ações, não só o diagnóstico do SKU. Diferente de `narrate_sku_diagnosis`, o contrato não viaja no corpo — a ação **já vive no banco** (`actions`, D-064/D-116): o input é só `{ actionId }` e a `api` lê a linha sob a RLS do próprio usuário (autorização e dado no mesmo ato; não existe contrato forjável). O prompt é montado pela **mesma leitura defensiva que a tela renderiza** (`describeActionEvidence`, movida de `apps/web/lib` para `@sb/domain/diagnostics` quando a `api` virou o segundo consumidor — regra de contenção de `docs/ARCHITECTURE.md` secao 7). O system prompt (`ACTION_EXPLANATION_SYSTEM_PROMPT`) impõe o **vocabulário obrigatório** do PRD em cinco seções rotuladas: causa mais provável, fatores contribuintes, hipóteses, evidências contrárias e o que não conseguimos verificar — seção sem dado declara a ausência, e a expressão "causa verdadeira" é proibida por instrução explícita. Botão "Explicar com IA" em `apps/web/app/acoes/action-card.tsx` (clique explícito, nunca no carregamento — secao 9), `maxTokens: 768` porque cinco seções não cabem com folga nos 512 padrão.
...
> Registrado em 2026-08-24 (D-071), junto com a Central de Atendimento/SAC — `docs/PRODUCT_REQUIREMENTS.md`, `docs/ROADMAP.md` Fase 7B. Atualizado em 2026-08-27: a infraestrutura em volta já existe por completo — ingestão de Perguntas (D-086 a D-089), Caixa de Entrada e detalhe (D-090/D-095), triagem (D-094), **envio manual de resposta** (D-096, `POST /v1/support/cases/:caseId/reply` — o comando privilegiado do fluxo abaixo, exatamente como desenhado aqui) e Mensagens pós-venda (D-097). **Atualizado em 2026-08-28 (D-112): a ferramenta de geração (`suggest_support_reply`) está implementada** — a sugestão entra na caixa de resposta para revisão humana e o texto sugerido é auditado em `support_reply_attempts.suggested_text` junto do texto final. **O que continua não implementado é a Base de Conhecimento Validada** (e, com ela, a consulta de compatibilidade como evidência — hoje o prompt manda dizer "será confirmado" quando o contexto não sustenta).
