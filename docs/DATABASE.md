...
### `shipping_sales` — as vendas que o detector de frete considera (D-409)

**Implementado em 2026-09-25**, migration `20260923195958_vendas_com_frete_para_o_detector.sql`. Uma linha por pedido de um item só, quantidade 1, pago ou parcialmente reembolsado, fora do Flex, com o frete do vendedor capturado: `order_id` (chave, cascata de `orders`), conta, data, a linha (`line_id`), o anúncio (`listing_id`), SKU, título, preço, comissão e frete. **Mantida por gatilhos** em `orders` (status, logística, data), `order_items` (inserção, remoção e as colunas que a regra lê) e `order_financials` (o frete), que recalculam a linha do pedido por `private.sync_shipping_sale`; funções `security definer` sem `execute` para ninguém. Só leitura: RLS de conta na forma de conjunto (D-181), nem o `service_role` grava. O custo do produto NÃO mora aqui: é calculado na leitura, como em `get_faturamento`.
...
Tipos: `ENTRADA_NFE` · `SAIDA_NFE` · `VENDA_ML` · `CANCELAMENTO_ML` · `DEVOLUCAO_ML` · `AJUSTE_MANUAL` · `AJUSTE_RECONCILIACAO` · `TRANSFERENCIA` · `RESERVA` · `LIBERACAO_RESERVA` · `ENTRADA_TRANSITO` · `RECEBIMENTO_TRANSITO` · `ESTORNO_PRE_CAPTURA` · `ESTORNO_REVERSAO_EXCEDENTE`.

`ESTORNO_PRE_CAPTURA` (D-351) é o par de um `VENDA_ML` cuja "venda em" (`date_closed ?? date_created`) é anterior ou igual ao corte do snapshot do UpSeller: a planilha já tinha descontado a venda. O worker grava os dois juntos, com a mesma `occurred_at` e `created_by` nulo; o par soma zero no saldo e no alvo de `compute_erp_target_balances`. **A chave é neutra: `estorno:<chave do movimento estornado>`** — o tipo diz a causa, a chave diz o movimento, e o `UNIQUE` de `idempotency_key` absorve um segundo estorno do mesmo movimento venha de onde vier (worker, F3, e na fatia do Full `ESTORNO_FULL`); quem lê separa estorno de venda pelo TIPO. Rascunho novo é sempre estornado; a linha já gravada, se ainda não foi absorvida: com `occurred_at` depois do corte (o worker antigo), sempre, espelhada; até o corte, só com `created_at` depois do último alinhamento do saldo a esse corte, `max(imported_at, reconciled_at)` — quem absorve a venda gravada é a reconciliação, não o import. O corte do alvo (`captured_at`), o fim do import dele (`imported_at`, o maior entre `applied_at` do lote do snapshot vencedor e o `created_at` dele), a última reconciliação da organização (`reconciled_at`, por `job_runs` concluído de `maintenance.reconcile-balances` ou pelo último `AJUSTE_RECONCILIACAO`; nula para SKU sem snapshot próprio) e a exportação que a planilha retrata (`exported_at`: o próprio `captured_at`, exceto no snapshot que ainda carrega o parse de uma planilha com o nome carimbado, em que é a exportação lida do nome) vêm de `get_erp_stock_cutoffs` (só `service_role`, que tem EXECUTE em `private.erp_stock_export_instant`). **O que a planilha tem se decide pela exportação; de que lado do alvo está uma linha gravada, pelo corte** (D-351 §10): venda com "venda em" depois de `exported_at` não é estornada — nem o rascunho novo, nem a linha do worker antigo —, e o cancelamento compara o instante dele com `exported_at`. Pedido cancelado cuja venda é anterior ao corte, sem NENHUM `VENDA_ML` gravado e cujo cancelamento é posterior ao corte ganha `VENDA_ML` + `ESTORNO_PRE_CAPTURA` + `CANCELAMENTO_ML` quando a V3 viu a transição de venda para cancelado (status anterior ou `order.cancelled` com `before.status` de venda).

**Uma unidade vendida volta ao estoque no máximo uma vez** (D-351 §9). `CANCELAMENTO_ML` (`cancelamento:<chave da venda>`) e `DEVOLUCAO_ML` (`devolucao:<claim>:<chave da venda>`) revertem o mesmo `VENDA_ML`: cada reversão grava só o que as outras ainda não devolveram daquela linha, e nada quando já devolveram tudo. A devolução tem a origem do claim e o pedido só aparece dentro da chave, por isso o worker a lê por `get_order_return_movements(p_organization_id, p_order_ids)` (só `service_role`, índice de expressão parcial no quarto campo da chave). A venda estornada cujo pedido cancelou, com instante conhecido, até a exportação do corte do SKU não grava `CANCELAMENTO_ML` — a planilha já tem a unidade de volta —, e a devolução entregue depois a trata como já revertida: `claim-return` lê também o `ESTORNO_PRE_CAPTURA`, o status e os instantes do pedido e o corte (D-351 §11).

`ESTORNO_REVERSAO_EXCEDENTE` (D-351 §12) anula a reversão que o legado gravou além da venda (cancelamento E devolução da mesma venda) numa venda estornada: o `ESTORNO_PRE_CAPTURA` é a venda inteira, e o excesso é atribuído às reversões mais recentes pelo `occurred_at` (empate pela chave em ordem de código), cada parte com a chave neutra `estorno:<chave da reversão>`, o `occurred_at` da reversão, o SKU, o local e a origem da venda e `created_by` nulo — cada par (reversão, anulação) fica do mesmo lado do corte do alvo. Tipo próprio porque quem lê `ESTORNO_PRE_CAPTURA` o entende como "venda estornada". Por isso `get_order_return_movements` devolve também o `occurred_at`.
...
**`p_sku_id` opcional desde 2026-08-25 (D-080, migration `20260825150000_add_sku_filter_to_stock_coverage.sql`)**: nulo continua varrendo todos os SKUs da organização (uso de `/cobertura`, inalterado); preenchido filtra para UM SKU só — o Simulador de decisão do Dashboard de SKU (item 10, `docs/PRODUCT_REQUIREMENTS.md` secao "Simulador de decisão") usa esse filtro para pré-preencher a venda média diária real como premissa inicial, ajustável pelo usuário. Mesma pegadinha de assinatura já resolvida em D-078: `drop function` explícito antes de recriar, não `create or replace`.
...
**`get_sku_sales_breakdown(organization_id, sku_id, date_from, date_to)`** — implementada em 2026-09-03 (D-227), migration `20260903134854_sku_sales_breakdown_rpc.sql`, para a aba `Vendas` do Dashboard de SKU. `security invoker`, soma em SQL. Devolve os seis canônicos de `docs/METRICS.md` 5.2 no grão SKU em **três grãos numa chamada só** — `grain = total | conta | dia`, via `group by grouping sets`: uma varredura de `daily_sku_metrics` pelo índice `(organization_id, sku_id, metric_date)`, um round trip (D-185). As razões (`average_ticket`, `average_selling_price`) são calculadas sobre as SOMAS de cada grupo, nunca média de médias, e vêm NULL com denominador zero. A linha `total` existe SEMPRE (zeros, não ausente — mesmo padrão de `get_sku_dashboard`); dias sem venda não aparecem (mesmo contrato de `get_sales_daily_series`). Somar `orders_count`/`purchases_count` entre contas e dias é exato: pack pertence a uma conta (D-017/D-050) e, medido na fonte em 03/09/2026, **172.624 packs, zero em mais de um dia**. Medida no Dev no SKU mais vendido (74 linhas, 4 contas): 35 linhas, 193 buffers, 1,4 ms.

**`get_sales_sku_declines(date_from, date_to, previous_date_from, previous_date_to, account?, order_by?, limit?)`** — migration `20260921210000_create_sales_sku_declines_rpc.sql`, para perguntas do Copiloto como “qual produto caiu?”. Soma `daily_sku_metrics` por SKU para as duas janelas explícitas, retém somente queda real na métrica escolhida (`units` ou `revenue`) e ordena/limita ainda no SQL (1–50, padrão 10). Não chama venda sem histórico anterior de queda. É `security invoker`: a RLS por conta filtra as linhas antes de qualquer agregação, inclusive quando o usuário não alcança toda a organização.
...
**`sync_runs.resource`/`sync_errors.resource` alargados**: o CHECK constraint desde a Fase 2 só previa `orders`/`listings`/`fulfillment` — primeira vez nesta sessão que esse enum precisou crescer de verdade (`alter table ... drop/add constraint`) para caber `'visits'`. **`'questions'` entrou em 2026-08-25** (`20260825180000_add_questions_sync_resource.sql`, D-089): a reconciliação de Perguntas tem janela, contagem e frescor reais, ao contrário do fetch por ID do webhook (D-087 deixou o CHECK intacto de propósito). O vocabulário continua fechado — `SyncResource` em `apps/worker/src/handlers/sync-runs.ts` é o espelho em TypeScript e precisa andar junto.
...
**`captured_at` é o instante da EXPORTAÇÃO da planilha, não o do parse** (D-351): lido do nome `Lista_de_Estoque_MMDDHHMMSS` (UTC), limitado a `[parsed_at − 24 h, parsed_at]`, e `parsed_at` quando o nome não traz o padrão (`resolveStockExportInstant` no worker, `private.erp_stock_export_instant` na migration que corrigiu os snapshots antigos). É o corte de três leitores que precisam concordar: `compute_erp_target_balances` (soma `occurred_at > captured_at`), `get_erp_stock_cutoffs` (o lado do alvo de uma venda já gravada; o gate da venda usa `exported_at`, o mesmo instante exceto no snapshot que ainda carrega o parse) e a compensação F3. Os dois primeiros desempatam a planilha reimportada com o mesmo nome (o mesmo `captured_at`) por `created_at desc, id desc`: vence o lote gravado por último. A migration que corrigiu os snapshots antigos só tocou organização sem `AJUSTE_RECONCILIACAO`: a reconciliada (o Dev) ficou com o corte do parse, que a reconciliação já tinha usado para alinhar o saldo.
...
UMA tabela só, grão `(ml_account_id, item_id)` — mesma granularidade de `sku_listing_links`/`fulfillment_stock_snapshots` para o mesmo conceito, evitando o split que a V2 tinha e nunca populou. **Projeção MUTÁVEL (upsert), não ledger** — a linha em si não guarda histórico. `sku_id` é `on delete set null`, não `restrict` — diferente de `order_items`/`stock_movements` (histórico imutável), esta linha é projeção viva, refeita a cada sync.

**Motor de diff — implementado em 2026-08-24 (D-072), pré-requisito crítico da Fase 7 (`docs/HANDOFF.md`):** `ml-listings-fetch.ts` lê a linha ANTERIOR antes do upsert sobrescrevê-la e chama `detectListingEvents` (`packages/domain/src/events/listing-events.ts`) — preço, título, status (só `paused`/`reactivated`, as duas únicas transições catalogadas) e quantidade disponível geram `domain_events`, best-effort via `recordDomainEvents`. Valores de `status` confirmados ao vivo (`docs/MERCADO_LIVRE.md` secao 2.13): `available_quantity` chegar a 0 pausa o anúncio sozinho no Mercado Livre, então os dois eventos podem disparar juntos para a mesma causa raiz — não é duplicidade, cada um responde uma pergunta diferente. `domain_events` ainda não tem consumidor (nem notificação, nem Central de Ações) — a emissão existe, falta a camada de cima.
...
subject_key, last_detected_on          -- D-403, só nos alertas da central
```
...
**Alertas da central (D-403, migration `20260923195930`).** Três `kind` a mais -- `frete_anomalo`, `ads_campanha`, `produto_prejuizo` -- gravados por `sincronizar_alertas_central(organização, hoje, fontes)` (`security invoker`, execute só para `service_role`), chamada pelo job `diagnostics.sync-central-alerts` no mesmo gatilho diário -- uma fonte por chamada desde D-404, para cada uma caber no `statement_timeout` de 8 s. Não há detector novo: a função lê `get_detector_frete`, `get_sinais_ads` e `get_ranking_produtos` (que ganhou `p_organization_id` para ser chamado sem RLS). Um episódio por assunto (`subject_key`: anúncio:faixa, conta:campanha, sku_id): o aberto é atualizado enquanto detectado; o fechado por uma pessoa não reabre enquanto a condição continua (só `last_detected_on` anda); 3 dias sem detecção numa fonte que rodou encerram o aberto (`resolvido` + `evidence.encerramento`); a volta depois disso é linha nova (`dedup_key` = tipo:assunto:dia em que nasceu). O índice parcial `actions_um_episodio_aberto` (`organization_id, kind, subject_key` com status aberto) garante um só aberto por assunto; `actions_assunto_idx` acha o episódio. Os tipos anteriores têm `subject_key` NULL e seguem como eram.
...
**Lidas também por `/skus/[skuId]?aba=decisoes` (D-228)**, direto sob RLS, sem RPC — é uma lista, não uma agregação: `action_decisions` com o embed `actions!inner` filtrado por `actions.sku_id` (o SKU vive na ação) e o embed reverso `action_outcomes`; a única contagem da aba (ações abertas do SKU) é `count` com `head: true`. O embed sai **sem cast** porque as três policies de SELECT são a mesma expressão — `organization_id in (select private.accessible_orgs())`, a forma de conjunto de D-181 — e `organization_id` é copiado da ação para a decisão pela própria `create_action_decision` e para o outcome pelo job. A paridade está fixada por teste em `rls-embed-nullability.integration.test.ts` (igualdade entre as três expressões, não o texto de uma delas).
...
**Achado ao conferir o GRANT desta tabela**: `has_table_privilege('authenticated', 'public.saved_filters', 'INSERT')` devolveu `true` mesmo sem nenhum `grant insert` explícito — privilégios padrão deste projeto Supabase concedem INSERT/UPDATE/DELETE a `authenticated` em tabela nova. Conferido também em `stock_movements` (tabela já existente com o mesmo padrão "só RPC escreve"): o mesmo é verdade lá. Os dados continuam seguros porque a RLS não tem policy de escrita para `authenticated` em nenhuma das duas (RLS nega por padrão sem policy correspondente ao comando), mas o GRANT em si nunca tinha sido apertado para `authenticated`, só para `anon`. `saved_filters` já nasce com `revoke all on ... from anon, authenticated` antes do `grant select` — a auditoria separada sinalizada aqui aconteceu em duas rodadas: D-066 (23 tabelas anteriores, migration `20260824132723`) e D-098 (4 tabelas que REINTRODUZIRAM o padrão dois dias depois de D-066, migration `20260827130000`) — e desde D-098 existe um teste-guarda permanente em `packages/db/src/rls.integration.test.ts` que falha na CI para qualquer tabela futura com escrita concedida a `authenticated` sem policy correspondente.
...
**Bug real corrigido em 2026-08-25 (D-081, migration `20260825160000_fix_sku_sales_baseline_multi_account_grain.sql`)**: `daily_sku_metrics` tem grão POR CONTA (`unique nulls not distinct (ml_account_id, sku_id, metric_date)`) — um SKU vendido em duas contas Mercado Livre no mesmo dia gera duas linhas na tabela. As CTEs `weekday_history`/`current_day` desta função liam essas linhas sem somar entre contas primeiro, e o join contra `baseline` (já agrupado por `sku_id`) multiplicava a saída — uma linha por conta em vez de uma por SKU. Achado ao confirmar o ciclo diário do job `diagnostics.detect-sales-anomalies` contra produção real (Checkpoint pré-Fase 7): o upsert de `actions` quebrou com "ON CONFLICT DO UPDATE command cannot affect row a second time" porque duas linhas da mesma função geravam o mesmo `dedup_key`. Corrigido somando por `(sku_id, metric_date)` ANTES de qualquer janela ou join — mesmo raciocínio (SKU é organizacional, D-006) já usado em toda outra leitura de `daily_sku_metrics` do projeto (`get_stock_coverage`, `get_sku_dashboard`, `get_sku_abc_curve` — conferidas nesta sessão, todas já somavam corretamente; esta função era a única exceção). Efeito colateral silencioso do mesmo bug, também corrigido: a janela de "últimas 8 ocorrências" do baseline podia contar duas linhas do mesmo dia (contas diferentes) como duas ocorrências distintas, inflando a amostra e distorcendo média/desvio padrão sem nenhum erro visível.
...
**`notification_preferences`** — `event_type`/`ml_account_id` nulos são curingas ("aplica a todos"); quando mais de uma linha bate, a mais específica vence. Mesma pegadinha já documentada para `sku_listing_links` (secao 4, acima): `NULL` não colide em `UNIQUE` simples, por isso os quatro índices únicos parciais (um por combinação de curinga) em vez de um `unique (user_id, event_type, ml_account_id)` ingênuo. UI implementada em D-076 (`apps/web/app/notificacoes/preferencias`) — ver correção abaixo sobre ONDE a preferência é aplicada.

**Correção 2026-08-24 (D-076) — `notification_preferences` NÃO filtra mais `notification_recipients`**: a trigger original de D-073 filtrava a inserção de `notification_recipients` pela preferência (`enabled`/`min_severity`), suprimindo TANTO a Central de Notificações quanto o alerta em tempo real pra quem desativou um `event_type` — contradizia `docs/NOTIFICATIONS.md` secao 1 ("o registro na Central continua existindo para consulta, só o alerta em tempo real é que respeita a preferência"). Achado ao construir a UI de preferências (item 6): até então a tabela nascia vazia, então o bug era inerte. Migration `20260824210000_fix_notification_preferences_scope.sql` corrige a trigger — `notification_recipients` volta a ser inserido incondicionalmente pra todo membro elegível por permissão de conta, igual antes de D-073 ter adicionado o filtro de preferência. `notification_preferences` passa a ser consultada só pelo CLIENTE (`apps/web/lib/notification-preferences.ts`, `shouldNotify`), na hora de decidir se mostra o toast — nunca mais na criação do registro durável. **Sem nenhuma preferência, tudo notifica por padrão** — continua valendo, só que agora esse "notifica" já era verdade pra Central desde sempre, preferência nenhuma muda isso.
...
**Verificação**: `packages/db/src/rls.integration.test.ts`, describe `"notificações (fan-out de domain_events, D-073)"` — regra de destinatário (organizacional, conta com/sem permissão), RLS de leitura/escrita, autogestão de preferência. O describe `"notification_preferences NÃO suprime o fan-out"` (renomeado em D-076) agora prova o oposto do nome original: criar uma preferência restritiva não impede o `notification_recipients`. Não executável nesta máquina (sem Docker) — verificado pela CI (`integration` job).
