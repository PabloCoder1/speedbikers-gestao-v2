...
**`notification_preferences`** — `event_type`/`ml_account_id` nulos são curingas ("aplica a todos"); quando mais de uma linha bate, a mais específica vence. Regra só por evento e regra só por conta têm a mesma especificidade; nesse empate o cliente escolhe a mais restritiva (desativada, depois maior severidade), de forma independente da ordem de criação. Mesma pegadinha já documentada para `sku_listing_links` (secao 4, acima): `NULL` não colide em `UNIQUE` simples, por isso os quatro índices únicos parciais (um por combinação de curinga) em vez de um `unique (user_id, event_type, ml_account_id)` ingênuo. UI implementada em D-076 (`apps/web/app/notificacoes/preferencias`) — ver correção abaixo sobre ONDE a preferência é aplicada.
