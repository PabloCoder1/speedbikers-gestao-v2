...
`ESTORNO_PRE_CAPTURA` (D-351) é o par de um `VENDA_ML` cuja "venda em" (`date_closed ?? date_created`) é anterior ou igual ao corte do snapshot do UpSeller: a planilha já tinha descontado a venda. O worker grava os dois juntos, com a mesma `occurred_at` e `created_by` nulo; o par soma zero no saldo e no alvo de `compute_erp_target_balances`. **A chave é neutra: `estorno:<chave do movimento estornado>`** — o tipo diz a causa, a chave diz o movimento, e o `UNIQUE` de `idempotency_key` absorve um segundo estorno do mesmo movimento venha de onde vier (worker, F3, e na fatia do Full `ESTORNO_FULL`); quem lê separa estorno de venda pelo TIPO. Só é estornada venda que entra no saldo DEPOIS de o corte chegar: rascunho novo sempre, linha já gravada só com `created_at > imported_at` — a venda gravada antes da planilha já foi absorvida pelo salto do alvo. O corte (`captured_at`) e quando ele chegou (`imported_at`, o menor `created_at` dos snapshots desse corte) vêm de `get_erp_stock_cutoffs` (só `service_role`). Pedido cancelado cuja venda é anterior ao corte, que a V3 nunca gravou e cujo cancelamento é posterior ao corte ganha `VENDA_ML` + `ESTORNO_PRE_CAPTURA` + `CANCELAMENTO_ML` quando a V3 viu a transição de venda para cancelado (status anterior ou `order.cancelled` com `before.status` de venda).
