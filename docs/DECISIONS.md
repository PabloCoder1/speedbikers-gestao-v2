...
**Contexto:** item P1 registrado ("Evoluir o Dashboard de SKU para abas/progressive disclosure"), puxado pela ordem da trilha 5E depois do Dashboard 360º do Anuncio (D-168). A pagina era uma vertical unica que disparava CINCO consultas sempre, mostrasse o usuario interesse nelas ou nao.

**Decisao 1 — cinco abas agora, e o item P1 continua ABERTO:** `Visao geral | Estoque | Anuncios | Historico | Diagnostico`. O item lista onze abas e manda "migrar incrementalmente"; marcar `[x]` com seis faltando seria a mesma afirmacao falsa que D-105 mandou eliminar da Home. Entao o item fica aberto, com o que falta escrito nele.

**Correcao de rota durante a fatia (achado da revisao adversarial, ver Decisao 5):** a primeira versao deste registro dizia que as seis abas restantes "nao tem read model por SKU". **Isso era falso e foi corrigido no codigo e no ROADMAP**: `Full` tem `fulfillment_stock_snapshots` por SKU+conta (a Visao geral ja mostra o total), `Decisoes` alcanca o SKU por `actions.sku_id`, `Precos` tem `listing.price.changed`, `Trafego` tem `daily_listing_visits`. O que falta e a CONSULTA agregada por SKU — somar isso em JS dentro da tela violaria a regra de agregacao em SQL. So `Atendimento` esta mesmo sem caminho: `support_cases` nao tem vinculo de SKU. A distincao importa para quem for puxar o item: nao e "falta dado", e "falta a consulta".

**Decisao 2 — progressive disclosure de verdade, nao so visual:** cada aba dispara SOMENTE as consultas que usa (`needsDashboard`/`needsCoverage`/`needsListings`/`needsHistory`; o resto vira `Promise.resolve`). A pagina saiu de 5 consultas fixas para 1 a 3 conforme a aba. Esconder com CSS e continuar consultando tudo seria trocar o custo de lugar, nao reduzi-lo.

**Decisao 3 — a aba vive na URL (`?aba=`), com conjunto fechado:** mesma mecanica dos filtros de Movimentacoes (D-167) — valor desconhecido cai para a Visao geral ANTES de tocar o banco, e cada aba e um link compartilhavel. O `<nav>` das abas tem `aria-label="Abas do SKU"` porque "Anuncios"/"Estoque"/"Diagnostico" tambem sao itens do menu lateral: sem o nome, nem leitor de tela nem Playwright distinguem os dois navs.

**Decisao 4 — ENSAIO VISUAL, encerrando a ressalva repetida:** todas as fatias anteriores fecharam com "⚠️ tela nao vista renderizada". Esta nao. Subi o web local contra o Supabase da maquina, entrei com o usuario do seed e olhei as CINCO abas, mais o Dashboard de Anuncio de D-168, que tambem nunca havia sido visto. O ensaio provou coisas que teste nenhum tinha provado: a timeline do SKU realmente puxa eventos do anuncio VINCULADO (correlacao de D-153 viva na tela), `?aba=inexistente` cai na Visao geral, e `eventTypeLabel` degrada para o valor cru num tipo desconhecido, exatamente como projetado.

**Achado do ensaio (registrado, nao corrigido aqui):** o Dashboard de Anuncio exibe **Visitas** e **Conversao** sem id canonico de metrica, porque `visitas` e `taxa_conversao` NAO estao em `metric_definitions` — `docs/METRICS.md` ainda as lista na secao 5.5 ("dependentes de fonte ainda nao confirmada"), texto que envelheceu: a fonte existe desde D-032, `daily_listing_visits` esta em producao e D-156 corrigiu a coleta. Sao dois numeros de tela sem definicao canonica, contra a regra de D-023. Vira fatia propria (catalogo + METRICS.md + as telas que os exibem), nao remendo no fim desta.

**Licao de ambiente (local, custou uma execucao):** rodar a suite de integracao contra o Postgres local QUEBRA o seed do Playwright depois — a suite cria usuarios por SQL direto e o GoTrue morre com `converting NULL to string is unsupported` ao listar usuarios com `confirmation_token` nulo. Normalizar as colunas de token (ou `supabase db reset`) antes do e2e local. E a suite Playwright nao e re-executavel sem reset: dois specs ESCREVEM (vinculam item de NF-e, assumem atendimento) e o seed nao desfaz nem um nem outro — a segunda rodada acusa "Vinculados: 1" onde espera 0. Na CI nada disso aparece porque cada job sobe o seu proprio Supabase. Sequencia local que funciona: `supabase db reset` -> `e2e:seed` -> `playwright test`.

**Decisao 5 — a fatia passou por revisao adversarial multiagente, e ela pegou coisa real:** quatro revisores independentes (regressao, testes, Next.js, disciplina) e um cetico por achado, instruido a REFUTAR. Dos 14 achados, 12 morreram na refutacao — mas dois sobreviveram ao meu proprio reexame depois, e os dois eram meus: (a) o comentario de `lib/action-shortcuts.ts` prometia que `/skus/{id}` era "o hub: diagnostico, simulador, custo e a linha do tempo", coisa que o refactor tornou FALSA — tres lentes acharam isso de forma independente; (b) a justificativa das abas ausentes era factualmente errada, como esta na correcao acima. O atalho ganhou destino preciso em vez de comentario remendado: `venda_anomala` agora aponta tambem para `?aba=diagnostico` — a regra de D-154 ("so se aponta para tela que EXISTE, com o filtro que ela realmente tem") passou a permitir isso justamente porque a aba virou URL. **Licao:** um verificador instruido a "na duvida, refute" devolve zero confirmados com facilidade; a lista bruta de achados vale mais que o veredito agregado, e ler os refutados foi o que encontrou os dois defeitos.

**Verificacao:** `check` 29/29, build 8/8, **13/13 Playwright locais** (a suite inteira, nao so o spec tocado), e as cinco abas conferidas a olho. Sem migration — a fatia e so de UI. `.claude/launch.json` ganhou a entrada `web-build` ao lado da `web-dev` que ja existia (o ensaio precisa do build servido, nao do dev server).
...
**O terceiro terco do item ja estava feito, e so se soube olhando.** O angulo "dados carregados por aba nao aberta" nao produziu achado confirmado porque `/skus/[skuId]`, a unica tela com abas de verdade, **ja** dispara por aba: `needsDashboard`/`needsCoverage`/`needsListings`/`needsHistory` trocam por `Promise.resolve({data: null, error: null})` tudo que a aba ativa nao usa. Os outros dois `<details>` do app nao tem leitura propria. Fica um caso de fronteira, registrado e deliberadamente NAO corrigido: `/vinculacoes:99` le 10 vinculos manuais para um `<details>` que nasce fechado — mas a leitura ja vive dentro do `Promise.all` da tela, entao nao custa ida nenhuma, so payload. **Um item do roadmap pode estar concluido sem nunca ter sido marcado**; a varredura serviu para saber, nao para consertar.
...
**Contexto:** D12 da frente visual (`docs/DESIGN_IMPLEMENTATION.md`) migrou `/anuncios` para o frame `Listings` do Figma. Como em D-241, a migracao foi varredura, e o que ela achou nao e de design — e de contrato de dados, de seed e de seguranca. Fica aqui, para quem procurar "por que `get_listings_dashboard` tem `p_stock`" ou "por que o seed cria anuncios".

**1. O frame pede seis contagens; o sistema mede quatro, e a tela mostra cinco.** Ativos, pausados, sem estoque e sem vinculo sao medidos (`listings.status`, `listings.available_quantity`, `link_state` da RPC). **"No Full" nao e fato de anuncio**: `listings` nao tem coluna de logistica; o Full vive em `fulfillment_stock_snapshots`, com grao de SKU. **"Com queda" nao existe**: nao ha deteccao de anomalia por anuncio nem entrada em `metric_definitions` (D-023). As duas ficaram de fora. A quinta celula e o **total**, lido — nao somado: existe `under_review`, e somar ativos + pausados + fechados seria inventar a aritmetica de um conjunto que a tela nao enumera.

**2. Contagem e lista sao a MESMA consulta.** Cada celula le o `total_count` de `get_listings_dashboard` com `p_limit => 1` e o predicado do seu estado; o link "ver lista" aplica o mesmo predicado a lista. Seis leituras num `Promise.all` (D-185: o custo e a ida e volta, e em paralelo e uma so). A alternativa — um `count` proprio ou uma RPC de resumo a parte — seria um segundo dono do mesmo dado (D-224), e e assim que faixa e tabela comecam a discordar. **Custo conhecido:** cinco execucoes extras da CTE por carga; sobre 5.085 anuncios a funcao mede ~60 ms, entao ~60 ms de relogio e ~300 ms de CPU do banco. Se um dia pesar, a saida e uma RPC de resumo que REUTILIZE `base` — nunca uma segunda definicao.

**3. `p_stock` entrou na RPC, e entrou POR ULTIMO.** "Sem estoque" na faixa e "Com estoque" na barra sao o mesmo predicado sobre `available_quantity`, coluna NOT NULL — a particao e completa. A RPC nao o conhecia, e contar no navegador contaria a pagina de 50 e chamaria de total (a classe que D-138 corrigiu nesta mesma tela). A primeira versao pos `p_stock` antes de `p_limit`, "junto dos filtros": **quebrou a suite de integracao**, que chama a funcao posicionalmente. Argumento novo vai no FIM da assinatura; quem nao passa continua igual. Valor desconhecido cai em `'all'`, nunca em zero linhas — zero linhas seria indistinguivel de filtro legitimo sem resultado.

**4. O guard D-182 pegou uma regressao real.** DROP + CREATE recria a funcao, e o Postgres da EXECUTE a PUBLIC em toda funcao nova. A primeira migration tinha o `grant` e nao tinha o `revoke ... from public, anon`: `anon` alcancaria a RPC. O teste "nenhuma funcao de public e alcancavel por anon" ficou vermelho antes de qualquer humano olhar. E o `grant` recriado inclui `service_role`, porque o DROP leva os grants antigos junto — perder o do worker seria silencioso.

**5. A tela nunca teve e2e, e o motivo era invisivel.** O seed nao criava anuncio nenhum; os nove do banco local eram residuo de `rls.integration.test.ts` (inclusive com `status = 'ACTIVE'` maiusculo, que o sync real nunca grava). Depois de um `db reset`, `/anuncios` ficava vazia — nao havia o que afirmar. O seed passou a criar quatro (`E2E_LISTINGS`): um por estado da faixa, mais um de vinculo POR VARIACAO com `sku_id` nulo, para que "sem vinculo" prove D-122 em vez de contar `sku_id is null`. So o primeiro recebe visita e venda: o segundo fica sem trafego de proposito, e e ele que prova na tela que conversao sem denominador e "—", nao 0% (D-123). Tres specs derivam as contagens do fixture. O spec do SKU, que afirmava o painel "Anuncios vinculados" vazio, passou a afirmar o VINCULO — o anuncio com `sku_id` aparece e os outros tres nao.
...
**Contexto:** A2 da frente visual — o acabamento das superficies migradas, a fatia que A1 deixou marcada. Cinco itens de componente, tres deles com achado tecnico que vale registro.

**1. `p_sku_id` em `get_sku_abc_curve` filtra DEPOIS do ranking, e essa e a unica posicao correta.** O frame do SKU mostra tres selos: estado, **classe ABC** e marca. A classe e canonica em SQL desde D-140, mas so era alcancavel pela tela `/curva-abc`, que pede a curva inteira. Filtrar antes do ranking deixaria o SKU sozinho respondendo por 100% do acumulado e ele sairia sempre "A" — o filtro entra em `filtrada`, ao lado de `p_only_without_full`, que ja ocupa esse lugar pelo mesmo motivo. **Provado com tres SKUs numa transacao revertida:** na curva inteira `ABC-DOM` e A, `ABC-MEIO` e B e `ABC-CAUDA` e C; pedindo so a cauda, ela volta **C com acumulado 100,00** — identico. Se o filtro estivesse antes, voltaria A.

**Consequencia declarada:** com `p_sku_id`, `total_count` e os tres `class_*_count` passam a contar o conjunto FILTRADO (1 e 0/0/0), porque sao janelas sobre `filtrada`. Quem chama por SKU le `abc_class`; quem quer o retrato da curva nao passa o argumento. E o mesmo comportamento que `p_only_without_full` ja tinha.

**Custo declarado, e por que ele foi aceito.** Esta e a leitura mais cara do Dashboard de SKU: a curva ordena 90 dias de `daily_sku_metrics` da organizacao inteira antes de devolver uma linha (D-166 mediu a familia em ~196 ms / 15 mil buffers no Dev). Ela entra no MESMO `Promise.all`, entao custa uma ida (D-185) e nao soma ao relogio das outras — mas passa a definir o piso da pagina. Vale porque o selo e dado canonico que o frame pede; **se a medicao do Dev mostrar que pesa, a saida e uma coluna materializada de classe, nunca uma segunda formula de ABC.** SKU sem venda no periodo nao ganha selo: fora da curva nao e "classe C", e afirmar C seria inventar posicao.

**2. A faixa de KPIs deixava uma fatia cinza, e o defeito era do proprio truque.** Os fios entre celulas vinham de `gap: 1px` sobre um fundo de borda — elegante, e correto so enquanto a grade esta CHEIA. A1 trocou o `auto-fit` por colunas fixas com degraus (5 → 3 em 1150px, 2 em 850px); com cinco celulas em tres colunas sobra um slot, e o slot mostrava o fundo de borda como um bloco cinza. **Visto na tela, a 961px, nao no codigo.** Agora os fios sao `box-shadow` da propria celula (nao ocupa espaco, e o `overflow: hidden` do cartao apara os das bordas) e a ultima celula recebe `grid-column: auto / -1`, fechando a linha em qualquer combinacao de contagem e breakpoint.

**3. `--sb-space-6` nao existe, e a declaracao inteira caiu.** A classe `.sb-empty`, criada em A1 para o estado vazio dos paineis, pedia `padding: var(--sb-space-6) var(--sb-space-3)`. A escala vai ate `--sb-space-5`. **Variavel indefinida nao cai em fallback: invalida a declaracao**, entao todo estado vazio ficou com `padding: 0px` — medido no app antes de corrigir, em duas telas. A licao e a de D-241 outra vez: build verde, tela errada. Passou a usar `--sb-space-5`.

**4. O `window.prompt` dos filtros salvos nao dizia O QUE estava sendo salvo.** O modal (`.sb-modal`, a forma do export) mostra o recorte antes de nomear: "Serao salvos 2 parametros da URL: days=7 · account=e2e-loja". E a mesma doutrina do painel de conferencia da curadoria (D-127) em escala menor — confirmar sem ver a consequencia nao e confirmar. As visoes salvas sairam das pilulas de raio 999px (a ultima forma de pilula solta no app) para um `.sb-menu`, como os demais filtros da barra.

**5. A paleta de comando agrupa por tipo porque a RPC devolve por tipo.** `search_entities` limita 5 por tipo; uma lista plana de 25 linhas repete "SKU" cinco vezes e depois "ANUNCIO" cinco vezes. O `.command` do frame tem rotulo de grupo, e ele diz o tipo uma vez. O placeholder do gatilho passou a nomear o que a RPC REALMENTE busca (SKU, anuncio, conta, fornecedor, pedido de compra) — o texto do frame prometia "acao", que nao esta na uniao.

## D-248 - Dashboard do Anuncio em oito abas: a fila pedia uma tela que nao existe, e a republicacao nao e tela

**Contexto:** a fila da frente visual trazia "D13 — Republicacao (`/republicacao`)". **Essa rota nao existe.** A republicacao vive dentro de `/anuncios/[itemId]`, o Dashboard do Anuncio (D-168) — uma pagina de 366 linhas em secoes verticais, sem abas, com `<h1>/<h2>` e estilos inline. A entrada da fila tinha sido escrita sobre uma premissa errada; a fatia virou o que ela realmente e: **migrar o Dashboard do Anuncio**, com a republicacao como historia dele.

**1. Pagina, nao drawer — e a justificativa vem do proprio frame.** O Figma desenha o anuncio como `MlbDetailDrawer` (600px, a direita), e o motivo de ser drawer e do prototipo: la o anuncio nao tem URL, o painel nasce de `useState`. Dentro dele, o frame monta um bloco comentado no proprio codigo como `{/* Operational Object Header */}` — o MESMO comentario literal do `SkuDetailScreen` — e uma fileira de abas com a classe byte-a-byte identica a da tela de SKU. O frame ja e uma tela de detalhe que perdeu a rota. A V3 tem a rota, e `/anuncios`, o Dashboard de SKU e (agora) as notificacoes linkam para ela. Trocar por drawer removeria um destino compartilhavel.

**2. As oito abas do frame sao exatamente as oito que o dono escreveu:** `VISAO GERAL, VENDAS, TRAFEGO, PRECO, FULL, HISTORICO, DIAGNOSTICO, DECISOES`. Todas foram implementadas com dado real e **sem migration**: `get_listing_dashboard_summary` (visao geral, vendas, trafego), `daily_listing_metrics` e `daily_listing_visits` por dia sob RLS, `domain_events` filtrado por `listing.price.changed` (preco), `get_listings_dashboard` com `p_search = item_id` (Full do anuncio), `domain_events` + `listing_relists` (historico), `action_decisions` com embed `actions!inner` filtrado por `mlb_id` (decisoes). Progressive disclosure real: cada aba dispara so as suas consultas.

**3. O Full do detalhe estava no grao ERRADO, e a lista ja estava no certo.** A tela chamava `get_fulfillment_overview(p_sku_id)`, que agrupa por (conta, SKU) — soma os buckets de TODOS os anuncios que compartilham o SKU. A lista `/anuncios` mostra o Full POR ANUNCIO desde D-243. Dois numeros sob o mesmo rotulo, em duas telas que se linkam: e assim que faixa e tabela comecam a discordar (D-224). O detalhe passou a ler a MESMA funcao da lista; o quadro por SKU+conta continua na aba Full, com o rotulo dizendo que e outro grao.

**4. Diagnostico RECUSA por anuncio, e a recusa e a aba.** O diagnostico de venda anomala (D-078) compara a venda de ontem com o mesmo dia da semana sobre `get_sku_sales_baseline` — baseline de SKU. Nao existe baseline por anuncio. Rodar a mesma formula sobre `daily_listing_metrics` daria um numero com a mesma cara e outra definicao: a classe que D-023 proibe. A aba explica isso e leva ao diagnostico do SKU que o anuncio vende.

**5. Republicar nao sai da tela, e o frame empurra para o contrario.** O `MlbDetailDrawer` tem um botao "Republicar anuncio" e o `RepublicationModal` e um assistente de cinco passos cujo passo 2 executa. O motor real existe (`listing_relists`, nove estados, indice unico parcial de uma operacao viva por pai), e **quem escreve e so o worker**; `complete_listing_relist_remap` nem sequer e executavel por `authenticated`. Os dois endpoints de escrita da API exigem ADMIN/GESTOR e **nao tem nenhum chamador na UI**. A primeira republicacao real continua pendente como ato humano deliberado em `docs/HANDOFF.md`. A tela LE o historico (pai e filho, estado, motivo da falha) e nao oferece disparo; o e2e afirma a ausencia do botao, para que reintroduzi-lo fique vermelho.

**Nota de leitura:** relist NAO escreve em `domain_events`, entao a linha do tempo nunca mostraria uma republicacao — por isso o historico dela e painel proprio, e nao uma linha da timeline.

**6. Tres defeitos de honestidade que a migracao encontrou.** (a) Unidades, receita e pedidos exibiam o `coalesce(...,0)` da RPC como zero cru, sem ressalva — agora o zero diz "nenhum dia com venda registrada no periodo", que e o que ele significa num rollup que so materializa dia com movimento. (b) O texto de "sem Full" declarava DUAS causas e negava a terceira: a leitura canonica so enxerga captura dos ultimos 3 dias (D-173), entao "sem linha" tambem pode ser saldo antigo nao recapturado. (c) A coluna "Onde" da linha do tempo era constante — a consulta fixa `entity_type='listing'`, entao ela imprimia "Anuncio" em toda linha. (d) A nota do cartao de Conversao dizia "visitas dos N dias observados" e imprimia **"dos 1 dias observados"** na tela — a mesma classe de defeito que `summarizePagedWindow` tinha com "1 anuncios". Aqui o conserto certo nao foi flexionar: o numero de dias ja esta no cartao ao lado ("observadas em 1 de 30 dias"), entao a nota perdeu o numero e virou "pedidos / visitas dos dias com coleta". **So a pagina RENDERIZADA mostrou isso** — typecheck, lint e cinco testes e2e passaram por cima da frase sem ver nada.
