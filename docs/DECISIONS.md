...
**Verificação:** typecheck/lint/test/build (monorepo completo) verdes — 4 testes novos (ZodError not_retryable; order sem `date_last_updated` aceita pelo schema E persistida; fallback em cascata até `date_created`; sub-recurso de claim processa o pai). Deploy do worker pendente no momento da escrita (autorização durável de deploy concedida pelo usuário em 2026-08-27 — "avisar antes e executar" — registrada no HANDOFF); até lá os retries dos 3 pedidos continuam falhando em produção, SEM perda de dado: a reconciliação horária já os pegou pelo search.
