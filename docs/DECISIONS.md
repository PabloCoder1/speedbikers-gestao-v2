...
**Verificação:** typecheck/lint/test/build (monorepo completo) verdes; a prova real (o teste-guarda contra Postgres zerado + migrations aplicadas) é o job `integração` da CI — sem Docker ativo nesta máquina no momento da escrita, mesma dependência de CI já usada por D-073/D-078/D-081.
...
**Verificação:** typecheck/lint/test/build (monorepo completo) verdes — 7 testes novos de domínio (`evaluateAiBudget`: teto exato não avisa, dedup por mês/organização, teto inválido lança), 7 do handler (custo como string do PostgREST, soma nula = zero, evento organizacional sem `ml_account_id`, dedup mensal na chave), 5 do gatilho da api, 3 do envSchema. Migration pendente de CI (job `integração` + `aplicar migrations`), `packages/db/src/types.ts` ainda sem a RPC nova (cast `as never` com nota, mesmo padrão temporário de D-077 — regenerar após a migration aplicada). **Deploy de `api` e `worker` + criação do job no Scheduler pendentes de autorização do usuário** — até lá o mecanismo existe só no repositório.
