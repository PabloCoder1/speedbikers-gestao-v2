...
**Pendência registrada — `packages/db/src/types.ts` não regenerado:** `supabase gen types typescript --local` exige Supabase local (Docker), indisponível aqui. As três tabelas novas não têm tipo gerado ainda — sem efeito nesta etapa (nenhum código de aplicação as consulta), mas bloqueia o item 4 (Central de Notificações) até ser regenerado, seja localmente com Docker ou contra o Supabase Dev depois que a migration aplicar lá.
