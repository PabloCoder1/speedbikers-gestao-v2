...
**Verificação:** `pnpm run typecheck`, `lint`, `test` (todos os pacotes, incluindo os 12 testes novos de `event-format.test.ts`) e `pnpm run build` (`apps/web`) — todos verdes. **Limitação que permanece, mesma de D-074**: a conexão Realtime em si (handshake do WebSocket, autenticação do token de sessão, entrega ponta a ponta de um evento real) não foi exercitada no navegador — exige sessão autenticada contra o Supabase Dev, e esta sessão não tinha credencial de usuário real disponível. A leitura de RLS que autoriza cada evento é a mesma já coberta pelos testes de integração de D-073; o risco não coberto por este trabalho é estritamente a integração ponta a ponta do transporte Realtime, não a autorização.
