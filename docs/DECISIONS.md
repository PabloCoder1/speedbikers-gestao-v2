...
**Não deployado nesta etapa.** `apps/api` e `apps/worker` mudaram e o Cloud Run exige `infra/deploy-cloud-run.sh` manual, com confirmação explícita do usuário (`docs/DEPLOYMENT.md` secao 7). Até o deploy acontecer, nenhuma pergunta real é ingerida — o código está correto e testado, mas o caminho não existe em produção. **Documentação não comprova deploy** continua valendo nos dois sentidos.
