...
**Verificação:** `pnpm run typecheck`, `lint`, `test` e `pnpm run build` (monorepo completo) — todos verdes. Um teste de integração novo em `packages/db/src/rls.integration.test.ts` (describe `"get_sku_sales_baseline (Fase 6, Diagnóstico)"`, já existente): SKU vendido em duas contas no mesmo dia (uma ocorrência histórica dividida 6+4, o dia atual dividido 5+3) devolve exatamente UMA linha, `current_units_sold` somado (8), `sample_count` correto (4, não 5) — reproduz a condição exata que quebrou o upsert em produção. Não executável nesta máquina (sem Docker) — pendente confirmação da CI contra Postgres real (job `integração`) e da aplicação no Supabase Dev (job `aplicar migrations`); **plano após a CI confirmar**: disparar `v3-detect-sales-anomalies` manualmente via `gcloud scheduler jobs run` para confirmar contra o dado real de produção que causou o crash original, não só contra fixture.

**Impacto:** `supabase/migrations/20260825160000_fix_sku_sales_baseline_multi_account_grain.sql` (novo). `packages/db/src/rls.integration.test.ts` (+1 teste, fixtures de segunda conta). `docs/DATABASE.md`, `docs/HANDOFF.md` atualizados. Nenhuma mudança em `apps/api`/`apps/worker` — a correção é inteiramente na função SQL, aplicada pela migration, sem deploy de Cloud Run necessário. **Checkpoint pré-Fase 7 item 1 ("confirmar reconcile-balances no ciclo natural") continua ABERTO** — motivo NÃO relacionado a este bug: `triggerBalanceReconciliation` falhou hoje com "JWT issued at future" ao listar organizações (erro visível graças ao log de D-067, mas sem retry automático — `retryConfig.maxRetryDuration: "0s"` no Cloud Scheduler, mesmo padrão nos quatro jobs "organizations_not_listed"). Evidência de que é transitório, não sistêmico: a MESMA instância `api` fez outras duas chamadas ao Supabase segundos depois, ambas bem-sucedidas. Precisa do próximo ciclo natural (2026-08-26, 6h/SP) para confirmar que se autorresolveu antes de fechar o checkpoint — não corrigido especulativamente sem mais evidência de recorrência. **Item 2 do checkpoint ("sincronização de visitas") fechado**: as 3 contas ML completaram `sync_listing_visits_snapshot_done` com `items_failed: 0` hoje, apesar de 429 intermitente do Mercado Livre absorvido pelo retry existente — confirma cadência e dado real, sem achado novo (429 já documentado em D-070).
