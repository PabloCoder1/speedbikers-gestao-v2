...
|---|---|---|---|
| `/webhooks/mercado-livre` | POST | Validação própria da origem | ACK rápido e enfileira. **Não é "zero chamada de rede"** (D-339): toda notificação consulta `ml_accounts` antes do ACK, e as com trabalho ainda criam a Cloud Task. Responder o tópico sem consumidor antes da consulta foi publicado e **revertido** — esfriou as conexões do caminho com trabalho (D-340). Desde 2026-08-25 (D-088) o ACK roteia por tópico: `questions` vira `sync.support.questions` com `{ mlAccountId, questionId }`; todo o resto continua em `sync.webhook.received`. Uma regra de dedupe só (`ml-webhook:{resource}:{janela-minuto}`) para os dois. `questions` com `resource` fora de `/questions/{id}` responde 200 sem enfileirar e sai como `ml_webhook_unroutable_resource` no log |
