...
|---|---|---|---|
| `/webhooks/mercado-livre` | POST | Validação própria da origem | ACK rápido e enfileira. **Não é "zero chamada de rede"** (D-339): as notificações com trabalho criam a Cloud Task antes do ACK. A conta dona do `seller_id` vem de um diretório **em memória** desde D-346 — a consulta por notificação custava p95 de 2,8 s nas rajadas (D-345), e a primeira tentativa de evitá-la, só para o tópico sem consumidor, esfriou as conexões e foi revertida (D-340). Desde 2026-08-25 (D-088) o ACK roteia por tópico: `questions` vira `sync.support.questions` com `{ mlAccountId, questionId }`; todo o resto continua em `sync.webhook.received`. Uma regra de dedupe só (`ml-webhook:{resource}:{janela-minuto}`) para os dois. `questions` com `resource` fora de `/questions/{id}` responde 200 sem enfileirar e sai como `ml_webhook_unroutable_resource` no log |
...
| `/v1/pricing/ml-shipping-quote` | POST | qualquer papel com acesso à conta | **Implementado em 2026-09-16 (D-359)** — cotação do frete grátis que o vendedor paga, para a calculadora de preço do `/faturamento`: `GET /users/{seller}/shipping_options/free` com medidas (cm), peso (g), preço, `listing_type_id` e logística. **Síncrona e só leitura** (GET curto, 2 tentativas, corte em 8 s). O token é só LIDO — perto de vencer devolve 503 em vez de renovar, porque o refresh é de uso único e é do worker. 404 fora da organização ou sem permissão na conta; 503 `unavailable` com o motivo quando a conta não está conectada ou o ML recusa |
| `/v1/listings/relist` | POST | ADMIN/GESTOR | **Implementado em 2026-08-31 (D-161)** — pedido de republicação (Fase 9): valida papel + escopo por conta (lição D-117) + anúncio conhecido, e **enfileira** `relist.prepare` na fila da conta. O worker captura o `parent_snapshot` e roda o preflight (D-160); a operação nasce `REQUESTED` ou morre `PREFLIGHT_FAILED` com os motivos. **Nada destrutivo nesta rota.** Sem OPERADOR: republicar começa fechando um anúncio (irreversível), decisão de gestão |
| `/v1/listings/relist/:relistId/execute` | POST | ADMIN/GESTOR | **Implementado em 2026-08-31 (D-162)** — a CONFIRMAÇÃO humana da execução, segundo ato separado do pedido. Só operação `REQUESTED`; **enfileira** `relist.execute`, que re-roda o preflight NA HORA, fecha o pai, emite o POST /relist e confirma o filho (id ≠ pai) — re-entrante por estado, com POST nunca re-tentado (um 5xx pode significar que o filho nasceu; `RELIST_FAILED` exige gente). 409 `invalid_state` fora de REQUESTED |
...
| `/internal/backfill/order-financials` | Manual (disparo único) | Recuperação do frete anterior à varredura diária (D-396) — **implementado em 2026-09-23**: enfileira o primeiro pedaço de cada conta CONNECTED na fila `backfill`; corpo opcional `{ "dias": 90, "conta": "<slug>" }` (8 a 365 dias; `conta` ensaia numa conta só). Não é agendado |
| `/internal/schedule/order-logistics` | Cloud Scheduler | Varredura da logística do envio (D-352, R2/R3) — lê `GET /shipments/{id}` dos pedidos com `VENDA_ML` ainda sem sinal (venda sem estorno, ou estornada pela D-351) e, quando o envio é `fulfillment`, carimba a captura, anula as reversões abertas e grava o `ESTORNO_FULL` que falta, nessa ordem. A cada 6h (`40 */6`), por CONTA, dedupe por `order-logistics:{slug}:{hora-cheia-ISO}`; tetos de 800 leituras e 7 min de rede por rodada; 429/5xx/401 param a rodada (falha retryable). **Na branch, não publicado** (ordem em `docs/DEPLOYMENT.md` 8.3) |
| `/internal/schedule/ads` | Cloud Scheduler | Mercado Ads (D-363) — **implementado em 2026-09-16**, DIÁRIO às 11h (métricas do ML fecham às 10h), por CONTA, escalonado em 300 s |
...
| `sync.ads.campaigns` | `ml-sync-<conta>` | `ads:{conta}:{dia-de-negócio}` — **implementado em 2026-09-16** (D-363): anunciante (`GET /advertising/advertisers?product_id=PADS`; 404 = conta sem Product Ads, gravado como `nao_habilitado`), campanhas (`/advertising/MLB/advertisers/{id}/product_ads/campaigns/search`) e métricas diárias de 90 dias por campanha (`/advertising/MLB/product_ads/campaigns/{id}?aggregation_type=DAILY`), por upsert em `ads_advertisers`/`ads_campaigns`/`daily_ads_campaign_metrics`. `resource: "ads"`. Payload `{ mlAccountId }`. Gatilho: `POST /internal/schedule/ads` + Cloud Scheduler `v3-ads-campaigns-sync` |
...
| `backfill.order-financials` | `backfill` | `backfill-order-financials:{conta}:{início do pedaço ISO}` — **implementado em 2026-09-23** (D-396): um DIA de pedidos válidos por execução, o mesmo laço de captura de `sync.order-financials` (frete de `GET /shipments/{id}/costs`, desconto de `GET /orders/{id}/discounts`, 4xx = NULL, checkpoint por linha); ao terminar, enfileira o dia anterior até `limite`. Payload `{ mlAccountId, ate, limite }` (ISO). `resource: "order_financials"`, `channel: "backfill"` |
| `maintenance.reconcile-balances` | `maintenance` | `reconcile-balances:{organization_id}:{data-negocio}` — **implementado em 2026-08-22**, por organização (não por conta ML). Payload `{ organizationId }`. Compara `compute_erp_target_balances` contra `inventory_balances`, gera `AJUSTE_RECONCILIACAO` + `stock.balance.adjusted` por divergência (D-029). **Duas correções de nomenclatura em 2026-08-29**: a função era `compute_erp_snapshot_balances` até D-132 trocá-la pelo alvo ROLADO PARA A FRENTE (e o nome antigo, deixado aqui, é o mesmo que quebrou a rodada de 29/08 em produção quando o worker no ar ficou para trás — ver D-134); e o evento era `stock.balance.diverged` até D-135 separar as duas origens |
...
| `sync.support.questions.reconcile` | `ml-sync-<conta>` | `support-questions:{conta}:{dia-de-negócio}:{bloco-6h}` — **implementado em 2026-08-25** (D-089), rede de segurança do webhook `questions`. Payload `{ mlAccountId }`. Pagina `GET /my/received_questions/search?api_version=4&status=UNANSWERED` por `offset`/`limit=100`, mapeia (D-086) e persiste pela mesma porta idempotente, item a item. `resource: "questions"` — primeiro uso do valor, exigiu alargar o CHECK de `sync_runs`/`sync_errors` (`20260825180000`). **Só `UNANSWERED`**: a API não tem filtro por data e não garante ordenação (`docs/MERCADO_LIVRE.md` secao 2.12), então "última janela" não é expressável. Teto de 20 páginas por execução; truncar vira `sync_run` PARCIAL, nunca `done`. **Gatilho automático**: `POST /internal/schedule/support-questions` + Cloud Scheduler a cada 6h (job `v3-support-questions-reconcile`) |
...
| `stock.balance.adjusted` | informativo | reconciliação contra o UpSeller (D-029) — **separado de `stock.balance.diverged` em 2026-08-29 (D-135)**. Aqui a divergência é ESPERADA (o ERP é operado por gente) e a MESMA execução já grava o `AJUSTE_RECONCILIACAO` que corrige o saldo: quando o evento existe, o problema acabou. Era `critico` e produzia 657–896 alertas/dia sobre rotina resolvida |
| `stock.balance.diverged` | crítico | vigia de integridade ledger×projeção (D-056), **e só ele desde D-135**. As duas fontes são internas e não deveriam divergir NUNCA, por construção; o job só DETECTA, nunca corrige. Com o truncamento de D-131 corrigido este caminho emite ZERO — medido em 2026-08-29 contra o banco real: 3.472 chaves comparadas, 0 divergências, contra 1.683/1.366 na última rodada da versão truncada. `before`/`after.checkedAgainst` continua marcando a origem. **Linhas históricas com prefixo `reconciliacao:` no `dedup_key` carregam o nome antigo** — `domain_events` é append-only e não houve backfill |
...
| `order.return.unreversed` | crítico | sync — **implementado em 2026-09-02 (D-208)**, mesmo handler. Emitido quando a devolução chega para um pedido SEM linha em `order_items`: sem a `position` não há como localizar a venda, então nada é revertido e o estoque fica preso deduzido. Até D-208 esse caminho era um `logger.warn` e um `continue` — o job fechava `done` e o banco não guardava vestígio, indistinguível de um no-op legítimo (D-205 mediu 4.903 deles saudáveis em `post_purchase`). **Não** falha o job: repetir a busca não faz a linha aparecer (D-202). Severidade CALIBRADA: teria disparado **zero** vezes em toda a história da base — os únicos 2 candidatos em 338.791 pedidos estão `delivered` desde julho sem reclamação —, então `critico` aqui não repete o erro de D-135 |
...
| `central.alert.opened` | importante | `sincronizar_alertas_central` — **implementado em 2026-09-25 (D-411)**. O episódio NOVO de severidade alta de um alerta da central (frete forte, campanha crítica, produto com margem até -10%), na mesma transação que o abre. `entity_type = 'action'`, `entity_id` = id da ação; `after` com `kind`, `subject_key`, `mlb_id`, `sku_id`, `impacto`, `recomendacao`. Sem conta no alerta de produto (vai à organização inteira). Chave `central.alert.opened:{actions.id}` — um por episódio. Medido: 15 no primeiro dia (a carga inicial), nenhum no segundo |
