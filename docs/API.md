...
| `sync.fulfillment.snapshot` | `ml-sync-<conta>` | `full:{conta}:{janela}` — **implementado em 2026-08-22**: uma execução varre TODOS os itens sem variação da conta (via `sku_listing_links`), não um `inventory_id` por task. O sketch original (`full:{conta}:{inventory_id}`) presumia granularidade por item, que dependeria de `sync.listings.page`/uma tabela `listings` ainda inexistente; a implementação real usa o vínculo já existente e dedupe por conta+janela, mesmo formato de `sync-orders:{conta}:{janela}`. Payload `{ mlAccountId }`. **Ainda não enfileirado por nenhum trigger** (Cloud Scheduler/`api`) — só o handler existe; disparo automático é a próxima etapa |
