...
| `/internal/schedule/order-logistics` | Cloud Scheduler | Varredura da logística do envio (D-352, R2) — lê `GET /shipments/{id}` dos pedidos com `VENDA_ML` sem estorno e ainda sem sinal, e grava o `ESTORNO_FULL` que falta quando o envio é `fulfillment`. A cada 6h (`40 */6`), por CONTA, dedupe por `order-logistics:{slug}:{hora-cheia-ISO}`; teto de 800 leituras por rodada. **Na branch, não publicado** (ordem em `docs/DEPLOYMENT.md` 8.3) |
