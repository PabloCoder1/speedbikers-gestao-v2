...
|---|---|---|---|
| `/webhooks/mercado-livre` | POST | Validação própria da origem | ACK rápido e enfileira. **Tópico sem consumidor responde antes de qualquer I/O (D-339)**; os tópicos com trabalho consultam `ml_accounts` e criam a Cloud Task antes do ACK — duas idas de rede, não zero. Desde 2026-08-25 (D-088) o ACK roteia por tópico: `questions` vira `sync.support.questions` com `{ mlAccountId, questionId }`; todo o resto continua em `sync.webhook.received`. Uma regra de dedupe só (`ml-webhook:{resource}:{janela-minuto}`) para os dois. `questions` com `resource` fora de `/questions/{id}` responde 200 sem enfileirar e sai como `ml_webhook_unroutable_resource` no log |
