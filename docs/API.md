...
| `/v1/listings/relist` | POST | ADMIN/GESTOR | **Implementado em 2026-08-31 (D-161)** — pedido de republicação (Fase 9): valida papel + escopo por conta (lição D-117) + anúncio conhecido, e **enfileira** `relist.prepare` na fila da conta. O worker captura o `parent_snapshot` e roda o preflight (D-160); a operação nasce `REQUESTED` ou morre `PREFLIGHT_FAILED` com os motivos. **Nada destrutivo nesta rota** — fechamento do pai e POST /relist são fatia própria, atrás de confirmação adicional. Sem OPERADOR: republicar começa fechando um anúncio (irreversível), decisão de gestão |
