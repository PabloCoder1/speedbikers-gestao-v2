...
- **Escrita em lote dentro do recorte — FEITA em D-393**, pela RPC `mark_notifications_read` (`security invoker`, migration `20260923170000`): quem filtra uma família limpa AQUELA e deixa as críticas por ler. Sem recorte a escrita continua sendo o `update` direto sob RLS de D-074.
