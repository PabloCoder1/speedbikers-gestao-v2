...
Granularidade: por usuário, por `event_type`, por severidade mínima e por conta — as quatro dimensões existem em `notification_preferences` (`event_type`/`ml_account_id` nuláveis como curinga, linha mais específica vence). Se uma regra só por evento e outra só por conta empatarem em especificidade, vence deterministicamente a mais restritiva (`enabled = false` antes de ativa; depois, maior severidade mínima), nunca a ordem de criação. Identidade da regra (evento + conta) é fixa depois de criada; severidade mínima e estado ativo/desativado são editáveis, o resto é apagar e recriar.
