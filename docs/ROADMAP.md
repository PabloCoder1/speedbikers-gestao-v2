...
- [ ] **Reduzir a população de casts sobre resultado de embed.** É o portão que dispensa portão: sem `as`, o tipo gerado valida a cardinalidade de graça, e a classe de defeito de D-188 deixa de existir naqueles pontos. `apps/api/src/copilot.ts` é o padrão a copiar. Hoje são 11 arquivos, todos corretos — isto é trabalho de prevenção, não de correção.
