...
- [x] **Desfazer vínculo + histórico auditável de vínculo** — banco concluído em 2026-08-28 (D-125): `sku_listing_link_events` append-only (CREATED/RETARGETED/REMOVED, com autor, motivo e SKU anterior) e três RPCs. **Remoção FÍSICA, não `removed_at`** — soft delete quebraria `resolveSku` (duas linhas na mesma chave natural → `.maybeSingle()` → a guarda anti-overselling dispara sobre estado legal), o anti-join de `get_unlinked_listings` e o revínculo. **Junto: fechado um furo aberto desde a Fase 2** — `authenticated` tinha DELETE/INSERT/UPDATE com policy `for all`, então qualquer OPERADOR já apagava vínculo pelo PostgREST sem auditoria. **SEGUE ABERTO**: a supressão no importador (vínculo removido à mão volta na próxima planilha) e o botão na interface
