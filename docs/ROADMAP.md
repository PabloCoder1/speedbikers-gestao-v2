...
- [ ] Exportação do pedido de compra em Excel (principal) e PDF (D-034) — bloqueado: modelos de referência ainda não recebidos do usuário
- [ ] **Geração de `stock_movements` (`ENTRADA_TRANSITO`/`RECEBIMENTO_TRANSITO`) a partir do ciclo do pedido de compra** — achado em revisão, 2026-08-22: o ciclo em si (item acima) não move o ledger; falta decidir QUANDO nasce TRANSITO (aprovação? confirmação de envio pelo fornecedor, que a V3 não tem como saber sem integração?) antes de implementar. É o que fecha TRANSITO de fato
- [ ] **Tela de edição do pedido em `DRAFT`** — achado em revisão, 2026-08-22: a RPC `update_purchase_order_draft` já existe e funciona (testada), só falta a UI em `/compras/[id]`
