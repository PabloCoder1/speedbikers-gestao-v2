...
**Marco atingido, com uma ressalva encontrada em revisão (2026-08-22):** pedidos ficam frescos **em até 1 hora** (reconciliação por janela) — não "em minutos" como este marco dizia. `sync.webhook.received` é enfileirado a cada notificação do Mercado Livre (ACK rápido, isso funciona), mas **não existe handler registrado pra esse tipo de job no worker** — `events.detect`, já citado como "não implementado, de propósito" no item acima, é exatamente essa peça que falta. O job fica na fila até esgotar tentativas e é descartado, sem efeito. Não é regressão: já estava documentado como decisão consciente ("de propósito"), só a formulação do marco estava otimista sobre o que isso significa na prática. Rastreado como item novo abaixo. Linha do tempo do ANÚNCIO especificamente fica para quando `listings` for construído (não estava no checklist desta fase) — a tela já está pronta para mostrar esse recurso assim que existir.

- [ ] **Fast Path do webhook** (achado em revisão, 2026-08-22, não bloqueia nada — a reconciliação horária já cobre frescor real): handler para `sync.webhook.received` — extrai `order_id` de `resource`, busca `GET /orders/{order_id}`, chama `persistOrder` (mesma função já usada por `sync.orders.window`/`backfill.orders`). Reduziria o frescor de "até 1h" para "segundos" no caminho feliz, com a reconciliação continuando como rede de segurança (papel que ela já tem hoje, documentado). Junto: revisar `dedupeKey` do webhook (`ml-webhook:{resource}`, sem janela de tempo) — mesma classe de risco que D-051 já corrigiu para o recompute (Cloud Tasks pode reter um nome de task por até 24h; duas notificações do MESMO recurso dentro dessa janela colidiriam e a segunda seria descartada)
...
- [ ] **Conferência automática ledger × projeção, com evento crítico na divergência** (job agendado via Cloud Scheduler chamando `private.compute_inventory_balances_from_ledger` e comparando contra `inventory_balances`) — a função SQL já existe desde 2026-08-21, mas nunca foi ligada a um job; agora que venda e cancelamento gravam movimento de verdade, há o que conferir. Promovido de nota solta para item próprio em 2026-08-22 (achado em revisão). **Achado ao implementar a reconciliação contra o UpSeller (2026-08-22): `private.compute_inventory_balances_from_ledger` é INALCANÇÁVEL do worker como está** — `supabase/config.toml` expõe só `schemas = ["public", "graphql_public"]` ao PostgREST, e o worker fala com o Postgres via `@supabase/supabase-js` (PostgREST), nunca conexão direta. Mover para `public` com `GRANT` restrito a `service_role` antes de implementar este job (mesmo ajuste já feito em `compute_erp_snapshot_balances`, ver `docs/DECISIONS.md`/`docs/HANDOFF.md`) — a segurança não depende do schema, o `GRANT` já impede `anon`/`authenticated`
- [ ] **Ajuste manual de estoque (tela/ação)** — `movement_type = AJUSTE_MANUAL` já existe no schema (`docs/DATABASE.md` secao 4), mas não há UI nem endpoint que o gere; hoje um ajuste só pode nascer via SQL direto. Achado em revisão, 2026-08-22
...
- [x] Reservado e em trânsito — **RESERVADO concluído em 2026-08-22, TRANSITO fica de fora de propósito.** As duas metades tinham maturidade muito diferente: nenhum código grava `location_kind = 'RESERVADO'` (venda/cancelamento/NF-e são sempre LOCAL) — a reconciliação contra o UpSeller (item abaixo) é a ÚNICA fonte, "Ocupado" no ERP vira RESERVADO. TRANSITO depende de Pedidos de Compra (ainda não implementado) — as colunas de trânsito do UpSeller vêm zeradas em 100% do export real (`docs/UPSELLER.md` secao 6), reconciliar contra elas geraria ajuste incorreto. Por isso os dois itens do checklist são a MESMA implementação, confirmando o que `docs/HANDOFF.md` já apontava
