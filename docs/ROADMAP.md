...
- [ ] NF-e/XML: upload, parse, conferência, confirmação humana, movimentos, `content_hash` UNIQUE — schema (`documents`/`document_items`) e parse (`nfe.import.parse`) prontos em 2026-08-22, contra o layout oficial (usuário optou por não esperar XML real). Faltam: rota de upload, tela de conferência, vínculo humano por item, geração de `stock_movements` na confirmação
