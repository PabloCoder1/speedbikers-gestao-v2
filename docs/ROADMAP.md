...
- [ ] Confirmar `maintenance.reconcile-balances` no próximo ciclo natural depois do fix que eliminou a lista excessiva de UUIDs no PostgREST. **Tentativa em 2026-08-25 falhou** ("JWT issued at future" ao listar organizações, zero organizações escaneadas) por um erro que parece transitório (mesma instância `api` teve duas outras chamadas ao Supabase bem-sucedidas segundos depois) — aguardando o ciclo de 2026-08-26 para confirmar autorresolução. Ver D-081 em `docs/DECISIONS.md`.
...
- [ ] **Registro de ferramentas do Copiloto e orquestração com streaming** — **registro de ferramentas concluído em 2026-08-25 (D-077)**: `packages/contracts/src/copilot-tools.ts` (schema Zod de entrada/saída), `POST /v1/copilot/query` (`apps/api/src/copilot.ts`), três ferramentas de vendas (`sales_summary`/`sales_period_comparison`/`sales_account_comparison`), leitura sob RLS real via `UserClient` novo (`@sb/db`). **Orquestração com streaming continua pendente** — só o caminho de curto-circuito existe (sem LLM, resposta síncrona); o planner por linguagem natural e o streaming de verdade dependem da escolha de modelo/orçamento (`docs/COPILOT.md` secao 10, pendência aberta).
