...
- [ ] **Custo de decodificação de WAL do Realtime** — achado em D-198: **43,4% de todo o tempo do banco** (78.261 chamadas, 496 s). A publicação está mínima e correta (só `notification_recipients`), então o custo escala com o volume de escrita total. Os maiores produtores são `daily_listing_metrics` e `daily_sku_metrics`, **485 mil escritas por dia** por apaga-e-insere — a tabela inteira reescrita 2,6× por dia. Medir se 1.080 recomputes/dia para ~512 pares (conta, dia) é o piso ou ainda tem folga.
