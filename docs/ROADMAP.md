...
- [ ] **`get_system_health`: escopo de plataforma com guard de tenant** — bloqueio a resolver **antes da segunda organização** (D-182). A função é de escopo de plataforma mas é protegida por `role = 'ADMIN'` sem correlação com organização. As duas correções óbvias causam regressão verificada: filtrar por organização apaga o heartbeat `system.ping` (a razão de ser da tela) e mudar a assinatura quebra `/saude` e os tipos. A correção que cabe é trocar o **guard** por um papel de operador da casa, não os dados.
- [ ] **`ml_accounts`: UPDATE/DELETE para `authenticated` sem consumidor, e sem trilha de auditoria** — registrado em D-182. Todas as escritas reais rodam como `service_role`; o DELETE atravessaria a blindagem de `ml_credentials` por cascade. As duas tabelas irmãs (`organization_members`, `user_account_permissions`) têm trigger de auditoria; esta não tem.
- [ ] **`pg_default_acl`: o que D-182 deixou aberto** — fechou `postgres`/tabelas e `postgres`/sequences. Continuam abertos `postgres`/funções (as RPCs dependem de `grant execute`; exige varrer todas as migrations que criam função) e todos os defaults de `supabase_admin` (deliberado — é o papel da plataforma). O teste de CI «nenhuma tabela de public dá QUALQUER privilégio a anon» é a guarda que cobre os dois casos.
...
- [ ] **Os quatro casts que sobraram com defesa em jogo** (`usuarios:121`, `anuncios/[itemId]:133`, `atendimento/[caseId]:212` e `:247`) — a cadeia de catálogo diz que os embeds não podem ser nulos, mas medir isso exige fixture com duas organizações, como o de D-192. Só vale numa fatia que já monte esse fixture.
