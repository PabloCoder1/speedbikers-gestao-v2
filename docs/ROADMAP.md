...
**Fase 3 — Mercado Livre e histórico** está em andamento. Sete dos oito itens do checklist concluídos: cliente `@sb/mercado-livre`, o webhook, a conexão OAuth de conta, a reconciliação por janela, o backfill retomável, a persistência estruturada de pedidos e o motor de diff/`domain_events` — ver `docs/HANDOFF.md`. Último item: Tela de Saúde da Sincronização.
