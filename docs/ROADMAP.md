...
**Fase 3 — Mercado Livre e histórico** está em andamento. Três itens do checklist concluídos: cliente `@sb/mercado-livre` (OAuth, backoff/jitter, classificação de erro, paginação offset), o webhook `POST /webhooks/mercado-livre` (allowlist de IP, resolução de conta, enfileiramento em `ml-sync-<conta>`) e a conexão OAuth de conta (`POST /v1/ml-accounts/connect` + `GET /oauth/mercado-livre/callback`, tokens cifrados com AES-256-GCM, D-046) — ver `docs/HANDOFF.md`. Próximo item: reconciliação por janela via Cloud Scheduler.
