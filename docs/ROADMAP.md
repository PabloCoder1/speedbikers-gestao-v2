...
- [x] Implementar filtros de Conta / Origem / Marca nas telas em que fizerem sentido, preservando a distinção entre estoque físico compartilhado e Full por conta. **PARCIAL em 2026-09-03 (D-235): `/curva-abc` ganhou o filtro de Marca** (já tinha o de Conta), com a curva **recalculada dentro do recorte** — mesma semântica que o filtro de conta já usava. Marca é `skus.supplier_brand`, nunca `skus.brand`: esta guarda a CATEGORIA do UpSeller (D-129) e diverge da marca real em **2.320 dos 3.554 SKUs**. **Origem segue FORA**, pelo mesmo critério: `is_imported` é medido como não confiável. **`/cobertura` entregue em 2026-09-03 (D-236)**, também sem filtro de conta — e o teste fixa que `?conta=` na URL é ignorado, porque estoque físico é da organização. Lá os TOTAIS do cabeçalho vêm de outra RPC e recebem o mesmo filtro; `history_days_90` NÃO segue o recorte de propósito (é guarda do pipeline de métricas, D-145). **CONCLUÍDO em 2026-09-03 com `/vendas` (D-237)** — a mais difícil das três, e a dificuldade não era SQL: **nem todo número de vendas tem versão por marca**. Cinco RPCs, cada uma chamada duas vezes. Receita, unidades, pedidos, preço médio, taxas do ML e SKUs distintos aceitam o recorte (decomposição provada: a soma dos itens bate exatamente com o total dos pedidos); **compras por pack, ticket médio, o trio de cancelamento e a margem inteira voltam NULL** — pack atravessa SKU e frete é do pedido, não há cota de marca (`docs/METRICS.md` 5E). As duas RPCs que liam o rollup de conta trocam de fonte **só quando há filtro**, por `union all` com guardas exclusivas: plano conferido, o ramo fino não aparece sem recorte (133 buffers, iguais aos de antes). **"Sem marca" é valor do filtro** — 23,2% da receita não tem SKU. **Origem segue FORA** nas três telas, pelo mesmo critério de sempre.
- [ ] Impedir mistura de SKU Nacional e Importado no mesmo pedido de compra. 🔵 **NÃO BLOQUEIA A V3** — **DEPENDENTE DE DADO** (anotado em D-337, sem mudar o estado): a mistura já é **avisada**, nunca bloqueada, na ponte cobertura → pedido (`detectOriginMix`, D-151). Bloquear exige uma origem confiável, e as duas colunas fiscais foram medidas contradizendo a rota de compra: `origin_code` em 707 SKUs (D-129) e `is_imported` em 187 dos 228 NAVETEC (D-139).
- [x] Criar fluxo de vinculação manual `Conta + MLB + variation_id? -> SKU` sem exigir `link_candidate` prévio — ✔ 2026-08-28 · D-119/D-117
- [ ] Criar alias reutilizável `Fornecedor + código do produto -> SKU` quando um vínculo for confirmado. 🔵 **NÃO BLOQUEIA A V3** — **DEPENDENTE DE RELAÇÃO INEXISTENTE** (anotado em D-337): D-174 mediu que não há vínculo fornecedor → SKU (`supplier_product_links` nunca foi criada; `skus.supplier_brand` é marca), e um alias só nasce de um vínculo confirmado.
- [x] Evoluir o Dashboard de SKU para abas/progressive disclosure — **primeira migração feita em 2026-08-31 (D-169); o item segue ABERTO de propósito, porque o próprio item manda "migrar incrementalmente" e faltam 6 das 11 abas.** Entregue: `Visão geral | Estoque | Anúncios | Histórico | Diagnóstico` na URL (`?aba=`), com progressive disclosure de verdade — cada aba dispara SOMENTE as suas consultas (a página saiu de 5 consultas fixas para 1–3). Valor fora do conjunto fechado cai para a Visão geral antes de tocar o banco. Vendas ficou como dois números na Visão geral, não como aba. **Primeira tela do projeto VISTA RENDERIZADA** (ensaio local com screenshot de cada aba). **ESCOPO REDUZIDO EM 2026-09-03 (D-224): 11 abas → 9.** Saem `Tráfego` (pertence ao ANÚNCIO — visita é medida por `item_id`, e o Dashboard 360º de D-168 é o dono) e `Atendimento` (não existe vínculo confiável SKU → `support_case`: D-084 registra que um case tem vários SKUs, sem "principal"). **Conferido no código em 2026-09-03:** 8 das 9 existem — as cinco de D-169 mais `Full` (D-225) e `Preços` (D-226) por reuso de RPC existente, e `Vendas` (D-227) com a única RPC nova das abas, `get_sku_sales_breakdown` (total, por conta e por dia num round trip, via grouping sets; razões sobre as somas). `Decisões` (D-228) fechou o conjunto: leitura direta sob RLS com o embed `actions!inner`, sem cast, provado rodando (`check:embeds` 34/34 e Playwright) e com teste de paridade das três policies (que D-181 reescreveu para a forma de conjunto — o teste fixa a igualdade entre elas, não o texto). **Concluído em 2026-09-03: as nove abas de D-224 existem.** Era categoria **B** de D-223.
...
- [~] **A venda acha o SKU pelo user product** (13% dos itens vendidos em 30 dias saem sem SKU; 3.588 vínculos `USER_PRODUCT` parados) — 1ª parte ✔ produção 2026-09-25 (`worker-00028-g7n`): `order_items.user_product_id` gravado do próprio pedido, migration `20260925170000`. 2ª parte ✔ código 2026-09-25: a venda sem vínculo por anúncio resolve pelo vínculo `USER_PRODUCT` (o por anúncio vence), com a baixa de estoque — hoje ~15-25 itens por dia (1,6-4%; eram 17-24% no fim de agosto). Falta: o histórico (~3.300 itens de 26/08 a 14/09, só relatórios) · D-362
...
- [x] **Bloqueio inicial de Full e Catálogo** — ✓ D-160 (`FULL_BLOQUEADO`/`CATALOGO_BLOQUEADO`), re-avaliado na execução (D-162); o de Full passou a olhar o estoque lido ao vivo, não o cadastro (D-360)
...
- [ ] **Experiência de compra na republicação** — pedido do dono em 2026-09-17, depois de D-352 e D-362: captura diária por anúncio; a nota no painel ANTES de fechar, guardada no retrato do pai (fechado, o anúncio sem user product na raiz só devolve cinza); a nota do filho na medição 7/15/30. Medido em 17/09: com user product na raiz, pai e filho leem a MESMA nota (`/reputation/items/{id}/purchase_experience/integrators` responde 302 para o user product); sem ele, a nota é do item e o efeito do relist ainda não foi medido. A V3 não escolhe o que o ML herda (o relist não tem parâmetro para isso).

- [x] **Experiência visual final da republicação** — ✔ D-295/D-310, **conferida item a item no código em 2026-09-13 (D-337)**: o checkbox tinha ficado aberto depois de D-295, e o HANDOFF já a dava como feita. Categoria **A, alinhamento da Fase 9 com o Figma**, sem duplicar o fluxo existente: **ação secundária** — "Republicações →" no cabeçalho do anúncio, que leva ao painel (D-310); **superfície de segurança** — painel com confirmação, e a execução nasce travada até a caixa de ciência (D-295); **preflight e riscos visíveis** — o texto do pedido diz o que a conferência recusa (já republicado, estoque no Full, catálogo, já filho), e o motivo da falha é coluna da tabela; **Full** — aba própria do anúncio, somado por variação; **catálogo** como atributo não existe em `listings` (recusa registrada na página); **dois atos** — pedir e executar; **progresso por estado** — os nove estados com rótulo em português e teste; **pai → filho** — a tabela lê o anúncio dos dois lados, com link para o filho; **remapeamento** — o estado `REMAPPED` (D-163); **acompanhamento 7/15/30** — **não no painel do anúncio**: a medição de D-164 é uma decisão com `action_outcomes`, e as janelas aparecem na aba **Decisões do SKU**, que lê por `actions.sku_id` — republicação sem SKU vinculado não aparece ali. A primeira versão visual não promete ganho de exposição, reputação ou venda e não autoriza a IA a executar. **O ensaio real contra o Mercado Livre segue ato humano** (D-162).
...
**D-391 recompôs a leitura:** as áreas saem em zonas rotuladas por **presença de configuração** (FALTA CONFIGURAR / COM CONFIGURAÇÃO / NÃO FOI POSSÍVEL LER), de modo que o que falta configurar abre a tela — a ordem de `describeSettings` continua a deste item, e o teste de unidade a prende. Cada cartão passou a dizer o que a área INCLUI no vocabulário literal da tela dona (guardado por `check:settings-vocabulary`), quem altera em segunda pessoa e a consequência quando há uma: cinco frases tiradas dos 16 campos que a RPC já devolve, cada uma terminando na ação e na tela que a resolve — ou, para quem não pode agir, no pedido a quem pode. A camada `hero` saiu (193px da primeira dobra em 1440px, 328px em 390px, para repetir o subtítulo) e a grade deixou de rolar para o lado no celular.
...
- [ ] **Atos externos** (não são código): branch protection da `v3` — **medida em 2026-09-14 (D-350): 2 checks obrigatórios** (`typecheck, lint, test, build` e `integração — migrations e RLS contra Postgres real`), com `enforcement_level: non_admins` — **não valem para admin**, e e2e e scripts de infraestrutura não são obrigatórios; em D-334 (13/09) não havia check nenhum. E `Leaked Password Protection` no Auth do Supabase, **no Dev e em produção**: o lint sumiu do advisor nos dois projetos, o que não prova que está ligada — nenhuma ferramenta lê essa configuração.
...
- [x] **Indicadores com comparação e tom** (faturamento, pedidos, ticket, unidades, resultado, margem, custo, comissão, frete, Ads), presets hoje/ontem/7/15/30 dias/mês atual/mês anterior terminando ontem, tabela de comparação e resumo em texto com a decomposição exata da margem — ✔ código 2026-09-23 · D-394. Migration `20260923100000` (só catálogo) a caminho de produção
...
- [x] **Alíquota de imposto com vigência** e lucro após imposto e Ads (`tax_rates`, imposto pedido a pedido em `get_faturamento`, `margem_contribuicao` de METRICS 5K) — ✔ código 2026-09-23 · D-395. Migration `20260923100100` a caminho de produção
- [x] **Calendário de datas comerciais** para a projeção — Carnaval, Consumidor, Mães, Pais, Black Friday e Natal por regra; o peso de cada dia da data vezes o efeito medido no mesmo dia da ocorrência do ano anterior, no esperado, na projeção e no ritmo; sem a ocorrência anterior no histórico, a data é dita e não pesa — ✔ código 2026-09-24 · D-406. Migration `20260923195955`
...
- [x] **Frete: gravar subsídio e frete do comprador** — `order_financials` ganha o frete cheio, o desconto do Mercado Livre no frete do vendedor, o frete pago pelo comprador e o bancado para ele, da resposta de `/costs` que já é lida (forma conferida em 12 envios reais: as quatro partes fecham com o frete cheio) — ✔ código 2026-09-24 · D-407. Migration `20260923195956`, antes do worker
- [x] **Frete: quem paga, na central** — `get_quem_paga_frete` e o bloco em `/central/frete`: frete cheio, vendedor, comprador e o que o Mercado Livre bancou de cada um nos últimos 30 dias, por logística, somado por envio (os pedidos de um pacote dividem o envio) e com a cobertura do detalhe dita (desde 25/09) — ✔ código 2026-09-25 · D-412. Migration `20260925150000`. Recuperar os 90 dias (~70 mil chamadas de `/costs`) só se o dono quiser
- [x] **Ads: sinais e recomendações** por campanha — `get_sinais_ads` e `/central/ads`: crítico, ROAS abaixo de 80% da meta, atenção (CPC e conversão, gasto e ROAS) e oportunidade de escala, com interpretação por CTR e conversão contra a mediana das campanhas e lucro estimado pela margem média (premissa declarada); dias de Ads que o Mercado Livre ainda não consolidou ficam fora dos julgamentos também na central e no faturamento — ✔ código 2026-09-23 · D-398, dia consolidado pela data do sync em D-401. Migrations `20260923195000` e `20260923195800`
...
- [x] **Frete: o detector sem estourar o tempo** — `shipping_sales`, uma linha por venda elegível mantida por gatilhos: o detector deixa de juntar 79 mil pedidos em páginas espalhadas (626 ms e 388 mil páginas com cache; 9,7 s a frio na sincronização das 8h) e dá o mesmo resultado na metade do tempo quente — ✔ código 2026-09-25 · D-409. Migration `20260923195958`
- [x] **Frete: medidas do pacote** — `shipment_packages` grava, da leitura de envio que já acontece, as medidas que o Mercado Livre usou em cada envio (`shipping_items[].dimensions`, cm e g, com a origem); o anúncio entra como sonda de `shipping.dimensions` no log — ✔ código 2026-09-24 · D-405. Migration `20260923195950`
- [x] **Frete: o envio compartilhado conta uma vez** — os pedidos de um pacote que repetem o custo do envio o dividem pelo valor (`order_financials.seller_shipping_share`, mantida por gatilho), e o faturamento, a central, o ranking, a margem de /vendas, o detector e quem paga o frete somam a parte; custos diferentes no mesmo envio (Full) ficam como vieram — ✔ código 2026-09-25 · D-413. Migration `20260925160000`
- [ ] **Frete: o detector usa as medidas** — pares de tamanho parecido pelas medidas dos envios (desde 24/09, 100% dos pacotes com medida) e o motivo "medida suspeita"; o declarado contra o cobrado ficou sem fonte: o multiget não traz `shipping.dimensions` (0 de 4.365 anúncios, D-405)
...
- [x] **Notificação dos alertas da central** — o episódio NOVO de severidade alta grava um `domain_event` `central.alert.opened` (importante) na mesma transação; o fan-out entrega a quem alcança a conta (o de produto, sem conta, à organização inteira), com toast e a família "central" na triagem; o link leva a `/acoes?tipo=` — ✔ código 2026-09-25 · D-411. Migration `20260925140000`
...
- [x] **Limites que moram no SQL** — a margem mínima dos produtos (`get_faturamento`) e o piso do ROAS contra a meta (`get_sinais_ads`, e o alerta gravado dele) lidos de `central_thresholds`, com a resposta dizendo o corte que usou; as pontuações do detector de frete ficam como calibragem — ✔ código 2026-09-25 · D-410. Migration `20260923195959`
...
- [x] Projeto Supabase de produção e Cloud Run de produção — **CRIADOS em 2026-09-14 (D-348, D-349, D-350)**: GCP `speedbikers-prod` (api e worker em `36a23ad`, 15 jobs, 7 filas), Supabase `speedbikers-prod` com as 166 migrations aplicadas por `migrations-producao.yml` em duas aprovações, e Vercel `speedbikers-prod` lendo o banco e a api de produção — o que fecha o buraco da Vercel escrito abaixo. O passo 10 da 8.2 e o corte seguem no rollout. **Antes disso, scripts prontos para um segundo ambiente e roteiro escrito em 2026-09-13 (D-333)**. Os scripts de `infra/` tinham o Supabase **fixo no Dev**: um deploy com o projeto de produção subiria apontando para o banco do Dev. Agora `AMBIENTE=dev|prod` é explícito, produção não tem padrão e exige `CONFIRMO_PRODUCAO=sim`, e a guarda que recusa mistura tem 13 casos na CI (provada por mutação). Roteiro em `docs/DEPLOYMENT.md` 8.2. **Criar os projetos é ato humano**, e dois buracos ficaram escritos: migrations de produção sem caminho (a CI só liga o Dev) — **fechado em D-334**: `.github/workflows/migrations-producao.yml` recusa rodar fora da `v3`, sem o ambiente `producao` travado (revisor e restrição de branch) ou sem a CI do mesmo commit verde e já aplicada no Dev, e aplica em duas aprovações; guarda com 24 casos na CI, provada por mutação — e o projeto da Vercel servindo produção com as variáveis do Dev — **fechado em 2026-09-14** pelo projeto `speedbikers-prod`
- [ ] Testes de carga e revisão de `pg_stat_statements` — **a carga REAL foi medida em 2026-09-13 (D-339)**, nos logs do Cloud Run: 65.801 webhooks em 24 h, pico de 1.050 por minuto, e o ACK passando de **7 s nos picos** contra a regra de 500 ms do Mercado Livre. A causa — tópico sem consumidor consultando o Postgres antes do ACK — foi atacada em D-339 e **revertida em D-340**: responder antes da consulta tirou o I/O do sem consumidor, mas esfriou as conexões e levou os ACKs lentos de `orders_v2` de ~1% a ~9%. O pico continua aberto. **A medição por etapa entrou em D-343** (`lookup_ms` e `enqueue_ms` no log do webhook); **a primeira rajada medida (D-345) apontou a consulta da conta sob concorrência** — p95 de 2,8 s na rajada, contra 103 ms fora dela, com a Cloud Task igual. A correção, com as contas em memória, foi escrita em D-346 e publicada em `api-00040-qrk`: depois da primeira carga, `lookup_ms` caiu para 0 ms, e **na rajada grande de 14/09 13:49 UTC** — 319 webhooks no mesmo segundo, 1.283 no minuto — ficou em 0 ms, com o ACK em no máximo 190 ms, numa instância só (D-346, seções 5 e 6). **O pico do ACK fechou**; ainda não se viu a duração de oito minutos das rajadas de fim de semana. Falta: a revisão de `pg_stat_statements` do Dev (`report:health` exige a senha do banco — ato humano) e decidir se as telas pedem teste sintético de concorrência (o tráfego de usuário na `api` é de 89 chamadas em 7 dias; as telas leem o Supabase direto)
- [ ] Backup e restore verificados — **O BACKUP FOI MEDIDO e o ENSAIO ESTÁ PRONTO em 2026-09-13 (D-332)**: backup físico diário, ~7 dias de retenção (oito listados), PITR não confirmado; arquivos no GCS com soft delete de 7 dias e sem versionamento. A condição de D-159 está satisfeita — há backup automático, nenhuma máquina própria se constrói. **Falta a metade que fecha o item, e ela é humana**: restaurar um backup num projeto novo e rodar `pnpm --filter @sb/db run check:restore` (roteiro em `docs/DEPLOYMENT.md` 8.1; comparador provado nos dois sentidos contra dano simulado). **Medição parcial em 2026-08-31 (D-159)**: a metade do SCHEMA já é provada todo dia (CI e o ambiente local recriam as 93 migrations do zero); o buraco é o DADO. Se o projeto Supabase tem backup automático depende do PLANO, que nem o MCP nem a API expõem — **ato do usuário**: Dashboard → Database → Backups do projeto `speedbikers-gestao-v3-dev`, e relatar o que existe (diário? PITR? nada?) antes de qualquer máquina própria de backup ser construída
- [x] Revisão de segurança e de secrets — **CONCLUÍDA em quatro fatias (D-328 → D-331), sem lacuna técnica aberta.** **Fatia 1 em 2026-09-13 (D-328)**: **nenhum segredo real** na árvore nem nos 740 commits do histórico (varredura por formato E por nome de variável; os 27 valores achados são fixture, padrão local da CLI ou chave publicável), e o `pnpm audit --prod` foi de **2 críticas + 1 alta + 4 moderadas para 1 moderada** (`next` 16.3.5, `sharp` 0.35.4, `hono` 4.13.7; o `uuid` residual é inalcançável e ficou aceito com o motivo). E uma decisão de fluxo sem dono: guarda de `audit` na CI reprova pela data, não pelo código — **decidida pelo usuário em D-336**: `pnpm audit --prod --audit-level high` como último passo da job `check`. **Fatia 2 (D-329):** a superfície de entrada conferida **ao vivo** — worker com um único invoker (a SA do Cloud Tasks) e **403** sem credencial; `/internal/*` com OIDC de audience e e-mail fechados; CORS só para `WEB_ORIGINS`; **D-045 fechada com tráfego real** (≥ 5.000 webhooks aceitos em 24 h, 0 rejeitados, 0 `X-Forwarded-For` ilegível). E a `web`, que não tinha **nenhum** cabeçalho de segurança além do HSTS da Vercel, ganhou `frame-ancestors 'none'`, `X-Frame-Options`, `nosniff`, `Referrer-Policy` e `Permissions-Policy`, com e2e. **Fatia 3 (D-330):** o logger redigia só pelo NOME da chave e deixava passar `{ reason: error.message }` inteiro — inclusive `message` e `stack` de `Error`. As regras de VALOR (rótulo + forma, DSN, `APP_USR-`/`TG-`/JWT/`sk-ant-`) existiam só na tela; saíram para `@sb/observability` e passaram a ter dois consumidores, logger e tela. Medido antes: **0** ocorrências em 7 dias de log do Dev (controle: 432 `job_failed`) — a rede é preventiva. **Fatia 4 (D-331):** os cabeçalhos de D-329 **confirmados no ar na Vercel** (deploy `13e6f15`), e a **CSP completa com nonce por requisição** no `proxy.ts` — `script-src` com nonce e `strict-dynamic`, `connect-src` derivado do ambiente (Supabase HTTPS e WSS, e a `api`). Medido antes: `/login` era a única página estática e teria ficado **trancada** (virou dinâmica); 999 atributos `style` exigem `style-src 'unsafe-inline'`; `upgrade-insecure-requests` quebraria o Supabase local. O e2e exige **zero violações de CSP** no console em login, Realtime e busca
- [ ] Rollout da V3
