...
- [ ] **P0-H — demais RPCs fora do budget** — `get_sku_sales_baseline`, `get_sku_timeline` e as consultas da Central de Notificações. Depois de D-181, `get_sales_expanded_summary` (700 ms), `get_sku_abc_curve` (417 ms), `get_unlinked_listings` (460 ms) e `get_stock_coverage` (314 ms) estão dentro do budget — sem "antes" medido, não se reivindica ganho, apenas se retira a suspeita. Reproduzir cada uma antes de concluir que está lenta: `pg_stat_statements` mistura versões antigas das funções.
