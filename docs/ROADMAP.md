...
- [ ] **Falha "permanente" não é permanente** (achado em D-201) — `job-outcome.ts` mapeia falha definitiva para HTTP 422 acreditando que "4xx a descarta sem repetir". **O Cloud Tasks reentrega qualquer não-2xx.** Medido: 535 job_ids reentregues, **2.234 execuções extras em 7 dias**, até 8 entregas do mesmo job (= o `--max-attempts 8` das filas). Uma pergunta apagada no ML é buscada 8 vezes mesmo classificada corretamente como `not_retryable`. Correção: devolver **200** em falha definitiva e ler `X-CloudTasks-TaskRetryCount` para `job_runs.attempt` — que hoje marca **1** nas 2.234. Muda o contrato HTTP do worker com a fila, então é fatia própria, e só tem efeito depois do deploy.
- [ ] **Espalhar a rajada do snapshot de visitas** (achado em D-201) — ~2.700 requisições de item às 10:00 UTC, 429 diário. `docs/ROADMAP.md` dizia que essa sincronização estava validada com "items_failed: 0"; era verdade em 25/08 e deixou de ser em 27/08.
