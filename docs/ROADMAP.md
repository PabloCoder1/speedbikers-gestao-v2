...
- [ ] Ingestão read-only (perguntas, mensagens, reclamações, devoluções, mediações) — **perguntas cobertas pelo webhook (D-087/D-088); falta a reconciliação por busca**, mais mensagens, reclamações, devoluções e mediações
...
**Falta o deploy do Cloud Run**: `api` e `worker` precisam ser publicados manualmente (`infra/deploy-cloud-run.sh`) para o caminho existir em produção. Até lá, o código está correto e testado, mas nenhuma pergunta real é ingerida.
