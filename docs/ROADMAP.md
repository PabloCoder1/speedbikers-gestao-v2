...
- [ ] Revisão de segurança e de secrets — **fatia 1 em 2026-09-13 (D-328)**: **nenhum segredo real** na árvore nem nos 740 commits do histórico (varredura por formato E por nome de variável; os 27 valores achados são fixture, padrão local da CLI ou chave publicável), e o `pnpm audit --prod` foi de **2 críticas + 1 alta + 4 moderadas para 1 moderada** (`next` 16.3.5, `sharp` 0.35.4, `hono` 4.13.7; o `uuid` residual é inalcançável e ficou aceito com o motivo). E uma decisão de fluxo sem dono: guarda de `audit` na CI reprova pela data, não pelo código. **Fatia 2 (D-329):** a superfície de entrada conferida **ao vivo** — worker com um único invoker (a SA do Cloud Tasks) e **403** sem credencial; `/internal/*` com OIDC de audience e e-mail fechados; CORS só para `WEB_ORIGINS`; **D-045 fechada com tráfego real** (≥ 5.000 webhooks aceitos em 24 h, 0 rejeitados, 0 `X-Forwarded-For` ilegível). E a `web`, que não tinha **nenhum** cabeçalho de segurança além do HSTS da Vercel, ganhou `frame-ancestors 'none'`, `X-Frame-Options`, `nosniff`, `Referrer-Policy` e `Permissions-Policy`, com e2e. **Segue aberto:** CSP completa com nonce, redação de log fora das telas, e confirmar os cabeçalhos na Vercel depois do próximo deploy da `web`
- [ ] Rollout da V3
