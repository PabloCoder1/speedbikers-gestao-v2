...
- [ ] Revisão de segurança e de secrets — **fatia 1 em 2026-09-13 (D-328)**: **nenhum segredo real** na árvore nem nos 740 commits do histórico (varredura por formato E por nome de variável; os 27 valores achados são fixture, padrão local da CLI ou chave publicável), e o `pnpm audit --prod` foi de **2 críticas + 1 alta + 4 moderadas para 1 moderada** (`next` 16.3.5, `sharp` 0.35.4, `hono` 4.13.7; o `uuid` residual é inalcançável e ficou aceito com o motivo). **Segue aberto:** autenticação dos endpoints internos e do webhook (D-045), CORS, cabeçalhos de segurança da `web` e redação de log fora das telas. E uma decisão de fluxo sem dono: guarda de `audit` na CI reprova pela data, não pelo código
- [ ] Rollout da V3
