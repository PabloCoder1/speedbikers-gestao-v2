...
- [ ] Ingestão read-only (perguntas, mensagens, reclamações, devoluções, mediações) — **perguntas cobertas por completo (webhook D-088 + reconciliação D-089)**; faltam mensagens, reclamações, devoluções e mediações
...
- [ ] Triagem interna (assumir, mudar status, resolver/reabrir) — RPC transacional que atualiza o case e acrescenta `support_case_events` na mesma transação (D-084). A Caixa de Entrada de D-090 é só leitura
