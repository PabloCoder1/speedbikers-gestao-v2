...
- [ ] **Remedir o custo do Realtime** — primeira leitura feita 36 min depois de D-199 e ela **NÃO confirma a hipótese**: as escritas de métrica foram a zero e o custo por chamada do Realtime caiu só **8,5%** (6,34 → 5,80 ms). Longe de explicar os 43,4% do tempo do banco medidos em D-198. A percentagem *subiu* para 68,8%, mas isso é o denominador encolhendo — a métrica robusta é **ms por chamada**, não percentagem. Suspeita que sobra: a frequência de polling do decodificador, que dobrou entre as janelas sem relação com a escrita da aplicação. Precisa de janela maior antes de concluir.
