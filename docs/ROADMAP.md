...
- [ ] **Lote de ESCRITA por página** — os três pré-requisitos que o bloqueavam foram resolvidos ou têm solução conhecida: ~~(1) a janela `delete`+`insert`~~ **resolvida em D-189** (virou `upsert` + exclusão da cauda, que fecha a janela em vez de multiplicá-la); ~~(3) o raio de alcance em `nfe-import-apply`~~ **resolvido em D-187**; resta **(2)** o trigger `AFTER INSERT FOR EACH ROW` segurando N travas de saldo em ordem arbitrária — **a solução é ordenar o lote por `sku_id`**, para que todo lote concorrente adquira as travas na mesma ordem (um lote contra um insert de uma linha não fecha ciclo). Com isso, o lote passa a ser trabalho de implementação, não de investigação.
