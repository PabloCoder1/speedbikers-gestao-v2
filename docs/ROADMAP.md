...
- [ ] **Atos externos** (não são código): branch protection da `v3` — **medida de novo em 2026-09-13 (D-334): `protected: true`, mas só contra force-push e exclusão**; nenhum check obrigatório (`required_status_checks` nulo), então a CI **segue não sendo tecnicamente obrigatória** — e `Leaked Password Protection` no Auth do Supabase.
