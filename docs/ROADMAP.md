...
- [ ] **Projeções do PostgREST que ainda não têm portão.** `packages/db/src/projections.ts` criou o lugar e o padrão (constante versionada + teste na pista de integração que importa a constante, nunca copia). Vale varrer os outros `select=` com embed do repo e trazer para lá os que a suíte de unidade não consegue provar.
...
- [ ] **Lote de ESCRITA por página** — deliberadamente NÃO feito em D-186, e não é "só mais um `in (...)`". Três razões medidas, todas a resolver antes: (1) `order_items` é `delete` + `insert` sem transação entre chamadas — em lote, um insert que falha depois do delete deixa **50** pedidos sem itens em vez de um, multiplicando por 50 a janela que D-184 fechou; (2) `stock_movements_apply_to_balance` é `AFTER INSERT FOR EACH ROW` com `DO UPDATE` em `inventory_balances` — 36 linhas num statement seguram 36 travas de saldo em ordem arbitrária, classe de deadlock que hoje não existe; ~~(3) `recordStockMovements` é compartilhada com `nfe-import-apply`, que marca a nota `APPLIED` incondicionalmente depois~~ — **resolvido em D-187**: a gravação passou a abortar, então a nota não é mais marcada sem os movimentos. Sobram (1) e (2).
