...
- [ ] Testes de carga e revisão de `pg_stat_statements` — **a carga REAL foi medida em 2026-09-13 (D-339)**, nos logs do Cloud Run: 65.801 webhooks em 24 h, pico de 1.050 por minuto, e o ACK passando de **7 s nos picos** contra a regra de 500 ms do Mercado Livre. A causa — tópico sem consumidor consultando o Postgres antes do ACK — foi atacada em D-339 e **revertida em D-340**: responder antes da consulta tirou o I/O do sem consumidor, mas esfriou as conexões e levou os ACKs lentos de `orders_v2` de ~1% a ~9%. O pico continua aberto. Falta: a revisão de `pg_stat_statements` do Dev (`report:health` exige a senha do banco — ato humano) e decidir se as telas pedem teste sintético de concorrência (o tráfego de usuário na `api` é de 89 chamadas em 7 dias; as telas leem o Supabase direto)
