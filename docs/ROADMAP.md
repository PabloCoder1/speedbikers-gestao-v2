...
- [ ] **Lote por página de pedidos** em `fetchOrdersWindow` (6,79 → ~0,25 idas por pedido) — **o topo do P1 depois de D-185**. Como o custo é *por chamada* e quase independente do que a chamada faz, cortar 96% das idas corta ~96% do tempo, e isso vale seja qual for o transporte. Commit próprio: muda a semântica de falha parcial, o significado de `assertWritten` com 50 pedidos juntos, e o caminho de cancelamento.
