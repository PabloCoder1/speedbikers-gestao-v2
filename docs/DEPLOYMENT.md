...
4. **Acompanhar a varredura até terminar.** Cada rodada lê no máximo **800 envios por conta**; o backlog medido em 17/09 era de 2.550 pedidos somando as contas, ou seja, no pior caso (todos numa conta só) 4 rodadas — um dia na cadência de 6 h. Para acelerar, dispare à mão — `gcloud scheduler jobs run v3-order-logistics-sweep --project <projeto> --location southamerica-east1` — no máximo **uma vez por hora**: a chave de dedupe é por hora cheia, e o redisparo na mesma hora é descartado. O que ler é o log `sync_order_logistics_done` de cada conta: `restantes` = `sem_envio` + `falhas` + `adiados`. **Terminou quando `adiados` e `falhas` são 0 na última rodada de todas as contas.** `sem_envio` não cai: é pedido sem `shipping_id`, que nunca terá sinal e continua baixando a loja (o lado conservador, D-352 §6). A prova no banco é a mesma que a compensação imprime no primeiro NOTICE:
   ...
   Deve dar só os pedidos sem envio da última rodada. Não use o `done` do job como prova: ele sai `done` com a rodada vazia (D-350).
