...
**GitHub Actions** (`.github/workflows/ci.yml`), obrigatório antes de entrar na `main`:

```text
typecheck -> lint -> testes unitários -> testes de integração -> build -> aplicar migrations no Dev
```

- Migrations aplicadas **por CI** (`supabase db push`, job `migrations`), nunca à mão, nunca pelo dashboard — só em push na `main`, depois de `check`/`scripts`/`integration` verdes. **Em produção**, só por `.github/workflows/migrations-producao.yml`, disparado à mão na `main`, com duas aprovações no ambiente `producao` (D-334; primeira execução em 2026-09-14).
...
2. **Projeto no Supabase**, em `sa-east-1`, com as migrations aplicadas por **`.github/workflows/migrations-producao.yml`** (D-334) — nunca `db push` do próprio computador, que não deixa registro nem sabe se o commit passou pela CI:
   - **Ambiente no GitHub**: Settings → Environments → `producao`, com *Required reviewers* (pelo menos um) e *Deployment branches* só na `main`. Sem as duas travas o workflow recusa — e é a restrição de branch, não o arquivo do workflow, que impede um workflow alterado noutra branch de receber os segredos.
   ...
   - **Disparo**: Actions → *Migrations de produção* → *Run workflow* na `main`, digitando o ref de produção. `origem` recusa se a CI do mesmo commit não passou ou não aplicou no Dev; `plano` pede a primeira aprovação e mostra `migration list` e `db push --dry-run`; `aplicar` pede a segunda e aplica. O workflow só aparece em Actions porque a `main` é a **branch padrão** do repositório (D-335): o GitHub só oferece disparo manual para workflow que exista na branch padrão. **Conferido no primeiro disparo** (2026-09-14, execução 34857929771): o GitHub pediu as duas aprovações em separado.
...
10. **Conferir antes do corte**: `GET /health` da `api` devolvendo o commit publicado; `check:restore` depois do primeiro backup (8.1); a CSP com nonce e zero violações em `/login` (D-331); e um job de cada tipo disparado à mão uma vez (seção 7), **na ordem das dependências** — anúncios, depois vínculos (a planilha do UpSeller), depois Full —, conferindo `processed` e a tabela alimentada, não o 200 do agendador. **Antes de importar a planilha num banco que já tem o histórico de pedidos**, leia D-350 §5: pedido antigo atualizado depois da captura baixa estoque que o UpSeller já tinha descontado.
    ...
    **A guarda de D-351 tem ordem, e cada passo depende do anterior. Não importe planilha do UpSeller em produção antes da F3 (4).** (0) As migrations pendentes de outras sessões com versão menor vão **antes** das da D-351: `20260916165243` (D-363) e `20260916170000` (D-361) — medido em 2026-09-16 18:05 UTC, as duas já estão aplicadas no Dev e em produção (176 versões, md5 da lista `3529e237…` nos dois). Migration de outra sessão que chegar à `v3` depois da D-351 com versão menor que `20260916180400` precisa ser **renomeada antes do merge**: o `db push` sem `--include-all` recusa versão anterior à última aplicada (D-351 §11); (1) as cinco migrations `20260916180000`–`20260916180400` pelo caminho de sempre (Dev pela CI, produção por `migrations-producao.yml`) — **antes** do worker, porque o worker novo grava `source = 'backfill'`, `ESTORNO_PRE_CAPTURA` e `ESTORNO_REVERSAO_EXCEDENTE`, que o CHECK antigo recusa e o flush da página aborta, e lê `get_order_return_movements` com o `occurred_at`. No Dev o `UPDATE` do corte não toca nada (organização reconciliada, D-351 §3); **(2) conferir o corte ANTES de publicar o worker**, e não importar planilha nenhuma entre a migration e o deploy: `select count(*) from public.erp_stock_snapshots s join public.erp_import_batches b on b.id = s.batch_id where b.kind = 'STOCK' and b.parsed_at is not null and s.captured_at = b.parsed_at and private.erp_stock_export_instant(b.file_name, b.parsed_at) <> b.parsed_at and not exists (select 1 from public.stock_movements a where a.organization_id = s.organization_id and a.movement_type = 'AJUSTE_RECONCILIACAO')` deve dar 0. Se der mais, uma planilha entrou pelo worker antigo com o corte do parse: rode de novo o `UPDATE` de `20260916180000_erp_corte_da_exportacao.sql` e repita a conferência até dar 0 — **só então** o deploy do worker. **Qualquer planilha importada antes da F3 torna a organização de produção inelegível** (o critério pede o primeiro movimento do ledger, 2026-09-14 18:43:59 UTC, depois do corte − 1 h, e o corte de uma planilha nova fica muito depois dele), e a F3 vira no-op: só o NOTICE "fora do criterio", nenhum estorno e nenhuma reposição — compensar nesse caso pede decisão do dono. Com o corte do parse ainda carimbado ela aborta, em vez de pular em silêncio (D-351 §11); (3) `worker` e `api` por `deploy-cloud-run.sh`; (4) **só então** a compensação `packages/db/scripts/compensacao-estorno-pre-captura-d351.sql`, **por psql** (`psql "$URL" -v ON_ERROR_STOP=1 -f ...`: o SQL Editor não mostra os NOTICE com o que entrou e com a organização que ficou de fora), com `v3-reconcile-balances` ainda pausado — ela não é migration de propósito, porque o worker antigo continuaria gravando venda sem par depois dela. Ela estorna o `VENDA_ML` sem par pela venda inteira (2.698 estornos em 2026-09-16 18:08 UTC), anula a reversão a mais do legado — cancelamento E devolução da mesma venda — com a data da reversão (20 anulações de 18 pedidos, 3 delas do KIT 2000017792822486, com a venda fora do alvo; D-351 §12), e repõe (venda + estorno + cancelamento) a venda anterior à planilha que o worker antigo nunca gravou e que cancelou depois dela (34 pedidos em 2026-09-15). **Aborta** se ainda houver snapshot carimbado com o corte do parse numa organização sem `AJUSTE_RECONCILIACAO`, elegível ou não; (5) a prova do próprio arquivo repetida depois de 1 h e de 24 h; (6) despausar a reconciliação é decisão do dono (D-350 §5). Retomar o Dev pede outra decisão: o legado de cancelamento E devolução dele (563 vendas) não é compensado (D-351 §6). O corte do parse que o Dev mantém não impede publicar este worker lá: a venda entre a exportação da planilha do Dev e o parse não é estornada (D-351 §10). O resíduo são os pedidos pagos dessa janela ainda sem `VENDA_ML` (4 em 2026-09-15): a venda entra com `occurred_at` antes do corte, fora do alvo, e a reconciliação a desfaz com +1 por unidade até a planilha seguinte.

### 8.3 Publicar a D-352 (a venda entregue pelo Full não baixa a loja)

**Pré-condição: a D-351 publicada e a F3 dela aplicada** (17/09, D-351 §13). A D-352 se apoia nela: a venda que já tem `ESTORNO_PRE_CAPTURA` não ganha um segundo estorno (a chave é a mesma), e a compensação daqui lê o que a F3 de lá gravou. Cada passo abaixo depende do anterior.

1. **A migration `20260918000000_estorno_full_e_logistica_do_pedido`** pelo caminho de sempre (Dev pela CI depois do merge, produção por `migrations-producao.yml`, D-334) — **antes** do worker. O worker novo grava `ESTORNO_FULL`, que o CHECK antigo recusa (o flush da página aborta), e escreve `orders.logistic_type`/`logistic_captured_at`, que não existem sem ela. O caminho inverso é seguro: o worker antigo ignora as duas colunas e nunca grava o tipo novo. **Versão:** se uma migration de outra sessão com versão MAIOR que `20260918000000` chegar à produção antes desta, renomeie esta antes do merge — o `db push` sem `--include-all` recusa versão menor que a última aplicada (D-351 §11).
2. **`worker` e `api`** por `bash infra/deploy-cloud-run.sh` (o worker primeiro, D-088). O worker registra `sync.order-logistics`; a `api` expõe `/internal/schedule/order-logistics`. A partir daqui a venda nova do Full já sai com o par, e só o passado espera.
3. **`bash infra/cloud-scheduler.sh`**, que cria `v3-order-logistics-sweep` (`40 */6 * * *`). Sem o agendador a varredura nunca roda e a pendência da R2 é permanente: o pedido antigo não volta à janela de `sync.orders.window`.
4. **Acompanhar a varredura até terminar.** Cada rodada lê no máximo **800 envios por conta** (e no máximo 7 min de rede); o universo é todo pedido com `VENDA_ML` ainda sem sinal — a venda sem estorno E a estornada pela D-351, que também precisa do sinal para o cancelamento dela não voltar +1 à loja (D-352 §7). Em 17/09 eram 5.470 linhas de `VENDA_ML` no ledger, ~5,3 mil pedidos: no pior caso (todos numa conta só) ~7 rodadas — dois dias na cadência de 6 h. Para acelerar, dispare à mão — `gcloud scheduler jobs run v3-order-logistics-sweep --project <projeto> --location southamerica-east1` — no máximo **uma vez por hora**: a chave de dedupe é por hora cheia, e o redisparo na mesma hora é descartado. O que ler é o log `sync_order_logistics_done` de cada conta: `restantes` = `sem_envio` + `falhas` + `adiados` + `concorrentes`. **Terminou quando `adiados` é 0 e `falhas` não cai mais na última rodada de todas as contas.** `sem_envio` não cai: é pedido sem `shipping_id`, que nunca terá sinal e continua baixando a loja (o lado conservador, D-352 §6); `falhas` que sobram são envios recusados com 400/403 (idem). `envio_inexistente` é o 404, já carimbado como resposta. Rodada com `interrompida: true` parou num 429/5xx/401 e a fila a repete com backoff — o que ela gravou fica. A prova no banco é a mesma que a compensação imprime no primeiro NOTICE:

   ```sql
   select count(distinct m.source_id)
   from public.stock_movements m
   join public.orders o on o.organization_id = m.organization_id
    and o.id = case when m.source_id ~ '^[0-9]{1,18}$' then m.source_id::bigint end
   where m.movement_type = 'VENDA_ML' and m.source_type = 'ORDER'
     and o.logistic_captured_at is null
     and not exists (select 1 from public.stock_movements e
                     where e.idempotency_key = 'estorno:' || m.idempotency_key
                       and e.movement_type = 'ESTORNO_FULL');
   ```

   Deve dar só os pedidos sem envio e as `falhas` da última rodada. A versão desta consulta de antes da revisão de 6965b0e (sem o filtro de `ESTORNO_FULL`) deixava de fora a venda estornada pela D-351 e dava zero cedo demais. Não use o `done` do job como prova: ele sai `done` com a rodada vazia (D-350).
5. **Só então a compensação** `packages/db/scripts/compensacao-estorno-full-d352.sql`, **por psql** (`psql "$URL" -v ON_ERROR_STOP=1 -f ...`: o SQL Editor não mostra os NOTICE), com `v3-reconcile-balances` **ainda pausado** — a organização que já reconciliou não é compensada, e despausar antes tiraria a de produção da lista. Ela não é migration de propósito: precisa do sinal que a varredura grava. Ler os NOTICE: quantos pedidos continuam sem sinal (esperado: só os sem envio), quantos `ESTORNO_FULL` e quantas anulações entraram, alguma organização inelegível (esperado: nenhuma) e as linhas de `RESIDUO` (a anulação parcial da D-351 §12 que o UNIQUE impede completar — decisão do dono). Ela **aborta** se o ledger divergir de `inventory_balances` ou se algum pedido do Full ficar com soma LOCAL diferente de zero.
6. **As provas do próprio arquivo** (rodapé dele), repetidas depois de 1 h e de 24 h: reexecutar grava 0 e 0, e o alvo LOCAL negativo cai para os SKUs que não vêm do Full (os 3 de D-351 §13: 24008, 12152E e BFE-PTA). Pedido que recebeu o sinal depois da primeira execução aparece aqui — rode o arquivo de novo.
7. **Despausar `v3-reconcile-balances` continua sendo decisão do dono** (D-350 §5), agora depois da compensação da D-352. **Com a varredura zerada antes:** o worker não tem a guarda de organização reconciliada que a compensação tem, e um `ESTORNO_FULL` de venda anterior ao corte, gravado DEPOIS de uma reconciliação, deixaria o saldo alto até a rodada seguinte desfazê-lo (D-352 §6).

**Voltar:** o worker e a `api` voltam para a revisão anterior como sempre. A migration é aditiva e fica; os `ESTORNO_FULL` já gravados ficam (o ledger é append-only) e continuam certos — o worker antigo só volta a baixar a loja na venda nova do Full.
