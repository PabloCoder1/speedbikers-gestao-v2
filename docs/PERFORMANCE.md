...
**Depois, no ensaio do Dev** (transação desfeita, como `authenticated`): mesmo JSON, **154–159 ms contra 302–328 ms** com o banco quente, 14.473 vendas. A leitura passa a ser ~1 mil páginas em sequência nos 90 dias de produção. Sem `analyze` depois do preenchimento, o planejador errou e a chamada passou de 3,5 min -- por isso ele está na migration. O número de produção a frio fica para depois da migration.
