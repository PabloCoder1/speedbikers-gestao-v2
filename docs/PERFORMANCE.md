...
⚠️ **`force_custom_plan` não é remédio universal** (D-324). Nas escritas em lote
por array de ids (`set_skus_supplier_brand`, `set_skus_stock_virtual`, 300 ids),
o plano **genérico foi mais rápido** que o custom em duas rodadas alternadas —
14–18 ms contra 22–27 ms numa, 17–20 contra 22–30 na outra: o custom replaneja a
cada chamada, e para busca por chave o plano é o mesmo. A cura de D-305/D-307/
D-319 vale para leitura parametrizada que agrega e ordena; aplicada por padrão,
ela piora escrita.

⚠️ **Compare os modos cada um na SUA transação, e alternados.** Numa transação
só, reescrever as mesmas linhas acumula versões mortas: o tempo sobe aos poucos,
e o bloco que roda por último paga o inchaço todo — foi assim que a primeira
medição de D-324 fez o custom parecer 50% mais lento pelo motivo errado. Três
blocos por função: oito execuções no padrão (olhando a sexta),
`force_generic_plan` e `force_custom_plan`.
...
**A rajada grande — o pico fechou** (14/09, 13:49:49–13:50:20 UTC, `api-00040-qrk`): 1.737 webhooks em 32 s, **319 no mesmo segundo** (quase o triplo da de D-345) e **1.283 no minuto** (acima do maior minuto de D-339, 1.050). `lookup_ms` **0 ms** nas 1.762 notificações da rajada; ACK **p50 2 · p95 3 · max 190 ms**, nenhum acima de 500 ms; uma instância só, sem escalar. Na janela de 13:35 a 14:00 UTC (4.878 webhooks), max 454 ms e nenhum acima de 500 ms; no `worker`, nenhum ERROR e `slow_operation` na faixa de antes da rajada. O maior `lookup_ms` foi a recarga de 5 minutos das 13:56 (268 ms). **Ainda não visto:** a duração de oito minutos das rajadas de fim de semana. Tabela em D-346, seção 6.
...
### Alertas da central em `actions` (D-403, 24/09/2026)
...
**O detector em produção, como `authenticated`, com os 90 dias completos** (versão de D-397, oito execuções seguidas numa transação desfeita): **4.383 ms** na primeira, **1.190–1.231 ms** nas sete seguintes, sem salto na sexta. 745 linhas analisadas, 665 com histórico. Com 3 semanas de frete eram 0,51 s como `postgres`: o custo cresce com os pedidos da janela (três vezes mais), como o de `get_faturamento` (~0,44 s em 30 dias). A tela e o painel da central carregam por streaming; se passar a incomodar, o caminho é um resumo diário por anúncio e faixa, não mexer nos sinais.

### O detector de frete lê `shipping_sales` (D-409, 25/09/2026)

**O que custava, em produção** (`EXPLAIN (ANALYZE, BUFFERS)` só de leitura, a junção de `orders`, `order_financials` e `order_items` dos 90 dias): 78.208 pedidos, 77.143 linhas elegíveis, **626 ms com tudo em cache e 387.963 páginas tocadas** -- 74 mil páginas de `orders` para 79 mil linhas e 313 mil de `order_items`. A frio: o detector fez 4,4 s na primeira chamada (D-399) e 9,7 s dentro da sincronização dos alertas das 8h de 25/09, acima do `statement_timeout` de 8 s.

**Depois, no ensaio do Dev** (transação desfeita, como `authenticated`): mesmo JSON, **154–159 ms contra 302–328 ms** com o banco quente, 14.473 vendas. A leitura passa a ser ~1 mil páginas em sequência nos 90 dias de produção. Sem `analyze` depois do preenchimento, o planejador errou e a chamada passou de 3,5 min -- por isso ele está na migration. **Em produção, logo depois da migration (25/09, como `authenticated`):** 77.962 vendas na tabela, zero diferenças contra a regra aplicada às três tabelas; **1.011 ms** na primeira chamada e **779–781 ms** nas seguintes, contra 4.383 ms e 1.190–1.231 ms antes (D-399). A primeira não foi fria de verdade -- o preenchimento acabara de ler essas páginas; a prova a frio é a rodada das 8h.
