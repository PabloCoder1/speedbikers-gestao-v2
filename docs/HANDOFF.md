...
| **HEAD conhecido** | `8beb58e` (D-277, **D37a** — as três telas de detalhe). A fila NOMEADA da frente visual fechou em D36; D37 começou |
...
| **Frente atual** | **Frente visual, em D37 (o passe global)**. A fila NOMEADA fechou em D36; **D37a entregou as três telas de DETALHE** (`/notas-fiscais/[id]`, `/compras/[id]`, `/fornecedores/[supplierId]`) e extraiu `ProcessSteps` no segundo consumidor (D-277). `check:table-styles` foi de **21 para 24**; **restam 6** arquivos com `<table>` e nenhum `.sb-table` — `/cobertura` (nunca migrada, D-261), `/importacoes` e `/importacoes/[id]` (terceiro candidato a `ProcessSteps`), `/compras/novo`, `/notificacoes/preferencias` e `/reposicao/configuracoes` — mais 16 `page.tsx` sem `PageTitle`. ⚠️ O guarda cobre **uma metade só**: tela que nunca declarou `.sb-table` depende da captura. Trilha 8B com P0 fechado (A–H) e em P1 |
