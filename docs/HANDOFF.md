...
| **HEAD conhecido** | `06f5f7f` (D-256, D20) — D18→D20 e D-254 no git. A **migration foi aplicada e conferida**; o resto da CI estava em execução quando isto foi escrito |
...
| **Migrations** | ⚠️ **148 locais, 148 no Dev** — `20260906140000_purchase_orders_list.sql` (D-255) ESTÁ aplicada e conferida no banco (função existe, `anon` não executa, `security invoker`), **mas NÃO foi a CI que aplicou**: o job `aplicar migrations no Supabase Dev` ficou `skipped` porque `integration` e `e2e` falharam. Quem aplicou foi a **integração GitHub do próprio Supabase** (branch com `git_branch: v3`, criada em 2026-09-03). **A trava "só depois da esteira verde" não existe na prática** — ver D-257. Antes dela: **131 locais, 130 no Dev** — o expurgo (`20260903120000`) está no git e **não pousou**; a CI não o aplicou, sem drift — D-209→D-212 aplicadas pela CI em 2026-09-02 e CONFERIDAS lá (`anon` alcança 0 funções; `ml_accounts` sem UPDATE/DELETE para `authenticated`; `created_by` presente). O caminho é o push, **nunca** o MCP (lição de D-207) |
