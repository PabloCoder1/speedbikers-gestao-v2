...
| **HEAD conhecido** | `df938ba` (D-331) — **a CSP completa com nonce, e com ela a revisão de segurança FECHOU** (um bloqueador a menos: 5). `/login` era a única página estática e teria ficado **trancada** sob `strict-dynamic` — virou dinâmica. `style-src` fica `'unsafe-inline'` (999 atributos `style`), `connect-src` sai do ambiente (Supabase HTTPS e WSS, e a `api`). O e2e exige zero violações de CSP no console, o detector foi **provado** contra o build, e a política está **confirmada no ar na Vercel** (`/login` pública: nonce variando, 11/11 scripts com nonce, 0 violações). ⚠️ **Toda página nova precisa ser dinâmica** — estática sai sem nonce e quebra em silêncio. Antes: `9ad7432` (D-330) redação de log por valor · `d94aba7` (D-329) superfície de entrada ao vivo e cabeçalhos · `d7114bd` (D-328) segredos e dependências. ⚠️ **Armadilha nova em `TESTING.md`:** depois de reiniciar a máquina o Docker Desktop pode não subir (socket `.stale` em `%LOCALAPPDATA%\Docker\run`) — **nunca "Reset to factory defaults"**, que apaga os volumes; mova a pasta `run` inteira. Antes: `f9b59cd` (D-327) legenda do gráfico no cabeçalho, e a frente de desenho sem item acionável. ⚠️ **Duas armadilhas de ambiente em `TESTING.md`, e cada uma custou uma rodada inteira:** `.env.local` aponta para o **Dev** e `NEXT_PUBLIC_*` é embutida no build (exporte as variáveis locais ANTES do `build`, senão o e2e reprova no login); e o OneDrive marca o `.next` como somente-leitura, e o `next build` morre em `EPERM` sem gerar build. Antes: `6e0b80f` (D-326) · `b161db6` (D-325) · `6dd7bb9` (D-324) · `df1a4b9` (D-323) busca do shell, e a fila de D-310 fechada · `8612111` (D-322) · `e263412` (D-321) · `c330ae4` (D-320) — **a lentidão de `/produtos` não era o tamanho da página.** `get_sku_curation` é `plpgsql` e não tinha `plan_cache_mode`: da **sexta execução da conexão** em diante o plano vira genérico e a chamada salta de ~20 ms para **360 a 4.000 ms** (a magnitude varia com a estatística; o salto, não). Com o genérico no lugar, 20/50/100/300 por página medem o mesmo — a troca de 100 para 50 de D-315 não acelerou nada, e agora isso está medido. Curado com uma linha, como D-305 e D-307. ⚠️ **Ficou aberta a varredura das outras `plpgsql`:** a de D-307 usou o critério de `language sql` (estado estável desproporcional), que não vale aqui — em `plpgsql` o sinal é a sexta execução. Antes: `2eb0a7a` (D-317) diagnóstico do SKU com régua · `571da6f` (D-316) vínculo dentro do SKU · `08900ff` (D-315) opções do UpSeller em `/produtos`. |
| **Fechamento da V3** | **186 de 213 itens do ROADMAP fechados (87%)** — 25 abertos e 2 parciais. Dos 25, **5 são bloqueadores**, e todos são hardening/lançamento: nenhum é feature faltando (D-223). O sexto, a revisão de segurança, fechou em D-328 → D-331 |
