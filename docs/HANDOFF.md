...
| **HEAD conhecido** | `1dd0638` (D-319) — **a lentidão de `/produtos` não era o tamanho da página.** `get_sku_curation` é `plpgsql` e não tinha `plan_cache_mode`: da **sexta execução da conexão** em diante o plano vira genérico e a chamada salta de ~20 ms para **360 a 4.000 ms** (a magnitude varia com a estatística; o salto, não). Com o genérico no lugar, 20/50/100/300 por página medem o mesmo — a troca de 100 para 50 de D-315 não acelerou nada, e agora isso está medido. Curado com uma linha, como D-305 e D-307. ⚠️ **Ficou aberta a varredura das outras `plpgsql`:** a de D-307 usou o critério de `language sql` (estado estável desproporcional), que não vale aqui — em `plpgsql` o sinal é a sexta execução. Antes: `2eb0a7a` (D-317) — **o diagnóstico do SKU passou a julgar com régua**: três níveis (ok/atenção/crítico, escolha do dono), e cada selo mostra na tela a condição que o acendeu. O que não tem régua **não vira selo**: a dispersão de preço sai como número e o painel "o que esta tela NÃO julga" lista o resto (divergência de estoque por conta não é calculável — o interno não tem coluna de conta em ponto nenhum da cadeia). Antes: `571da6f` (D-316) — gerir vínculo dentro do SKU. Antes: `08900ff` (D-315, outra sessão) — as opções do UpSeller em `/produtos`. |
...
| **Frente atual** | **O pedido do dono sobre o Dashboard do SKU está entregue nas duas metades** (D-316 vínculos, D-317 diagnóstico). O que ficou registrado como SEM FONTE, e é o que impede a próxima pessoa de prometer: Shopee e a dimensão plataforma (D-037), divergência de estoque por conta (o interno é da organização, não há coluna de conta), tipo de anúncio, catálogo, logística, qualidade da publicação e substatus de pausa. **Uma régua espera decisão do dono:** teto de dispersão de preço — sem ele o número aparece sem selo, e com ele vira selo sem mexer na tela. Da auditoria de design, a fila: o painel "Últimas decisões" do SKU, os cartões da gaveta do pedido, a altura do SVG de `/vendas` e o rótulo da busca do shell. **Duas sessões neste repo:** portas 3000/3100 separadas e `db reset` combinado. |
