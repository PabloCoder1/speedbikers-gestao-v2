...
| **Frente atual** | **A frente visual FECHOU.** `check:table-styles` conta **30** telas e **nenhum arquivo do app tem `<table>` sem `.sb-table`**; a única `page.tsx` sem `PageTitle` ou `ObjectHeader` é `/login`, fora do `Shell` de propósito. **O que ficou aberto NÃO é design, é CORREÇÃO:** `get_purchase_suggestions` não classifica **nenhum** SKU (`units_90d = 0` dentro da RPC contra 586 SKUs elegíveis em `daily_sku_metrics`), então `/reposicao` — a superfície de decisão de compra — está muda; é regressão contra a distribuição que D-250 mediu em 05/09, e **nenhum dos 633 testes de integração pegou**, porque nenhum afirma que algum SKU CHEGA a ter estado (D-279). Depois dela: fundir `/cobertura` com `/reposicao` (o frame as trata como uma tela), os drawers adiados desde A1 e os sete itens abertos. Trilha 8B com P0 fechado (A–H) e em P1 |
