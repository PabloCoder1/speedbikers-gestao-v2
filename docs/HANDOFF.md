...
| **HEAD conhecido** | `cef72ee` (D-314) — **"Cobertura" de um SKU passou a ter UMA conta.** O cartão do Dashboard de SKU e a gaveta Inspeção Rápida imprimiam `local ÷ venda média`, a definição que D-288 aposentou: eram **300 dias** contra os **318** de `/reposicao`, para o mesmo SKU. As três superfícies passaram a ler a régua canônica (`aproveitável ÷ taxa de 30 dias`), por um módulo só. Antes: `b7e5ef7` (D-312) — Importações virou tela de ADMIN e mudou de grupo no menu (⚠️ a RLS e as rotas da api ainda autorizam GESTOR). Antes: `e83914e` (D-311) — a Home: o seletor de janela e a idade do FATO no feed. Antes: `fed1fd0` (D-310) — o cabeçalho do anúncio. |
