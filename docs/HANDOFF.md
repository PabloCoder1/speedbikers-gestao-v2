...
| **HEAD conhecido** | `db7890b` (D-333) — **os scripts de `infra/` agora distinguem `AMBIENTE=dev` de `AMBIENTE=prod`**: antes, um deploy com o projeto de produção subiria com o Supabase do Dev, sem aviso. `prod` não tem padrão, exige `CONFIRMO_PRODUCAO=sim` e recusa identificador do Dev (13 casos na CI, provados por mutação). Roteiro de produção em `DEPLOYMENT.md` 8.2, com dois buracos abertos: migrations de produção sem caminho e a Vercel servindo produção com variáveis do Dev. Antes: `536e2ae` (D-332) backup medido e ensaio de restore pronto — **falta o restore real num projeto novo** (ato humano, 8.1). ⚠️ **Toda página nova precisa ser dinâmica** (D-331): estática sai sem nonce e a CSP a bloqueia em silêncio. ⚠️ **Armadilhas de ambiente em `TESTING.md`:** `.env.local` aponta para o Dev (exporte as variáveis locais ANTES do `build`); o OneDrive trava o `.next` em `EPERM`; e o Docker Desktop pode não subir depois de reiniciar — **nunca "Reset to factory defaults"**, que apaga os volumes; mova a pasta `run`. Antes: `df938ba` (D-331) CSP com nonce e a revisão de segurança fechada · `9ad7432` (D-330) · `d94aba7` (D-329) · `d7114bd` (D-328) · `f9b59cd` (D-327) frente de desenho sem item acionável · `6dd7bb9` (D-324) varredura das `plpgsql` · `1dd0638` (D-319). |
