...
> Última atualização: 2026-08-21 — **Gráfico de tendência de receita bruta adicionado ao Dashboard de Vendas** (`get_sales_daily_series`, SVG próprio sem dependência nova), verificado com dado real de produção. Achado e corrigido no caminho: `<title>` de SVG com múltiplos filhos JSX causava divergência de hidratação — corrigido para template string única. Também: **Fase 4 iniciada** — ledger de estoque (`stock_movements`) e projeção `inventory_balances` criados e verificados contra Postgres real (127/127 testes de integração). Full fica fora do ledger (D-018). Modelos de pedido de compra (Excel/PDF) desenhados pela V3 e entregues ao usuário. **Próxima etapa: dedução por venda aplicada na persistência do pedido (`apps/worker/src/handlers/persist-order.ts`) — primeiro código de aplicação a escrever no ledger.**
...
**Gráfico de tendência no Dashboard de Vendas — pedido direto do usuário, ao ver a tela em produção pela primeira vez com dado real.**
