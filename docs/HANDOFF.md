...
| **Frente atual** | **Frente visual**: a fila NOMEADA fechou — D18→D25 e D27→D36 entregues (+A3/A3b). **Próxima: D37, o passe visual global**; D26 (Tráfego) foi avaliada e RECUSADA por falta de fonte (D-266). ⚠️ **`/cobertura` nunca foi migrada** (D-261). O guarda `check:table-styles` (D-262) reprova a migração pela metade e cobre **uma metade só** — tela que nunca declarou `.sb-table` depende da captura. `components/table-styles.ts` foi APAGADO em D-275 — não há mais consumidor. Trilha 8B com P0 fechado (A–H) e em P1 |
