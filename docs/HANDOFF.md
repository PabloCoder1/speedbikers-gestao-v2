...
> Última atualização: 2026-08-22 — **Reversão de estoque por cancelamento concluída (D-052): segundo item do checklist da Fase 4, fecha o ciclo de venda do ledger.** `computeCancellationReversals` (`@sb/domain/inventory`, puro) reverte os movimentos `VENDA_ML` JÁ GRAVADOS no ledger, não recalcula a partir dos itens atuais do pedido — imune a vínculo de SKU trocado entre a venda e o cancelamento. Devolução fica de fora de propósito (API de Reclamações e Devoluções não integrada, mesmo motivo já registrado para `order.returned`). Publicado no Cloud Run como `worker-00011-8bl` (tag `3c299a3`, o commit exato), `worker_started` confirmado no Cloud Logging, zero linha `ERROR`. NF-e/XML segue bloqueada aguardando XML real do usuário (pesquisa do layout oficial concluída, `docs/NFE.md`); enquanto isso, avancei o próximo item **não bloqueado** do checklist da Fase 4, Full por conta: schema criado (migration `20260822122526`) e agora o detector de eventos por diff (`detectFulfillmentEvents`, `@sb/domain/events`, puro). **Próxima etapa: o job de captura em si — cliente ML para `GET /items/{item_id}` + `GET /inventories/{inventory_id}/stock/fulfillment`, enumerando via `sku_listing_links`, gravando `fulfillment_stock_snapshots` e chamando o detector.**
...
- Supabase V3 Dev (`nmgccyqquwxecqffsidr`, `sa-east-1`): migrations aplicadas até `20260822122526_create_fulfillment_stock_snapshots`, `list_migrations` confere local = remoto sem drift (checado antes de cada migration nova, depois do achado de divergência entre sessões em 2026-08-22).
...
**`fulfillment_stock_snapshots` (Full por conta) — schema criado, próximo item não bloqueado do checklist da Fase 4.**
...
- **NF-e/XML (Fase 4, próximo item) bloqueado aguardando arquivo real do usuário.** Pesquisa do layout oficial concluída e registrada em `docs/NFE.md` (estrutura do XML modelo 55/4.00, chave de acesso, `ide/tpNF` para direção entrada/saída, impacto da reforma tributária 2026 no bloco `imposto`). Usuário confirmou que vai enviar XML real (fonte: `meudanfe.com.br`) — schema de `documents`/`document_items` e o parser só devem ser desenhados depois de ver pelo menos um arquivo real, mesmo padrão já usado para as planilhas do UpSeller. `skus` não tem coluna de EAN/GTIN hoje — matching automático por `cEAN` exigiria migration própria, não decidir isso sem necessidade de produto mais ampla.
