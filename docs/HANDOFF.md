...
| **Frente atual** | **A FRENTE VISUAL FECHOU EM ≈ 90% E VIROU LISTA DE ITENS ABERTOS**, e cinco já saíram: **fusão de `/cobertura` com `/reposicao`** (D-288 — uma tela só, com a definição de ruptura da reposição), **paginação de `/atendimento`** (D-289 — 829 dos 929 abertos eram inalcançáveis), **recorte de não lidas em `/notificacoes`** (D-290), **lista de execuções que falharam** (D-291, com migration já aplicada no Dev: `get_job_failures`, `security definer` restrita a ADMIN) e a **exportação de `/precos`** (D-292 — exporta o recorte da tela, com o teto declarado DENTRO do arquivo). **A PRÉ-CONDIÇÃO da gaveta do Copiloto foi paga (D-293):** `/v1/copilot/chat` recebe contexto de tela (`{ kind, id }`, conjunto fechado; o id é dado, nunca autoridade) e ganhou duas ferramentas além de venda — `sku_replenishment`, que usa a MESMA composição de `/reposicao` (`composeSkuReplenishment`, agora em `@sb/domain`), e `listing_performance`. **O que resta:** a gaveta em si. **Medição que vale para qualquer tela paginada:** o **416 `PGRST103`** de `.range()` além do fim só acontece com `count: exact` na MESMA consulta; sem `count` volta 200 vazio, e aí quem detecta é a aritmética. Trilha 8B com P0 fechado (A–H) e em P1 |
