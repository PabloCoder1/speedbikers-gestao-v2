...
| **Deploy no ar** | ✅ **api em `59f49dd`** (`api-00039-9vm`, 2026-09-14 03:01 UTC, D-343), com o log do webhook medindo cada etapa do ACK, e **worker em `b1a547a`** (`worker-00051-thq`, 2026-09-14 09:51 UTC, D-344), com o 404 de devolução de claim recém-nascido deixando de ser falha. Os dois seguem com `hono` 4.13.7 (D-328) e a redação de log por valor (D-330). `GET /health` responde `{"commit":"59f49dd"}`; tráfego no **LATEST** nos dois. ⚠️ **`api-00037-bqb` é o código revertido de D-339** (D-340): **não mande tráfego para ela**, e nunca use `--to-latest` com tráfego fixo sem conferir `latestReadyRevisionName` (D-342). Para voltar: api `api-00038-2hg`; worker `worker-00050-qnt`. **Piso de frescor no ar** (D-304): `v3-refresh-sales-metrics`, `35 * * * *`. **Esta linha envelhece sozinha** (D-070): `/health` só cobre a api — o worker se confere por `gcloud run services describe worker`, comparando a imagem com o `HEAD`. |
