...
| **HEAD conhecido** | `765d8c0` (D-334) — **migration de produção ganhou caminho**: `.github/workflows/migrations-producao.yml`, disparo manual na `v3`, que recusa rodar sem o ambiente `producao` travado (revisor e restrição de branch) ou sem a CI do mesmo commit verde e já aplicada no Dev, e aplica em duas aprovações (24 casos na CI, provados por mutação). **Hoje ele para no primeiro job**: o ambiente `producao` não existe — criá-lo é ato humano (`DEPLOYMENT.md` 8.2, passo 2). Resta a Vercel servindo produção com variáveis do Dev. Antes: `db7890b` (D-333) `AMBIENTE` explícito em `infra/` · `536e2ae` (D-332) ensaio de restore pronto — **falta o restore real num projeto novo** (ato humano, 8.1). ⚠️ **Toda página nova precisa ser dinâmica** (D-331): estática sai sem nonce e a CSP a bloqueia em silêncio. ⚠️ **Armadilhas de ambiente em `TESTING.md`:** `.env.local` aponta para o Dev (exporte as variáveis locais ANTES do `build`); o OneDrive trava o `.next` em `EPERM`; e o Docker Desktop pode não subir depois de reiniciar — **nunca "Reset to factory defaults"**, que apaga os volumes; mova a pasta `run`. Antes: `df938ba` (D-331) CSP com nonce e a revisão de segurança fechada · `9ad7432` (D-330) · `d94aba7` (D-329) · `d7114bd` (D-328) · `f9b59cd` (D-327) frente de desenho sem item acionável · `6dd7bb9` (D-324) varredura das `plpgsql` · `1dd0638` (D-319). |
