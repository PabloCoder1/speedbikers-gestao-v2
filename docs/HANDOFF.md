...
| **Deploy no ar** | ✅ **api e worker em `d828eac`** (`api-00036-5l4`, `worker-00050-qnt`; D-338) — levaram `hono` 4.13.7 (D-328) e a redação de log por valor (D-330). `GET /health` responde `{"commit":"d828eac"}`. ⚠️ **A api em `3ef3ee1` (D-339) foi publicada e VOLTADA em 2026-09-14** (D-340): tirou o I/O do tópico sem consumidor, mas os ACKs acima de 500 ms de `orders_v2` foram de ~1% a ~9%. A revisão `api-00037-bqb` segue existindo sem tráfego — **não mande tráfego para ela**. E o tráfego ficou **fixo** em `api-00036-5l4`: o próximo deploy pode sair verde servindo **0%** — confira `status.traffic` depois. Para voltar do `d828eac`: api `api-00035-6fw`; worker `worker-00049-r62`. **Piso de frescor no ar** (D-304): `v3-refresh-sales-metrics`, `35 * * * *`. **Esta linha envelhece sozinha** (D-070): `/health` só cobre a api — o worker se confere por `gcloud run services describe worker`, comparando a imagem com o `HEAD`. |
...
| load tests e revisão de `pg_stat_statements` | **carga real medida (D-339)**: 65,8 mil webhooks/dia, pico de 1.050/min, e o ACK passa de 7 s nos picos. **A correção de D-339 foi publicada e voltada** (D-340): esfriou as conexões e piorou `orders_v2`. O pico continua aberto — a próxima tentativa começa medindo qual chamada esfria. Falta também a revisão de `pg_stat_statements` (`report:health` pede a senha do Dev — ato humano) |
