...
> Última atualização: 2026-08-21 — **Dedução de estoque por venda concluída: primeiro código de aplicação a escrever em `stock_movements`.** `computeSaleDeductions` (`@sb/domain/inventory`, puro) chamado a cada order persistida; KIT deduz componentes, não o próprio kit; idempotência comprovada por teste. Publicado no Cloud Run como `worker-00010-289` (tag `c083c62`, o commit exato), `worker_started` confirmado no Cloud Logging, zero erro. **Próxima etapa: reversão de estoque por cancelamento/devolução — segundo item do checklist que falta para fechar o ciclo de venda do ledger.**
...
**Dedução de estoque por venda — segundo item do checklist da Fase 4, primeiro código de aplicação a escrever no ledger.**
