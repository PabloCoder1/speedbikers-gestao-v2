...
| **Frente atual** | **A FRENTE VISUAL FECHOU EM ≈ 90% E VIROU LISTA DE ITENS ABERTOS**, e cinco já saíram: **fusão de `/cobertura` com `/reposicao`** (D-288 — uma tela só, com a definição de ruptura da reposição), **paginação de `/atendimento`** (D-289 — 829 dos 929 abertos eram inalcançáveis), **recorte de não lidas em `/notificacoes`** (D-290), **lista de execuções que falharam** (D-291, com migration já aplicada no Dev: `get_job_failures`, `security definer` restrita a ADMIN) e a **exportação de `/precos`** (D-292 — exporta o recorte da tela, com o teto declarado DENTRO do arquivo). **A LISTA DE ITENS ABERTOS FECHOU:** D-293 pagou a pré-condição do Copiloto (contexto de tela na rota, `{ kind, id }` em conjunto fechado, e as ferramentas `sku_replenishment` — que usa a MESMA composição de `/reposicao`, `composeSkuReplenishment`, agora em `@sb/domain` — e `listing_performance`), e **D-294 entregou a gaveta**: o ✦ da barra de topo abre o Copiloto por cima da tela, com o contexto publicado pela própria página. **O que resta da frente visual: nada nomeado** — o próximo passo é escolha de produto, não item de lista. **Medição que vale para qualquer tela paginada:** o **416 `PGRST103`** de `.range()` além do fim só acontece com `count: exact` na MESMA consulta; sem `count` volta 200 vazio, e aí quem detecta é a aritmética. Trilha 8B com P0 fechado (A–H) e em P1 |
