...
| **HEAD conhecido** | `b7e5ef7` (D-312) — **Importações virou tela de ADMIN** e mudou de grupo no menu (OPERAÇÃO → ADMINISTRAÇÃO, ao lado de Sincronização): importar planilha reescreve o catálogo. As três telas recusam **no servidor**, não só no menu. ⚠️ **Pendência registrada:** a RLS de `erp_import_batches`/`rows` e as duas rotas da api (`POST /v1/erp-imports` e `.../apply`) continuam autorizando **GESTOR** — fechar isso é migration + api, e muda quem trabalha, não só quem enxerga. Antes: `e83914e` (D-311) — a Home: o seletor de janela do gráfico e a idade do FATO no feed, com o vocabulário de período mudando de casa para `lib/period.ts`. Antes: `fed1fd0` (D-310) — o cabeçalho do anúncio. Antes: `66ef20d` (D-309) — os seis cartões de `/saude`. |
