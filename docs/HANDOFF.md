...
| **Frente atual** | **A FRENTE VISUAL ESTÁ EM ≈90%, E O QUE FALTA NÃO É DESENHO.** A composição do Figma fechou (D-281/D-282: as cinco gavetas), o passe de campo e botão fechou com guarda (D-284: 194 controles, `check:control-styles`), **o passo cinza foi dado** (D-285: chão `#f4f5fa` no `<main>`, medido na tela em dez rotas — a diferença ESTRUTURAL entre o Figma e a V3 deixou de existir) e **o inbox de três colunas foi decidido** (D-286: recusado com medição — o centro fica vazio em 50,7% das reclamações, que são 63% da base; o que ele protegia virou o parâmetro `?volta=`). A auditoria de render de D22–D36 foi feita (D-283, A4): catorze telas — não dezesseis —, todas 200, bloco de 25% para 88%. Os **quatro P3 de acabamento** fecharam em D-287 (três feitos, um recusado com número: a largura de `/integracoes` ocuparia 24 linhas por célula em metade da largura, contra 4 hoje) — e a captura pegou um defeito que o passo cinza tinha introduzido: sete botões primary ficaram brancos sobre branco, e o guarda ganhou a regra. **O que resta:** **fundir `/cobertura` com `/reposicao`** (exige escolher UMA definição de ruptura — as duas discordam em 185 SKUs) e os sete itens abertos. Trilha 8B com P0 fechado (A–H) e em P1 |
