...
| **HEAD conhecido** | `fed1fd0` (D-310) — **A10 da frente de design**: a próxima fatia foi ESCOLHIDA por medição (sete superfícies lidas contra o frame, 49 achados, rodada adversarial sobre os melhores — sete sobreviveram, um caiu). Nenhuma superfície estava desalinhada na composição principal. A fatia foi o cabeçalho de `/anuncios/[itemId]`: entraram os dois fatos que ele devia desde D-168 (**Preço atual · Disponível**, e `available_quantity` era **select morto** desde D13) e o caminho **"Republicações →"**, que leva ao painel onde os dois atos de D-295 moram — caminho no cabeçalho, ato no painel. Antes: `66ef20d` (D-309) — os seis cartões de serviço de `/saude` remedidos e mantidos fora; entraram o link do cabeçalho e o tempo cronometrado da ida ao `/health`. Antes: `1f18147` — o deploy da api em `8ebf022` com o piso de frescor de D-304 no ar. Antes: `912746c` (D-308) — período e "com/sem venda" em `/anuncios`. |
...
| **Frente atual** | **Design, escolhendo por medição.** A auditoria da Administração fechou em D-309 (duas telas refeitas, quatro com recusa medida) e a varredura de D-310 leu as outras sete superfícies: **28 achados acionáveis** ficaram mapeados, e os que sobreviveram à rodada adversarial e NÃO entraram nesta fatia são a próxima fila — o painel "Últimas decisões" do SKU sem a linha de contexto, os cartões da gaveta do pedido, a altura do SVG de `/vendas` e o rótulo da busca do shell (a RPC já devolve sete entidades e o campo promete cinco). **Produção está em dia:** a api e o Cloud Scheduler de D-304 no ar desde 11/09, e a URL da aplicação configurada nos redirecionamentos do Supabase — os links de acesso de D-303 abrem na aplicação. |
