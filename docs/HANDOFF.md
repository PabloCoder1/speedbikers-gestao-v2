...
| revisão de segurança e de secrets | **PARCIAL** (D-328): segredos no repositório e dependências de produção revistos e corrigidos. Falta autenticação dos endpoints internos e do webhook (D-045), CORS, cabeçalhos da `web` e redação de log fora das telas |
