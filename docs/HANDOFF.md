...
| **HEAD conhecido** | `2b9ea8d` (D-313) — **a tela de vinculação tinha parado de vincular.** D-284, ao trocar `style` inline por classe, apagou junto os campos "Variação" e "SKU de destino" de `manual-link-form.tsx`: sem o campo de SKU o botão "Vincular" nascia `disabled` e nunca saía disso, e nenhum teste viu porque a única asserção sobre o formulário era o heading estar visível. Voltaram também o filtro por conta, a comparação entre contas (D-128, perdida no reduce de D-259) e a coluna "Ação" que pré-preenche o formulário. **Com migration** (`20260911180000`): `get_link_integrity.com_vinculo` ignorava `listings.sku_id` e fazia a comparação dizer 4 sem vínculo enquanto a faixa dizia 3 — duas definições da mesma palavra na mesma tela. Antes: `cef72ee` (D-314) — a "Cobertura" de um SKU passou a ter UMA conta: o cartão do SKU e a gaveta diziam **300 dias** contra os **318** de `/reposicao`, e as três superfícies passaram a ler a régua canônica de D-288. Antes: `b7e5ef7` (D-312) — Importações virou tela de ADMIN (⚠️ a RLS e as rotas da api ainda autorizam GESTOR). |
...
| **Migrations** | **163 locais** — a última é `20260911180000` (D-313). ⚠️ Quem aplica no Dev é a integração GitHub do Supabase, **não** a CI (D-257); o caminho é o push, **nunca** o MCP (D-207). O nome do arquivo precisa ser um **instante válido**, não só um número crescente: `...240000` (hora 24) deixou seis casos de `get_system_health` vermelhos, e é esse teste que serve de guarda (D-307) |
| **Frente atual** | **Duas sessões trabalhando no mesmo repo, cada uma numa frente** (pedido do dono do produto): a Cobertura do SKU (D-314) e `/vinculacoes` (D-313) **fecharam as duas**. A próxima fatia de `/vinculacoes` já está pedida pelo dono do produto: gestão de vínculos DENTRO de `/skus/[skuId]`, reusando as mesmas Server Actions e RPCs de escrita — **sem** um segundo sistema de vinculação. Para ela: não existe vínculo "direto" a desfazer, porque `listings.sku_id` é DERIVADO de `sku_listing_links` pelo handler de sincronização e acompanha sozinho (medido em D-313). Da auditoria de design, a fila que sobrou: o painel "Últimas decisões" do SKU (o `kind`/`recommendation` vêm no mesmo select e não são impressos) e os dois painéis que cortam a lista sem dizer, os cartões da gaveta do pedido, a altura do SVG de `/vendas` e o rótulo da busca do shell (a RPC devolve sete entidades, o campo promete cinco). ⚠️ **Pendência de segurança em aberto:** a RLS de `erp_import_batches`/`rows` e as rotas `POST /v1/erp-imports` e `.../apply` continuam autorizando GESTOR, enquanto a tela já é de ADMIN (D-312). **Produção em dia.** |
