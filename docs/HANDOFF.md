...
| revisão de segurança e de secrets | **PARCIAL**: segredos e dependências (D-328); superfície de entrada conferida ao vivo, D-045 fechada e cabeçalhos da `web` (D-329). Falta CSP completa com nonce, redação de log por VALOR fora das telas, e confirmar os cabeçalhos na Vercel depois do próximo deploy da `web` |
