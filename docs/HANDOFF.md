...
| **Frente atual** | **A FRENTE VISUAL FECHOU EM ≈ 90% E VIROU LISTA DE ITENS ABERTOS**, e quatro já saíram. Antes disso: composição do Figma (D-281/D-282), campo e botão no design system com guarda (D-284), passo cinza (D-285), inbox de três colunas recusado com medição (D-286), auditoria de render A4 (D-283) e os quatro P3 (D-287). Depois: **fusão de `/cobertura` com `/reposicao`** (D-288 — uma tela só, definição de ruptura da reposição; das 325 "rupturas" da cobertura, **150 tinham Full ou trânsito**), **paginação de `/atendimento`** (D-289 — **829 dos 929 abertos** eram inalcançáveis), **recorte de não lidas em `/notificacoes`** (D-290 — 8.350 de 42.511, com paginação junto) a **lista de execuções que falharam** (D-291, **com migration**: `get_job_failures`, `security definer` restrita a ADMIN — `job_runs` segue com RLS e zero policies) e a **exportação de `/precos`** (D-292 — link para `GET /precos/export/xlsx`, que exporta o recorte da tela, pagina até 20.000 linhas e imprime o teto DENTRO do arquivo; a inspeção da planilha pegou o fuso, que saía 3h adiantado). A migration `20260909190000` **já está no Dev** (aplicada pela esteira no push da `v3`) e os tipos foram regerados pelo MCP. **Medição que vale para qualquer tela paginada:** o **416 `PGRST103`** de `.range()` além do fim só acontece com `count: exact` na MESMA consulta; sem `count` volta 200 vazio, e aí quem detecta é a aritmética. **O que resta:** a gaveta do Copiloto (D-276), que precisa de parâmetro de contexto na API e de ferramentas além de venda — backend antes de tela. Trilha 8B com P0 fechado (A–H) e em P1 |
