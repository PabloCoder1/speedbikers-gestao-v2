...
| **Frente atual** | **A FRENTE VISUAL ESTÁ EM ≈ 90%, E O QUE FALTA NÃO É DESENHO.** A composição do Figma fechou (D-281/D-282: as cinco gavetas), campo e botão entraram no design system com guarda (D-284: 194 controles, `check:control-styles`), o **passo cinza** foi dado (D-285), o **inbox de três colunas** foi recusado com medição (D-286: o que ele protegia virou `?volta=`), A4 fotografou as catorze telas de D22–D36 (D-283) e os **quatro P3** fecharam (D-287). **A fusão de `/cobertura` com `/reposicao` foi feita (D-288):** uma tela só, com a definição de ruptura da reposição — das 325 "rupturas" que a cobertura acusava, **150 tinham Full ou trânsito**; a rota virou redirect 308 com o `?marca=` junto. **A última fatia é a PAGINAÇÃO de `/atendimento` (D-289)**, primeiro item da lista de abertos: a fila lia as 100 mais recentes e nenhum filtro separava essas 100 do resto — **829 dos 929 abertos eram inalcançáveis**. De quebra ficou medido que `.range()` além do fim devolve **416 `PGRST103`** (página vermelha por link antigo), e **quatro telas com `.range()` têm a mesma exposição** — `isPageBeyondEnd` já está em `lib/filters.ts` para quando entrarem. **O que resta:** os itens abertos de `docs/DESIGN_IMPLEMENTATION.md`. Trilha 8B com P0 fechado (A–H) e em P1 |
