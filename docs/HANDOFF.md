...
| **HEAD conhecido** | `571da6f` (D-316) — **o vínculo de anúncio passou a ser gerido de dentro do SKU**, sobre a mesma fonte de `/vinculacoes`: a aba lê a régua canônica (o SKU do seed foi de 1 para 2 anúncios — o vínculo por VARIAÇÃO aparecia em `/produtos` e não aqui), ganhou Estoque e as ações Ver/Remover, e o "+ Vincular anúncio" confirma o MLB em `listings` antes de gravar — porque `create_sku_listing_link` aceita qualquer `MLB[0-9]+` sem FK e um dígito errado virava vínculo morto. Antes: `0fb16e8` (D-313, outra sessão) — `/vinculacoes` tinha parado de vincular em D-284. Antes: `cef72ee` (D-314) — a Cobertura do SKU passou a ter uma conta só. |
