...
| **HEAD conhecido** | `5f9c4e9` (D-278, **D37b** — o importador do UpSeller + varrimento de cabeçalho). A fila NOMEADA fechou em D36; o passe visual (D37) está em curso |
...
| **Frente atual** | **Frente visual, no passe global (D37)**. **D37a**: as três telas de DETALHE + `ProcessSteps` extraído no segundo consumidor (D-277). **D37b**: o importador do UpSeller (terceiro consumidor — a aplicação mede sobre `ok_rows`, não sobre o total) + varrimento de `PageTitle` em 8 telas (D-278). `check:table-styles` foi de **21 para 28**; telas sem `PageTitle`, de **17 para 3**. **Faltam três, e cada uma por um motivo diferente**: `/cobertura` (o frame a trata com Reposição como UMA tela de abas — composição, D-261), a tabela de ENTRADA de `/compras/novo` (células são campos) e `/atendimento/[caseId]` (509 linhas, pede `ObjectHeader`). ⚠️ O guarda cobre **uma metade só**: tela que nunca declarou `.sb-table` depende da captura. Trilha 8B com P0 fechado (A–H) e em P1 |
