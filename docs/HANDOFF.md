...
| **Frente atual** | Trilha 5J — central de inteligência do negócio (D-394 a D-408). Próximo: o detector de frete usar as medidas do pacote (depois de ver a cobertura real) e a notificação dos alertas (depois do PR #77). |

### O que está pronto

SKU/ML;visitas;migração

`/central` (D-394/D-395): indicadores contra o período anterior, meta do mês com projeção e imposto com vigência (cadastro em `/central/metas`). D-396 frete 100% desde 25/06; D-397/399 frete; D-398/401 Ads; D-400 atenção na central; D-402 ranking de produtos; D-403/D-404 alertas em `actions`, uma fonte por chamada; D-405 medidas do pacote por envio; D-406 datas comerciais na projeção (migration `20260923195955`); D-407 quem paga o frete (migration `20260923195956` ANTES do worker); D-408 limites em `/central/limites`. Próxima: notificação dos alertas (5J), depois do #77.
