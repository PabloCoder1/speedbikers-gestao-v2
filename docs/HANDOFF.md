...
| **Deploy no ar** | ✅ **api em `45be035`** (`api-00038-2hg`, 2026-09-14 02:21 UTC, D-342) e **worker em `d828eac`** (`worker-00050-qnt`, D-338) — o mesmo runtime (`git diff d828eac 45be035` vazio fora de `infra/` e `docs/`), com `hono` 4.13.7 (D-328) e a redação de log por valor (D-330). `GET /health` responde `{"commit":"45be035"}`, e o tráfego da api voltou ao **LATEST**. ⚠️ **`api-00037-bqb` é o código revertido de D-339** (D-340): **não mande tráfego para ela**, e nunca use `--to-latest` com tráfego fixo sem conferir `latestReadyRevisionName` (D-342). Para voltar: api `api-00036-5l4`; worker `worker-00049-r62`. **Piso de frescor no ar** (D-304): `v3-refresh-sales-metrics`, `35 * * * *`. **Esta linha envelhece sozinha** (D-070): `/health` só cobre a api — o worker se confere por `gcloud run services describe worker`, comparando a imagem com o `HEAD`. |
