...
| **HEAD conhecido** | `e263412` (D-321) — **A13: a gaveta do pedido em cartões**, com o chão cinza opcional (`Drawer chao`) só onde há cartão, e a célula "Logística" do frame trocada por frete e desconto do vendedor, que têm fonte. ⚠️ **Duas armadilhas de ambiente em `TESTING.md`, e cada uma custou uma rodada inteira:** `.env.local` aponta para o **Dev** e `NEXT_PUBLIC_*` é embutida no build (exporte as variáveis locais ANTES do `build`, senão o e2e reprova no login); e o OneDrive marca o `.next` como somente-leitura, e o `next build` morre em `EPERM` sem gerar build. Antes: `c330ae4` (D-320) "Últimas decisões" do SKU com autor e idade · `834be81` (D-318) · `1dd0638` (D-319) — **a lentidão de `/produtos` não era o tamanho da página.** `get_sku_curation` é `plpgsql` e não tinha `plan_cache_mode`: da **sexta execução da conexão** em diante o plano vira genérico e a chamada salta de ~20 ms para **360 a 4.000 ms** (a magnitude varia com a estatística; o salto, não). Com o genérico no lugar, 20/50/100/300 por página medem o mesmo — a troca de 100 para 50 de D-315 não acelerou nada, e agora isso está medido. Curado com uma linha, como D-305 e D-307. ⚠️ **Ficou aberta a varredura das outras `plpgsql`:** a de D-307 usou o critério de `language sql` (estado estável desproporcional), que não vale aqui — em `plpgsql` o sinal é a sexta execução. Antes: `2eb0a7a` (D-317) diagnóstico do SKU com régua · `571da6f` (D-316) vínculo dentro do SKU · `08900ff` (D-315) opções do UpSeller em `/produtos`. |
