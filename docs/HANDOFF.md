...
| **Branch** | `v3` (otimizações publicadas e verificadas) |
| **HEAD conhecido** | `2f5a158` na `v3`; código publicado `f88e0b2`, CI verde (incluindo E2E/RLS). O commit posterior só documenta a migration no Dev. Toda página nova precisa ser dinâmica (D-331): estática sai sem nonce. Armadilhas de ambiente/build em `docs/TESTING.md`. |
...
| **Produção** | Verificada em 15/09: GCP `speedbikers-prod`, API `api-00004-n7c` e worker `worker-00005-pcz` em `f88e0b2`, 100% do tráfego; health confirma commit. Vercel Production `dpl_4D8FTefUk3PueEJgsdyZVDBmqbmi`, mesmo commit. Supabase com 167 migrations. Webhooks só em produção; Dev pausado (D-350). `v3-reconcile-balances` continua pausado pela guarda de estoque pendente. |
| **Migrations** | **167 locais e aplicadas em Dev/produção** — última `20260915133738`, confirmada em 15/09; produção via workflow `35002134628`. No Dev, o caminho é o push na `v3` (a CI tem o job *aplicar migrations no Supabase Dev*, verde em 2026-09-14; D-257 registrava a integração do Supabase); **em produção, só `migrations-producao.yml`**, com duas aprovações (D-334). **Nunca** pelo MCP (D-207). O nome do arquivo precisa ser um **instante válido**, não só um número crescente: `...240000` (hora 24) deixou seis casos de `get_system_health` vermelhos, e é esse teste que serve de guarda (D-307) |
