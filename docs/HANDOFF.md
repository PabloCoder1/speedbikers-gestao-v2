...
| **HEAD conhecido** | `57099c9` (D-301) — o 404 do convite era a api de PRODUÇÃO três dias mais velha que a tela; a frase de erro passa a nomear o commit no ar. Antes: **D-299** (A8 — `/contas` contra o frame `Accounts`). Antes: `c6b18fc` (D-298, o convite que "não ia") e `50e4a21` (D-297, `/usuarios` refeita) |
...
| **Deploy no ar** | ⚠️ **`6baa641`, MEDIDO em 2026-09-10 contra o serviço real** (`GET /health` da api responde `{"commit":"6baa641","startedAt":"2026-09-07T13:18:48Z"}`), e o `HEAD` está **110 commits à frente** — **5** tocam `apps/api` e **32** tocam `apps/api` ou `packages/`, que ela empacota junto. **Isso não é abstrato: o botão de convidar usuário responde 404 em produção** porque a rota nasceu em `4ef8d18`, três dias depois do que está no ar (D-301). A api atende nos dois endereços (`api-925657154158.southamerica-east1.run.app` e `api-rrquw5upla-rj.a.run.app`) com o mesmo commit. **O worker não é medível daqui** — `/health` dele responde 403 sem credencial, então nada se afirma sobre a versão dele. Antes desta medição a linha dizia `0702969` e 76 commits (09/09), e antes disso "sem atraso" em 02/09 — a linha que envelhece sozinha é o risco de D-070. Deploy é manual, na ordem worker → api (D-088). |
