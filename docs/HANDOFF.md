...
| **Frente atual** | **A FRENTE VISUAL ESTÁ EM ≈ 90%, E O QUE FALTA NÃO É DESENHO.** A composição do Figma fechou (D-281/D-282: as cinco gavetas), campo e botão entraram no design system com guarda (D-284: 194 controles, `check:control-styles`), o **passo cinza** foi dado (D-285: chão `#f4f5fa` no `<main>`, medido em dez rotas), o **inbox de três colunas** foi recusado com medição (D-286: centro vazio em 50,7% das reclamações — o que ele protegia virou `?volta=`), A4 fotografou as catorze telas de D22–D36 (D-283) e os **quatro P3** fecharam (D-287: três feitos, a largura de `/integracoes` recusada com número). **A última fatia é a FUSÃO de `/cobertura` com `/reposicao` (D-288):** o frame sempre desenhou uma tela só, e a definição de ruptura escolhida foi a da reposição — aproveitável (local + Full + trânsito) contra a venda média, com lead time e cobertura alvo. O número que decidiu: das **325** "rupturas" que `/cobertura` acusava, **150 tinham Full ou trânsito** (as duas discordavam em **186 SKUs**). A rota virou redirect 308 com o `?marca=` junto, a navegação tem UMA entrada e a Home passou a dizer o que mede ("SKUs sem saldo local"). **O que resta:** os sete itens abertos de `docs/DESIGN_IMPLEMENTATION.md`. Trilha 8B com P0 fechado (A–H) e em P1 |
