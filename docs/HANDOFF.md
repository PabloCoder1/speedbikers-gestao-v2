...
| **Frente atual** | **Frente visual**: D18→D25, D27→D34 entregues (+A3/A3b). **Próxima: D35–D36** (Configurações, Copiloto); D26 (Tráfego) foi avaliada e RECUSADA por falta de fonte (D-266). ⚠️ **`/cobertura` nunca foi migrada** (D-261). O guarda `check:table-styles` (D-262) reprova a migração pela metade e cobre **uma metade só** — tela que nunca declarou `.sb-table` depende da captura. `/configuracoes` é a ÚLTIMA consumidora de `components/table-styles.ts` (D-272). Trilha 8B com P0 fechado (A–H) e em P1 |
