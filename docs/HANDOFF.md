...
| **Frente atual** | **A6 — AUDITORIA DA ADMINISTRAÇÃO (D-296)**, a pedido do usuário: as seis telas do grupo lidas contra o frame. **Feito:** convite de usuário (`POST /v1/organization/invites`, ADMIN; LINK em vez de e-mail porque não há SMTP) e as três colunas que D-271 recusou — Status, Último acesso e e-mail —, pela janela `get_organization_members` (definer, guard de ADMIN **daquela** organização). ⚠️ **Migration `20260910120000` pendente de `db push`** (a esteira aplica no push da `v3`) e tipos a regerar pelo MCP. **Próxima da auditoria: `/contas`, que NUNCA foi migrada** (não está em D0→D37); `/saude` tem seis cartões de serviço a medir; Integrações, Sincronização e Configurações seguem com recusa medida (D-272/D-287, D-273, D-275). Antes disso a frente visual fechou (D-281→D-287) e a lista de itens abertos saiu inteira: fusão `/cobertura`×`/reposicao` (D-288), paginação de `/atendimento` (D-289), não lidas em `/notificacoes` (D-290), execuções que falharam (D-291), exportação de `/precos` (D-292), contexto e ferramentas do Copiloto (D-293), a gaveta (D-294) e a republicação com confirmação humana (D-295). Trilha 8B com P0 fechado (A–H) e em P1 |
