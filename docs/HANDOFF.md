...
| **Frente atual** | **A FRENTE VISUAL ESTÁ EM ≈ 90%, E O QUE FALTA NÃO É DESENHO.** A composição do Figma fechou (D-281/D-282), campo e botão entraram no design system com guarda (D-284), o **passo cinza** foi dado (D-285), o **inbox de três colunas** foi recusado com medição (D-286), A4 fotografou as catorze telas de D22–D36 (D-283) e os **quatro P3** fecharam (D-287). Depois disso a frente virou **lista de itens abertos**, e três já saíram: a **fusão de `/cobertura` com `/reposicao`** (D-288 — uma tela só, com a definição de ruptura da reposição; das 325 "rupturas" que a cobertura acusava, **150 tinham Full ou trânsito**), a **paginação de `/atendimento`** (D-289 — **829 dos 929 abertos** eram inalcançáveis) e o **recorte de não lidas em `/notificacoes`** (D-290 — 8.350 de 42.511, com a paginação junto). **Medição que vale para qualquer tela paginada:** o **416 `PGRST103`** de `.range()` além do fim só acontece com `count: exact` na MESMA consulta — sem `count` volta 200 vazio, e aí quem detecta a página inexistente é a aritmética (`isPageBeyondEnd` em `lib/filters.ts` serve o primeiro caso). **O que resta:** os itens abertos de `docs/DESIGN_IMPLEMENTATION.md` (exportação de `/precos` — recusada em D-264, lista de execuções que falharam, gaveta do Copiloto). Trilha 8B com P0 fechado (A–H) e em P1 |
