...
| **Branch** | `v3` (D-354 em produção; D-356 na `v3`). D-357 a D-359 na `v3`/principal; D-363 (Mercado Ads) na `feat/faturamento` — pendente: migration `20260916165243` pelo workflow, deploy worker→api e `cloud-scheduler.sh` (job `v3-ads-campaigns-sync`). D-365 (`/compras` refeita) na `feat/compras-d365` — migration `20260917140000` pelo caminho de sempre; a tela funciona sem ela (fallback para `get_purchase_orders`) |
