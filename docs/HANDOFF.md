...
| **HEAD conhecido** | `c6b18fc` (D-298) — o convite que "não ia": `NEXT_PUBLIC_API_URL` ausente fazia o `fetch` bater no próprio Next. Antes: `50e4a21` (D-297) — `/usuarios` refeita contra o desenho: a tabela volta a ser tabela e o controle vai para a gaveta. Antes: `4ef8d18` (D-296) — A6: a auditoria da ADMINISTRAÇÃO e o convite de usuário. Antes: `b65cd6f` (D-295) — a superfície de confirmação humana da republicação. Antes: `1172eeb` (D-294) — a GAVETA do Copiloto, que fechou a lista de itens abertos da frente visual. Antes: `07d22df` (D-293), `8445cb9` (D-292), `816a53f` (D-291), `f83d03f` (D-290), `856daec` (D-289) e `1dd2c7b` (D-288) — os itens abertos, um por commit. Antes: `4ee0a5f` (D-279, D37c), quando a migração de superfícies fechou (D0→D25, D27→D36 + passe D37a/b/c; D26 recusada com medição, D-266) |
...
| **Frente atual** | **A7 — `/usuarios` REFEITA CONTRA O DESENHO (D-297):** o usuário comparou as duas telas ("o seu está muito inferior, acompanhe 100% o design figma") e o que ele viu tem número — **cinco controles por linha** (papel + uma caixa por conta). A tabela voltou a ser tabela: selo em Papel e Status, texto em contas, o **nome** abre a gaveta, e a EDIÇÃO mora nela (mesmos componentes, mesma autorização); entraram a busca e o menu "Status" do painel, na URL. **A6 (D-296) é a auditoria do grupo ADMINISTRAÇÃO**, a pedido do usuário: convite de usuário (`POST /v1/organization/invites`, ADMIN; LINK em vez de e-mail porque não há SMTP) e as três colunas que D-271 recusou, pela janela `get_organization_members` (definer, guard de ADMIN **daquela** organização). ⚠️ migration `20260910120000` empurrada na `v3` e **não conferida** no Dev daqui; tipos a regerar pelo MCP. **Próxima da auditoria: `/contas`, que NUNCA foi migrada** (não está em D0→D37); `/saude` tem seis cartões de serviço a medir; Integrações, Sincronização e Configurações seguem com recusa medida (D-272/D-287, D-273, D-275). Antes disso a frente visual fechou (D-281→D-287) e a lista de itens abertos saiu inteira: fusão `/cobertura`×`/reposicao` (D-288), paginação de `/atendimento` (D-289), não lidas em `/notificacoes` (D-290), execuções que falharam (D-291), exportação de `/precos` (D-292), contexto e ferramentas do Copiloto (D-293), a gaveta (D-294) e a republicação com confirmação humana (D-295). Trilha 8B com P0 fechado (A–H) e em P1 |
