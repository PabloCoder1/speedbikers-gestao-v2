...
| **HEAD conhecido** | `8612111` (D-322) — **A14: o gráfico de vendas com a altura do frame** (224px em `/vendas`, 165px na Home) e o texto fora do SVG — medido antes, o eixo caía a 2,4px numa tela de 375px. Primeiro e2e que olha para o gráfico. ⚠️ **Duas armadilhas de ambiente em `TESTING.md`, e cada uma custou uma rodada inteira:** `.env.local` aponta para o **Dev** e `NEXT_PUBLIC_*` é embutida no build (exporte as variáveis locais ANTES do `build`, senão o e2e reprova no login); e o OneDrive marca o `.next` como somente-leitura, e o `next build` morre em `EPERM` sem gerar build. Antes: `e263412` (D-321) gaveta do pedido em cartões · `c330ae4` (D-320) "Últimas decisões" com autor · `834be81` (D-318) · `1dd0638` (D-319) — **a lentidão de `/produtos` não era o tamanho da página.** `get_sku_curation` é `plpgsql` e não tinha `plan_cache_mode`: da **sexta execução da conexão** em diante o plano vira genérico e a chamada salta de ~20 ms para **360 a 4.000 ms** (a magnitude varia com a estatística; o salto, não). Com o genérico no lugar, 20/50/100/300 por página medem o mesmo — a troca de 100 para 50 de D-315 não acelerou nada, e agora isso está medido. Curado com uma linha, como D-305 e D-307. ⚠️ **Ficou aberta a varredura das outras `plpgsql`:** a de D-307 usou o critério de `language sql` (estado estável desproporcional), que não vale aqui — em `plpgsql` o sinal é a sexta execução. Antes: `2eb0a7a` (D-317) diagnóstico do SKU com régua · `571da6f` (D-316) vínculo dentro do SKU · `08900ff` (D-315) opções do UpSeller em `/produtos`. |
...
| **Frente atual** | **O pedido do dono sobre o Dashboard do SKU está entregue nas duas metades** (D-316 vínculos, D-317 diagnóstico). O que ficou registrado como SEM FONTE, e é o que impede a próxima pessoa de prometer: Shopee e a dimensão plataforma (D-037), divergência de estoque por conta (o interno é da organização, não há coluna de conta), tipo de anúncio, catálogo, logística, qualidade da publicação e substatus de pausa. ~~Uma régua espera decisão do dono~~ — **decidida em D-318** (10% sobre o menor, por organização). Da auditoria de design, a fila: ~~o painel "Últimas decisões" do SKU~~ (D-320), ~~os cartões da gaveta do pedido~~ (D-321), ~~a altura do SVG de `/vendas`~~ (D-322) e **o rótulo da busca do shell (em curso — medido: lista 5 das 7 entidades que `search_entities` busca desde D-216, é cortado a partir de 1100px, e as 7 por extenso não cabem nem a 1440px)**. **Duas sessões neste repo:** portas 3000/3100 separadas e `db reset` combinado. |
