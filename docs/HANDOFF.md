...
| **HEAD conhecido** | `08900ff` (D-315) — **`/produtos` ganhou as duas opções do UpSeller**: quantos por página (20/50/100/300, com o padrão caindo de 100 para **50**) e em que ordem (fila de curadoria — a de sempre, e continua sendo o padrão —, atualizados, criados), as duas na URL. **Com migration** (`20260911210000`): a ordem tinha de morar no banco, porque ordenar as 50 linhas que chegaram ordenaria a página, não o conjunto; e toda ordem termina em `sku` porque uma importação do ERP grava o mesmo `updated_at` em centenas de SKUs, e ordem não determinista com OFFSET repete e pula linha entre páginas. Antes: `571da6f` (D-316) — **o vínculo de anúncio passou a ser gerido de dentro do SKU**, sobre a mesma fonte de `/vinculacoes`, com a régua canônica e confirmação do MLB antes de gravar (`create_sku_listing_link` aceita qualquer `MLB[0-9]+` sem FK, e um dígito errado virava vínculo morto). Antes: `0fb16e8` (D-313) — `/vinculacoes` tinha parado de vincular em D-284. |
...
| **Migrations** | **165 locais** — a última é `20260911210000` (D-315). ⚠️ Quem aplica no Dev é a integração GitHub do Supabase, **não** a CI (D-257); o caminho é o push, **nunca** o MCP (D-207). O nome do arquivo precisa ser um **instante válido**, não só um número crescente: `...240000` (hora 24) deixou seis casos de `get_system_health` vermelhos, e é esse teste que serve de guarda (D-307) |
| **Frente atual** | **Duas frentes fecharam hoje: `/produtos` (D-315) e o vínculo dentro do SKU (D-316).** De D-315 fica UMA pergunta para o dono: a tabela de `/produtos` tem dez colunas e rola na horizontal a 1440px (1.220px de conteúdo em 1.116px de caixa; ela já rolava com nove), e encolher de verdade é escolher o que SAI — decisão de produto, não de implementação. **Pedido grande do dono, em duas metades — a primeira entregou (D-316).** Gerir vínculo dentro do SKU está feito: adicionar por MLB com confirmação, remover com confirmação, listar com conta/MLB/título/estado/estoque/preço, tudo sobre as RPCs que `/vinculacoes` já usava. **A segunda metade é o diagnóstico do SKU**, e ela tem quatro perguntas em aberto para o dono antes de codificar (teto de dispersão de preço e em que escopo; quatro níveis 🟢🟡🟠🔴 ou os três com régua que a casa já usa; "última análise" compartilhada ou da sessão; e se MLB fora do catálogo deve BLOQUEAR ou avisar). ⚠️ **Sem fonte, medido:** Shopee e a dimensão "plataforma" (D-037), tipo de anúncio, catálogo, logística, qualidade da publicação e divergência de estoque POR CONTA — o estoque interno não tem coluna de conta em lugar nenhum da cadeia. **Duas sessões trabalhando neste repo ao mesmo tempo:** porta 3000 para uma, 3100 para a outra, e `db reset` combinado. |
