...
| load tests e revisão de `pg_stat_statements` | **carga real medida (D-339)**: 65,8 mil webhooks/dia, pico de 1.050/min, e o ACK passava de 7 s nos picos — corrigido no código, **a confirmar no primeiro pico depois do deploy**. Falta a revisão de `pg_stat_statements` (`report:health` pede a senha do Dev — ato humano) |
