...
| revisão de segurança e de secrets | **PARCIAL**: segredos e dependências (D-328); superfície de entrada conferida ao vivo, D-045 fechada e cabeçalhos da `web` (D-329); redação de log por VALOR, com as regras de segredo compartilhadas entre logger e tela (D-330). Falta CSP completa com nonce, e confirmar os cabeçalhos na Vercel depois do próximo deploy da `web` |
