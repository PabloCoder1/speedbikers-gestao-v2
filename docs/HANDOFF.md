...
| **HEAD conhecido** | `e83914e` (D-311) — **A11**: a Home, a única superfície nunca remedida desde a primeira auditoria, ganhou o controle de janela que o frame põe no cabeçalho do gráfico e passou a mostrar a **idade do FATO** no feed (`domain_events.occurred_at`, não o carimbo do fan-out — o desvio máximo já medido entre as duas colunas foi de 278 dias). O vocabulário de período mudou de casa antes de ganhar o terceiro consumidor: `lib/period.ts` é o dono, `/vendas` apagou a cópia, e o tipo faz "14 dias" **não compilar**. Antes: `fed1fd0` (D-310) — o cabeçalho do anúncio: os dois fatos que ele devia desde D-168 e o caminho até a republicação. Antes: `66ef20d` (D-309) — os seis cartões de serviço de `/saude` remedidos e mantidos fora. |
...
| **Frente atual** | **Design, escolhendo por medição — e a rodada adversarial está derrubando tanto quanto aprovando.** Das oito diferenças atacadas em D-311 e na varredura do SKU, **cinco caíram**: as quatro do Dashboard de SKU (lead time, variação %, quadro de identidade, ressalva no subtítulo — todas já recusadas com motivo que continua verdadeiro) e a quinta célula da faixa da Home. ⚠️ **O achado mais valioso ainda não foi feito, e não é de design:** o cartão "Cobertura" de `/skus/[skuId]` e a gaveta Inspeção Rápida imprimem `local ÷ venda média`, a definição que **D-288 aposentou** — no seed são **300 dias contra os 318** que `/reposicao` mostra para o mesmo SKU. Dois números sob a mesma palavra. Depois dele, a fila de design: o painel "Últimas decisões" do SKU (o `kind`/`recommendation` vêm no mesmo select e não são impressos), os cartões da gaveta do pedido, a altura do SVG de `/vendas` e o rótulo da busca do shell (a RPC devolve sete entidades, o campo promete cinco). **Produção em dia.** |
