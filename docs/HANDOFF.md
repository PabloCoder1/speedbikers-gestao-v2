...
| **HEAD conhecido** | `d2c1be8` (D-291) — a lista de execuções que falharam, com migration pendente de `db push`. Antes: `f83d03f` (D-290) — o recorte de não lidas em `/notificacoes`, com a paginação junto. Antes: `856daec` (D-289) — a paginação da Caixa de Entrada, e o 416 do PostgREST tratado. Antes: `1dd2c7b` (D-288) — a FUSÃO de `/cobertura` com `/reposicao`: uma tela, uma definição de ruptura. Antes: `c468881` (D-287) — os quatro P3 de acabamento. Antes: `c4fad81` (D-286) — o inbox de três colunas, decidido com medição. Antes: `a725f1f` (D-285, **passo cinza**), o chão de `#f4f5fa` no `<main>`. Antes: `d949183` (D-284, **A5**), campo e botão no design system com guarda. Antes: A4 (D-283), auditoria de render de D22–D36. Antes: `9e0d06e` (D-282, **D39**) — as cinco gavetas do Figma entregues e **a composição do desenho fechou**. Antes: `4ee0a5f` (D-279, D37c), quando a migração de superfícies fechou (D0→D25, D27→D36 + passe D37a/b/c; D26 recusada com medição, D-266) |
...
| **Frente atual** | **A FRENTE VISUAL FECHOU EM ≈ 90% E VIROU LISTA DE ITENS ABERTOS**, e quatro já saíram. Antes disso: composição do Figma (D-281/D-282), campo e botão no design system com guarda (D-284), passo cinza (D-285), inbox de três colunas recusado com medição (D-286), auditoria de render A4 (D-283) e os quatro P3 (D-287). Depois: **fusão de `/cobertura` com `/reposicao`** (D-288 — uma tela só, definição de ruptura da reposição; das 325 "rupturas" da cobertura, **150 tinham Full ou trânsito**), **paginação de `/atendimento`** (D-289 — **829 dos 929 abertos** eram inalcançáveis), **recorte de não lidas em `/notificacoes`** (D-290 — 8.350 de 42.511, com paginação junto) e a **lista de execuções que falharam** (D-291, **com migration**: `get_job_failures`, `security definer` restrita a ADMIN — `job_runs` segue com RLS e zero policies). ⚠️ **PENDENTE de D-291:** `supabase db push` da migration `20260909190000` para o Dev e regeração dos tipos pelo MCP — `packages/db/src/types.ts` carrega a entrada MANUAL da RPC até lá. **Medição que vale para qualquer tela paginada:** o **416 `PGRST103`** de `.range()` além do fim só acontece com `count: exact` na MESMA consulta; sem `count` volta 200 vazio, e aí quem detecta é a aritmética. **O que resta:** exportação de `/precos` (recusada em D-264) e a gaveta do Copiloto (D-276). Trilha 8B com P0 fechado (A–H) e em P1 |
