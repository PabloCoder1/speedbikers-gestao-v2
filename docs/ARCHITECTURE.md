...
- `POST /webhooks/mercado-livre` — ACK dentro dos 500 ms do Mercado Livre, e enfileira. Tópico sem consumidor responde sem I/O; os com trabalho fazem duas idas de rede antes do ACK (a conta no Postgres e a Cloud Task) — D-339.
...
**`support` (Fase 7B, D-071/D-084 a D-088 — núcleo de banco, persistência e ingestão de Perguntas pelo webhook implementados):** Central de Atendimento/SAC. `support_cases` é a projeção unificada da caixa de entrada, sem apagar a semântica externa: um case é uma pergunta, uma conversa pós-venda por pack/pedido ou um claim; mediação e devolução são facetas do claim, nunca recursos raiz inventados. `support_messages` mantém o transcript atual; `support_case_events` é auditoria append-only; somente transições escolhidas serão promovidas a `domain_events support.*` para notificação/diagnóstico. As seis tabelas do núcleo read-only existem desde D-085. D-086 acrescentou contrato/mapper puro no pacote Mercado Livre e persistência idempotente de Perguntas no worker; D-087 registrou o job `sync.support.questions`; D-088 ligou o produtor — o ACK do webhook roteia `topic=questions` direto para esse job, e por isso o ACK passou a conhecer tópico. **Enfileirar é da `api`, não do worker**: o worker só recebe `cloudtasks.enqueuer` em `backfill`/`analytics-recompute` (secao 11), nunca em `ml-sync-<conta>`, que é a fila onde este job precisa entrar (D-036). Reconciliação por busca, triagem, UI e resposta continuam pendentes.
...
Webhook ML -> api  POST /webhooks/mercado-livre
   ACK < 500ms (regra do ML). Tópico sem consumidor: ACK sem I/O (D-339).
   Tópico com trabalho: resolve a conta e cria Cloud Task
