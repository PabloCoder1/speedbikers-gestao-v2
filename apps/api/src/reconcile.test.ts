...
function fakeDb(options: { accountsFail?: boolean } = {}): ReconcileDeps["db"] {
  return {
    from: () => ({
      select: () => ({
        eq: () =>
...
function deps(options: {
...
} = {}): { deps: ReconcileDeps; enqueued: EnqueueRequest[] } {
  ...
  return {
    ...
    deps: {
      ...
      now: () => new Date("2026-08-21T15:37:00.000Z"),
      enqueuer: {
        enqueue: (request) => {
...
describe("triggerOrdersReconciliation", () => {
  it("enfileira sync.orders.window para cada conta CONNECTED, uma vez cada", async () => {
    ...
    expect(enqueued[0]).toMatchObject({
      ...
      dedupeKey: "sync-orders:speedbikers-loja-1:2026-08-21T15",
      payload: { mlAccountId: ACCOUNTS[0]?.id },
    });
  });

  it("a chave de dedupe é a hora cheia — mesma hora produz a mesma chave", async () => {
    const { deps: d, enqueued } = deps();

    await triggerOrdersReconciliation(d);

    // 15:37 vira o bucket "2026-08-21T15", igual para qualquer minuto dentro da hora.
    expect(enqueued.every((e) => e.dedupeKey.endsWith(":2026-08-21T15"))).toBe(true);
  });
