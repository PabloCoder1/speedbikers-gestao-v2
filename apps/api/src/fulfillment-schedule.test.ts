...
function fakeDb(options: { accountsFail?: boolean } = {}): FulfillmentScheduleDeps["db"] {
  return {
    from: () => ({
      select: () => ({
        eq: () =>
...
function deps(options: {
...
} = {}): { deps: FulfillmentScheduleDeps; enqueued: EnqueueRequest[] } {
  ...
  return {
    ...
    deps: {
      ...
      now: () => new Date("2026-08-22T18:15:00.000Z"),
      enqueuer: {
        enqueue: (request) => {
          ...
          return Promise.resolve({
            ...
            envelope: {
              ...
              attempt: 1,
              enqueuedAt: "2026-08-22T18:15:00.000Z",
            },
          });
        },
      },
...
describe("triggerFulfillmentSnapshot", () => {
  it("enfileira sync.fulfillment.snapshot para cada conta CONNECTED, uma vez cada", async () => {
    ...
    expect(enqueued[0]).toMatchObject({
      ...
      dedupeKey: "full:speedbikers-loja-1:2026-08-22T18",
  ...
  it("a chave de dedupe é a hora cheia — mesma hora produz a mesma chave", async () => {
    ...
    await triggerFulfillmentSnapshot(d);

    expect(enqueued.every((e) => e.dedupeKey.endsWith(":2026-08-22T18"))).toBe(true);
  });
