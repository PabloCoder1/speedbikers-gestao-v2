...
function fakeDb(options: { accountsFail?: boolean } = {}): ListingsScheduleDeps["db"] {
  return {
    from: () => ({
      select: () => ({
        eq: () =>
...
} = {}): { deps: ListingsScheduleDeps; enqueued: EnqueueRequest[] } {
...
describe("triggerListingsSnapshot (D-058)", () => {
  it("enfileira sync.listings.snapshot para cada conta CONNECTED, uma vez cada", async () => {
    const { deps: d, enqueued } = deps();

    const outcome = await triggerListingsSnapshot(d);
    ...
    expect(enqueued[0]).toMatchObject({
      ...
      dedupeKey: "listings:speedbikers-loja-1:2026-08-23T18",
  ...
  it("a chave de dedupe é a hora cheia — mesma hora produz a mesma chave", async () => {
    ...
    await triggerListingsSnapshot(d);

    expect(enqueued.every((e) => e.dedupeKey.endsWith(":2026-08-23T18"))).toBe(true);
  });
