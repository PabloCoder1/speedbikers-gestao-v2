...
function fakeDb(options: { organizationsFail?: boolean } = {}): BalanceReconcileScheduleDeps["db"] {
  return {
    from: () => ({
      select: () =>
        Promise.resolve(
  ...
  } as unknown as BalanceReconcileScheduleDeps["db"];
}

function deps(
  options: { organizationsFail?: boolean; deduplicateOrgId?: string } = {},
): { deps: BalanceReconcileScheduleDeps; enqueued: EnqueueRequest[] } {
...
describe("triggerBalanceReconciliation", () => {
  it("enfileira maintenance.reconcile-balances para cada organização, uma vez cada", async () => {
    const { deps: d, enqueued } = deps();

    const outcome = await triggerBalanceReconciliation(d);
    ...
    expect(enqueued[0]).toMatchObject({
      ...
      queue: "maintenance",
      dedupeKey: "reconcile-balances:org-1:2026-08-23",
  ...
  it("a chave de dedupe é a data de negócio (America/Sao_Paulo), não a hora — cadência diária", async () => {
    ...
    await triggerBalanceReconciliation(d);
  ...
  it("sem nenhuma organização, não enfileira nada", async () => {
    ...
    d.db = { from: () => ({ select: () => Promise.resolve({ data: [], error: null }) }) } as unknown as
      BalanceReconcileScheduleDeps["db"];
